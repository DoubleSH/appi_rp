...
lsc.carPos = [
    ...
    [1767.5126953125, 3329.434814453125, 40.43852615356445, 169],
    [-217.70619201660156, 6201.20263671875, 30.489940643310547, 170],
    [477.60467529296875, -1281.692138671875, 28.539329528808594, 171],
    [-1410.029052734375, -276.8239440917969, 45.36894989013672, 172],
    [146.7510528564453, 320.76947021484375, 111.1386489868164, 173],
    [135.43988037109375, -1050.6561279296875, 28.15288734436035, 174],

    [5188.09912109375, -5133.18603515625, 2.3411810398101807, 161], //Cayo Lsc
    [5180.60791015625, -5131.7685546875, 2.3138363361358643, 161],

    [4919.8505859375, -5238.38525390625, 1.5227940082550049, 164], //Cayo AutoRepair
...
lsc.loadAll = function() {
    ...
    lsc.list.forEach(function (item) {
        ...
        if (item[3] == 145 || item[3] == 146 || item[3] >= 169 || item[3] == 999)
...
lsc.getInRadius = function(pos, radius = 2) {
    methods.debug('lsc.getInRadius', pos, radius);
    let shopId = -1;
...
lsc.findNearest = function(pos) {
    methods.debug('lsc.findNearest', pos);
...
lsc.repair = function(player, price, shopId, payType) {
    methods.debug('lsc.repair', price, shopId, payType);
...
lsc.buyNeon = function(player, price, shopId, payType) {
    methods.debug('lsc.buyNeon', price, shopId, payType);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.buyTyreColor = function(player, price, idx, shopId, payType) {
    methods.debug('lsc.buyNeon', price, idx, shopId, payType);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.buyLight = function(player, price, shopId, payType) {
    methods.debug('lsc.buyLight', price, shopId, payType);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.buyNumber = function(player, shopId, newNumber, payType) {
    methods.debug('lsc.buyNumber', shopId, newNumber, payType);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
    ...
    if (vehicles.set(veh.getVariable('container'), 'number_type') < 0) {
        user.showCustomNotify(player, 'Перед тем как установить, необходимо оформить транспорт в здании правительства', 1, 9);
        return;
    }
...
lsc.buySTun = function(player, modType, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buySTun', modType, idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    ...
    if (modType === 1)
        vSync.setCamberState(veh, idx);
    if (modType === 2)
        vSync.setSuspensionRaiseState(veh, idx);
    if (modType === 3)
        vSync.setSteeringLockState(veh, idx);
...
lsc.buySFix = function(player, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buySTun', idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    ...
    if (idx === 's_whel_1')
        vSync.setVehicleTyreState(veh, 0, 0);
    if (idx === 's_whel_2')
        vSync.setVehicleTyreState(veh, 1, 0);
    if (idx === 's_whel_3')
        vSync.setVehicleTyreState(veh, 4, 0);
    if (idx === 's_whel_4')
        vSync.setVehicleTyreState(veh, 5, 0);
    if (idx === 's_whel_5')
        vSync.setVehicleTyreState(veh, 2, 0);
    if (idx === 's_whel_6')
        vSync.setVehicleTyreState(veh, 3, 0);
...
lsc.resetSTun = function(player, modType) {
    try {
        methods.debug('lsc.resetSTun', modType);
        if (!user.isLogin(player))
            return;
        ...
        if (!vehicles.exists(veh)) {
            user.showCustomNotify(player, 'Нужно находиться в транспорте', 1, 9);
            return;
        }
        ...
        let car = vehicles.getData(veh.getVariable('container'));
        let upgrade = JSON.parse(car.get('upgrade'));
        if(upgrade[modType.toString()] == -99) return user.showCustomNotify(player, `У вас не установлена данная деталь`, 1, 9)
        upgrade[modType.toString()] = -99;
        vehicles.set(veh.getVariable('container'), 'upgrade', JSON.stringify(upgrade));

        vehicles.save(veh.getVariable('container'));

        if (modType === 101)
            vSync.setCamberState(veh, 0.0);
        if (modType === 102)
            vSync.setSuspensionRaiseState(veh, 0.0);
        if (modType === 103)
            vSync.setSteeringLockState(veh, 0.6);

        let lscSNames = [400000, 40000, 20000, 10000, 2000, 2000];
        let money = Math.round(lscSNames[modType-100] * 0.8);
        //user.addCashMoney(player, money, 'Компенсация за ЧИП тюнинг VID: ' + vehicles.getContainer(veh.id, 'container'));
        user.showCustomNotify(player, 'Вы сняли деталь', 0, 9);
    }
    catch (e) {
        methods.error(`catch in lsc.resetSTun`, e.toString())
    }
};

lsc.buyTun = function(player, modType, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buyTun', modType, idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    ...
    if (modType == 76) {
        ...
        if (idx < 0)
        {
            veh.setVariable('sbreak', false);
            vehicles.set(veh.getVariable('container'), 's_h_break', 0);
        }
        else {
            veh.setVariable('sbreak', true);
            vehicles.set(veh.getVariable('container'), 's_h_break', 1);
        }
    }
    else {
        let car = vehicles.getData(veh.getVariable('container'));
...
lsc.showNumberType = function(player, idx) {
    methods.debug('lsc.showNumberType', idx);
    try {
        if (!user.isLogin(player))
            return;
        let veh = player.vehicle;
...
lsc.buyNumberType = function(player, idx, price, shopId, payType) {
    methods.debug('lsc.buyNumberType', idx, price, shopId, payType);
    if (!user.isLogin(player))
        return;
    ...
    if (vehicles.set(veh.getVariable('container'), 'number_type') < 0) {
        user.showCustomNotify(player, 'Перед тем как установить, необходимо оформить транспорт', 1, 9);
        return;
    }
...
lsc.showColor1 = function(player, idx) {
    methods.debug('lsc.showColor1', idx);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.showColor2 = function(player, idx) {
    methods.debug('lsc.showColor2', idx);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.showColor3 = function(player, idx) {
    methods.debug('lsc.showColor3', idx);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.showColor4 = function(player, idx) {
    methods.debug('lsc.showColor4', idx);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.showColor5 = function(player, idx) {
    methods.debug('lsc.showColor5', idx);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.showColor6 = function(player, idx) {
    methods.debug('lsc.showColor6', idx);
    if (!user.isLogin(player))
        return;
    let veh = player.vehicle;
    if (!vehicles.exists(veh))
...
lsc.buyColor1 = function(player, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buyColor1', idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
...
lsc.buyColor2 = function(player, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buyColor2', idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    if (user.getMoney(player, payType) < price) {
        user.showCustomNotify(player, 'У вас недостаточно средств', 1, 9);
...
lsc.buyColor3 = function(player, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buyColor3', idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    if (user.getMoney(player, payType) < price) {
        user.showCustomNotify(player, 'У вас недостаточно средств', 1, 9);
...
lsc.buyColor4 = function(player, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buyColor4', idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    if (user.getMoney(player, payType) < price) {
        user.showCustomNotify(player, 'У вас недостаточно средств', 1, 9);
...
lsc.buyColor5 = function(player, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buyColor5', idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    if (user.getMoney(player, payType) < price) {
        user.showCustomNotify(player, 'У вас недостаточно средств', 1, 9);
...
lsc.buyColor6 = function(player, idx, price, shopId, itemName, payType) {
    methods.debug('lsc.buyColor6', idx, price, shopId, itemName, payType);
    if (!user.isLogin(player))
        return;
    if (user.getMoney(player, payType) < price) {
        user.showCustomNotify(player, 'У вас недостаточно средств', 1, 9);
...
lsc.checkValidNumber = function(number) {
    methods.debug('lsc.checkValidNumber', number);
