...
tattoo.getInRadius = function(pos, radius = 2) {
    methods.debug('tattoo.getInRadius', pos, radius);
    let shopId = -1;
...
tattoo.findNearest = function(pos) {
    methods.debug('tattoo.findNearest', pos);
...
tattoo.buy = function(player, collection, overlay, zone, price, itemName, shopId, payType) {
    methods.debug('tattoo.buy', collection, overlay, zone, price, itemName, shopId, payType);
...
tattoo.destroy = function(player, collection, overlay, zone, price, itemName, shopId, payType) {
    methods.debug('tattoo.destroy', collection, overlay, zone, price, itemName, shopId, payType);
    if (!user.isLogin(player))
        return;
