...
bank.addBankHistory = function(userId, card, text, price) {
    try {
        methods.debug('bank.addBankHistory', userId, card, text, price);
...
bank.transferMoney = function(player, bankNumber, money) {
    ...
    methods.debug('bank.transferMoney', bankNumber, money);
    ...
    setTimeout(function () {
        try {
            methods.debug('bank.transferMoney.setTimeout');
            if (!isOnline) {
...
bank.transferCryptoMoney = function(player, bankNumber, money) {
    ...
    setTimeout(function () {
        methods.debug('bank.transferCryptoMoney.setTimeout');
        if (!isOnline) {
...
bank.changePin = function(player, pin) {
    methods.debug('bank.changePin', pin);
    if (!user.isLogin(player))
        return;
...
bank.withdraw = function(player, money, procent = 0) {
    methods.debug('bank.withdraw', money, procent);
...
bank.deposit = function(player, money, procent = 0) {
    methods.debug('bank.deposit', money, procent);
    procent = methods.parseInt(procent);
    if (!user.isLogin(player))
        return;
...
bank.addBusinessBankMoneyByCard = function(prefix, money) {
    methods.debug('bank.addBusinessBankMoneyByCard', prefix, money);
    switch (prefix)
    {
        case 6000:
...
bank.openCard = function(player, bankId, price) {
    methods.debug('bank.openCard', bankId, price);
    if (!user.isLogin(player))
        return;
...
bank.getInRadius = function(pos, radius = 2) {
    methods.debug('bank.getInRadius', pos, radius);
    let stationId = -1;
    bank.markers.forEach(function (item) {
...
bank.getGrabInRadius = function(pos, radius = 1.7) {
    methods.debug('bank.getGrabInRadius', pos, radius);
    let idx = 0;
    let result = -1;
    bank.grabPos.forEach(function (item) {
        let fuelStationShopPos = new mp.Vector3(item[0], item[1], item[2]);
...
bank.getBombInRadius = function(pos, radius = 1.7) {
    methods.debug('bank.getBombInRadius', pos, radius);
    let idx = 0;
    let result = {
        idx: -1,
        type: -1,
...
bank.getHackDoorInRadius = function(pos, radius = 1.2) {
    methods.debug('bank.getHackDoorInRadius', pos, radius);
    let idx = 0;
    let result = -1;
    bank.doorPos2.forEach(function (item) {
        let fuelStationShopPos = new mp.Vector3(item[0], item[1], item[2]);
...
bank.getLockPickDoorInRadius = function(pos, radius = 1.2) {
    methods.debug('bank.getLockPickDoorInRadius', pos, radius);
    let idx = 0;
    let result = -1;
    bank.doorPosLockPick.forEach(function (item) {
        let fuelStationShopPos = new mp.Vector3(item[0], item[1], item[2]);
...
bank.hackFleecaDoor = function(player) {
    methods.debug('bank.hackFleecaDoor');
    if (!user.isLogin(player))
        return;
    ...
    if (doorId >= 0) {
        ...
        setTimeout(function () {
            if (!user.isLogin(player))
                return;
            methods.debug('bank.hackFleecaDoor.setTimeout1');
            ...
            setTimeout(function () {
                if (!user.isLogin(player))
                    return;
                methods.debug('bank.hackFleecaDoor.setTimeout2');
                try {
...
bank.lockPickDoor = function(player, radius = 3) {
    methods.debug('bank.lockPickDoor', radius);
    if (!user.isLogin(player))
        return;
    ...
    if (doorId >= 0) {
        ...
        setTimeout(function () {
            try {
                methods.debug('bank.lockPickDoor.setTimeout');
                if (!user.isLogin(player))
                    return;
                user.stopAnimation(player);
                user.blockKeys(player, false);
...
bank.findNearest = function(pos) {
    methods.debug('bank.findNearest', pos);
...
bank.findNearestFleeca = function(pos) {
    methods.debug('bank.findNearestFleeca', pos);
    let prevPos = new mp.Vector3(9999, 9999, 9999);
    bank.fleecaMarkers.forEach(function (item,) {
        let fuelPos = new mp.Vector3(item[0], item[1], item[2]);
        if (methods.distanceToPos(fuelPos, pos) < methods.distanceToPos(prevPos, pos))
