...
vShop.loadAllShop = () => {
    methods.debug('vShop.loadAllShop');
...
vShop.loadAllShopVehicles = () => {
    ...
    methods.debug('vShop.loadAllShopVehicles');
    /*enums.autoSell.forEach(item => {
        ...
        }, new mp.Vector3(item[0], item[1], item[2]), item[3], 'Toros');
    });*/
...
vShop.getInRadius = function(pos, radius = 2) {
    methods.debug('vShop.getInRadius', pos, radius);
    let shopId = -1;
...
vShop.checkPosForOpenMenu = function(player) {
    methods.debug('vShop.checkPosForOpenMenu');
    try {
...
vShop.findNearest = function(pos) {
    methods.debug('vShop.findNearest', pos);
...
vShop.buy = function(player, model, color1, color2, shopId, payType) {
    try {
        methods.debug('vShop.buy', model, color1, color2, shopId, payType);
        ...
        setTimeout(function () {
            methods.debug('vShop.buy.setTimeout1');
            if (user.isLogin(player))
        ...
        mysql.executeQuery(`SELECT * FROM cars WHERE user_id = '0' AND name = '${vInfo.display_name}' ORDER BY rand() LIMIT 1`, function (err, rows, fields) {
            ...
            rows.forEach(row => {
                try {
                    ...
                    setTimeout(function () {
                        methods.debug('vShop.buy.setTimeout2');
                        if (user.isLogin(player)) {
        ...
        methods.error(`catch in vShop.buy`, e.toString())
    }
};

vShop.sendNotify = function(player, shopId, title, text) {
    methods.debug('vShop.sendNotify')
    if (user.isLogin(player)) {
        try {
            let shopItem = enums.carShopList[shopId];
            ...
            methods.error('vShop.sendNotify', shopId, e.toString())
        }
    }
};
...
vShop.rent = function(player, model, color1, color2, shopId) {
    ...
    methods.debug('vShop.rent', model, color1, color2, shopId);
    ...
    try {
        ...
        methods.error('vShop.rent', e.toString())
    }
};