...
inventory.loadAll = function() {
    ...
    methods.debug('inventory.loadAll');
...
inventory.deleteWorldItems = function() {
    methods.debug('inventory.deleteWorldItems');
...
inventory.getVehicleHash = function(vehicle) {
    methods.debug('inventory.deleteWorldItems');
    if (vehicles.exists(vehicle))
        return mp.joaat(vehicle.numberPlate.trim());
    return '';
};

inventory.getItemList = function(player, ownerType, ownerId, isFrisk = false, justUpdate = false) {

    methods.debug('inventory.getItemList', ownerType, ownerId, isFrisk, justUpdate);
    ...
    try {
        ...
        let sql = `SELECT * FROM items WHERE owner_id = '${ownerId}' AND owner_type = '${ownerType}'${addWhere} ORDER BY is_equip DESC, item_id`;
        if (ownerType === 1 && ownerId === user.getId(player))
            sql = `SELECT * FROM items WHERE owner_id = '${ownerId}' AND owner_type = '${ownerType}'${addWhere} ORDER BY is_equip DESC, item_id`;
        if (ownerId == 0 && ownerType == 0)
            sql = `SELECT * FROM items WHERE ST_DISTANCE(POINT(pos_x, pos_y), POINT(${player.position.x}, ${player.position.y})) < 2 AND owner_type = 0 ORDER BY is_equip DESC, item_id`; //TODO DISTANCE MARIADB
            ...
            try {
                rows.forEach(row => {
                    ...
                    try {
                        ...
                        methods.error('inventory.getItemList:rows.forEach', e);
                    }
                });
                ...
                methods.error('inventory.getItemList:mysql.executeQuery', e);
            }
        });
    } catch(e) {
        methods.error(e);
...
inventory.getItemListSell = function(player) {
    ...
    methods.debug('inventory.getItemListSell');
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        try {

            methods.debug('inventory.getItemListSell.setTimeout');
            let data = [];
            //let data2 = new Map();
...
inventory.getItemListTrade = function(player, ownerId, ownerType) {
    ...
    methods.debug('inventory.getItemListTrade', ownerId, ownerType);
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        try {

            methods.debug('inventory.getItemListTrade.setTimeout');
            let data = [];
            //let data2 = new Map();
...
inventory.getItemListGunTranferSell = function(player) {
    methods.debug('inventory.getItemListGunTranferSell');
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        try {

            methods.debug('inventory.getItemListGunTranferSell.setTimeout');
            let data = [];
            //let data2 = new Map();
...
inventory.getItemListGunFix = function(player) {
    methods.debug('inventory.getItemListGunFix');
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        try {

            methods.debug('inventory.getItemListGunFix.setTimeout');
            let data = [];
            //let data2 = new Map();
...
inventory.getItemListGunColor = function(player) {
    methods.debug('inventory.getItemListGunColor');
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        try {

            methods.debug('inventory.getItemListGunColor.setTimeout');
            let data = [];
            //let data2 = new Map();
...
inventory.getItemListGunFixFree = function(player) {
    methods.debug('inventory.getItemListGunFixFree');
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        try {

            methods.debug('inventory.getItemListGunFixFree.setTimeout');
            let data = [];
            //let data2 = new Map();
...
inventory.getItemListClothTranferSell = function(player) {
    methods.debug('inventory.getItemListClothTranferSell');
    if (!user.isLogin(player))
        return;
    try {
...
inventory.getItemListSellFish = function(player, shopId) {
    methods.debug('inventory.getItemListSellFish', shopId);
    if (!user.isLogin(player))
        return;
    try {
...
inventory.unEquip = function(player, id, itemId) {
    methods.debug('inventory.unEquip', id, itemId);
    if (itemId === 50) {
...
inventory.upgradeWeapon = function(player, id, itemId, weaponStr, ownerType, ownerId) {

    methods.debug('inventory.upgradeWeapon', id, itemId, weaponStr);
    if (!user.isLogin(player))
        return;
    try {
        ...
        mysql.executeQuery(sql, function (err, rows, fields) {
            ...
            user.giveWeaponComponent(player, methods.parseInt(wpHash), methods.parseInt(hashModifer), wpName);
...
inventory.fixItem = function(player, id) {
    methods.debug('inventory.fixItem', id);
    if (!user.isLogin(player))
        return;
    try {
...
inventory.colorItem = function(player, id, tint, sTint) {
    methods.debug('inventory.fixItem', id, tint, sTint);
    if (!user.isLogin(player))
        return;
    try {
        let count = 5;
...
inventory.fixItemFree = function(player, id) {
    methods.debug('inventory.fixItemFree', id);
    if (!user.isLogin(player))
        return;
    try {
        inventory.updateItemCount(id, 100);
...
inventory.craft = function(player, id, itemId, countItems, count, params) {
    ...
    methods.debug('inventory.craft', id, itemId, countItems, count, params);
    if (!user.isLogin(player))
        return;
    try {
        let sql2 = '';
        ...
        mysql.executeQuery(sql, function (err, rows, fields) {
            ...
            items.recipes[id].craft.forEach(item => {
                setTimeout(function () {
                    methods.debug('inventory.craft.setTimeout1');
            ...
            setTimeout(function () {
                methods.debug('inventory.craft.setTimeout2');
                inventory.getItemList(player, inventory.types.Player, user.getId(player));
            }, 1000);
        });
    } catch (e) {
...
inventory.equip = function(player, id, itemId, count, aparams, ownerType, ownerId) {

    methods.debug('inventory.equip', id, itemId, count, aparams, ownerType, ownerId);
    ...
    try {
        ...
        mysql.executeQuery(sql, function (err, rows, fields) {
            if (rows.length === 0) {
                user.showCustomNotify(player, `Этот предмет вам не принадлежит`, 1, 9);
                user.callCef(player, 'inventory', JSON.stringify({ type: 'removeItemId', itemId: id }));
                return;
            }

            if (rows[0]['is_equip'] === 1 && itemId === 50) {
                user.showCustomNotify(player, `Карта уже экипирована`, 1, 9);
                return;
            }
            ...
            if (itemId == 50) {
                if (user.get(player, 'bank_card') == 0) {
                    ...
                    user.save(player).then(
                        result => alert("Fulfilled: " + result),
                        error => methods.error("Rejected: " + error)
                      );
                }
                else {
                    user.showCustomNotify(player, `Карта уже экипирована, для начала снимите текущую`, 1, 9);
                    return;
                }
            }
                ...
                if (user.get(player, 'phone_type') === 0) {
                    ...
                    user.showCustomNotify(player, `Телефон уже экипирован, для начала снимите текущий`, 1, 9);
                    return;
                }
            }
                ...
                try {
                    ...
                    if (user.get(player, 'weapon_' + slot) == '') {
                        ...
                        setTimeout(() => {
                            inventory.getItemList(player, 1, user.get(player,'id'))
                            ...
                            let wpNamed = items.getItemNameHashById(itemId)
                            player.removeAllWeaponComponents(wpHash);
                            user.setWeaponTint(player,wpHash,0,wpNamed)
                            if (params.slot1){
                                player.giveWeaponComponent(wpHash, params.slot1hash, wpNamed);
                            }
                                
                            if (params.slot2){
                                player.giveWeaponComponent(wpHash, params.slot2hash, wpNamed);
                            }
                                
                            if (params.slot3){
                                player.giveWeaponComponent(wpHash, params.slot3hash, wpNamed);
                            }
                                
                            if (params.slot4){
                                player.giveWeaponComponent(wpHash, params.slot4hash, wpNamed);
                            }
                                
                            if (params.superTint){
                                player.giveWeaponComponent(wpHash, params.superTint, wpNamed);
                            }
                                
                            if (params.tint){
                                user.setWeaponTint(player,wpHash,params.tint,wpNamed)
                            }
                        ...
                        user.showCustomNotify(player, `Слот под оружие уже занят`, 1, 9);
                        return;
                    }
                }
                catch (e) {
                ...
                inventory.updateEquipStatus(id, true);
                // params.userName = user.getRpName(player);
                ...
                if (params.sex !== user.getSex(player)) {
                    user.showCustomNotify(player, `Одежда подходит только для противоположного пола`, 1, 9);
                    return;
                }
                //inventory.updateEquipStatus(id, true);
               ...
               // inventory.updateEquipStatus(id, true);
                ...
                if (user.get(player, 'mask') == -1) {
                    ...
                    user.showCustomNotify(player, `Одежда уже экипирована, для начала снимите текущую`, 1, 9);
                    return;
                }
            }
            else if (itemId == 252) {

                if (params.armor === 60 || params.armor === 59) {
                    if (!user.isGos(player)) {
                        user.showCustomNotify(player, `Вам запрещено надевать бронежилеты гос-структур`, 1, 9);
                        return;
                    }
                }
                inventory.updateEquipStatus(id, true);
            ...
            // setTimeout(function () {
            //     try {
            //         methods.debug('inventory.craft.setTimeout2 itemId == 252');
            //         inventory.updateEquipStatus(id, true);
            //     }
            //     catch (e) {}
            // }, 1000)
...
inventory.updateEquipStatus = function(id, status) {
    methods.debug('inventory.updateEquipStatus', id, status);
    try {
        let newStatus = 0;
        if (status == true)
            newStatus = 1;
...
inventory.updateItemsEquipByItemId = function(itemId, ownerId, ownerType, equip, count = -1) {
    methods.debug('inventory.updateItemsEquipByItemId', itemId, ownerId, ownerType, equip, count);
    try {
        if (count >= 0)
...
inventory.updateOwnerId = function(id, ownerId, ownerType) {
    methods.debug('inventory.updateOwnerId', id, ownerId, ownerType);
    try {
...
inventory.updateOwnerIdWithPrice = function(id, ownerId, ownerType, price) {
    methods.debug('inventory.updateOwnerIdWithPrice', id, ownerId, ownerType, price);
    try {
...
inventory.updateOwnerAll = function(oldOwnerId, oldOwnerType, ownerId, ownerType) {
    methods.debug('inventory.updateOwnerAll', oldOwnerId, oldOwnerType, ownerId, ownerType);
    try {
...
inventory.updateItemParams = function(id, params) {
    methods.debug('inventory.updateItemParams', id, params);
    try {
...
inventory.updateItemCount = function(id, count) {
    methods.debug('inventory.updateItemCount', id, count);
    try {
...
inventory.updateItemCountByItemId = function(itemId, count, ownerId, ownerType = 1, isEquip = 1) {
    methods.debug('inventory.updateItemCountByItemId', itemId, count, ownerId, ownerType, isEquip);
    try {
...
inventory.updateAmount = function(player, ownerId, ownerType) { // Фикс хуйни, котороая просто поломала все, заметочка никогда не писать код когда ты очень сильно хочешь спать.
    ...
    methods.debug('inventory.updateAmount', ownerId, ownerType);
    if (!user.isLogin(player))
        return;
...
inventory.dropItem = function(player, id, itemId, posX, posY, posZ, rotX, rotY, rotZ) {
    methods.debug('inventory.dropItem', id, itemId, posX, posY, posZ, rotX, rotY, rotZ);
    if (!user.isLogin(player))
        return;
    try {
...
inventory.dropItemJust = function(id, itemId, posX, posY, posZ, rotX, rotY, rotZ) {
    ...
    methods.debug('inventory.dropItemJust', id, itemId, posX, posY, posZ, rotX, rotY, rotZ);
    try {
...
inventory.deleteDropItem = function(id) {
    methods.debug('inventory.deleteDropItem', id);
    try {
        let entity = props.get(id.toString());
        if (mp.objects.exists(entity)) {
            //1410065407
            entity.dimension = 1410065407;
            setTimeout(function () {
                try {
                    methods.debug('inventory.deleteDropItem.setTimeout');
                    entity.destroy();
                }
                catch (e) {}
            }, 1000);
...
inventory.deleteItem = function(id) {
    methods.debug('inventory.deleteItem', id);
    try {
...
inventory.deleteItemByItemId = function(id, isEquip = 0, limit = -1) {
    methods.debug('inventory.deleteItemByItemId', id, isEquip, limit);
    try {
        if (limit > 0) {
...
inventory.deleteUserItemByItemId = function(ownerType, ownerId, id, isEquip = 0, limit = -1) {
    methods.debug('inventory.deleteUserItemByItemId', ownerType, ownerId, id, isEquip, limit);
    try {
        if (limit > 0) {
...
inventory.deleteItemByUsers = function(id) {
    methods.debug('inventory.deleteItemByUsers', id);
    try {
...
inventory.deleteItemsRange = function(player, itemIdFrom, itemIdTo) {
    methods.debug('inventory.deleteItemsRange', itemIdFrom, itemIdTo);
    try {
        if (!user.isLogin(player))
            return;
...
inventory.deleteItemsByItemId = function(itemId) {
    methods.debug('inventory.deleteItemsByItemId', itemId);
    try {
...
inventory.addItem = function(itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout = 1) {
    methods.debug('inventory.addItem', itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout);
...
inventory.addPlayerWeaponItem = function(player, itemId, count, ownerType, ownerId, countItems, isEquip, params, text = 'Выдано оружие', timeout = 1) {
    methods.debug('inventory.addPlayerWeaponItem', itemId, count, ownerType, ownerId, countItems, isEquip, params, text, timeout);
...
inventory.addWeaponItem = function(itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout = 1) {
    methods.debug('inventory.addWeaponItem', itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout);
    let serial = weapons.getWeaponSerial(itemId);
    let paramsObject = JSON.parse(params);
    paramsObject.serial = serial;
...
inventory.addArmourItem = function(itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout = 1) {
    methods.debug('inventory.addArmourItem', itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout);
    let paramsObject = JSON.parse(params);
...
inventory.addAmmoItem = function(itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout = 1) {
    methods.debug('inventory.addAmmoItem', itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout);
...
inventory.addItemSql = function(itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout = 1) {
    ...
    methods.debug('inventory.addItemSql', itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout);
    setTimeout(function () {
        try {
...
inventory.addWorldItem = function(itemId, count, countItems, posx, posy, posz, rotx, roty, rotz, params, timeout = 1) {
    methods.debug('inventory.addWorldItem', itemId, count, countItems, posx, posy, posz, rotx, roty, rotz, params, timeout);
    setTimeout(function () {
        try {
            for (let i = 0; i < count; i++) {
...
inventory.dropWeaponItem = function(player, itemId, posx, posy, posz, rotx, roty, rotz) {
    ...
    methods.debug('inventory.dropWeaponItem', itemId, posx, posy, posz, rotx, roty, rotz);
    setTimeout(function () {
...
inventory.usePlayerItem = function(player, id, itemId, ownerType, ownerId) {
    methods.debug('inventory.usePlayerItem', id, itemId);
    if (!user.isLogin(player))
        return;
    ...
    switch (itemId) {
        case 277: {
            try {
                ...
                setTimeout(() => {
                    chat.sendMeCommand(player, "осмотрел(а) человека на земле");
                }, 1000);
                setTimeout(() => {
                    chat.sendMeCommand(player, "растегнул(а) верхнюю одежду человеку на земле");
                }, 5000);
                setTimeout(() => {
                    chat.sendMeCommand(player, "достал(а) адреналин из сумки медика");
                }, 7000);
                setTimeout(() => {
                    chat.sendMeCommand(player, "вколол(а) адреналин в сердце человеку на земле");
                }, 7000);
                setTimeout(() => {
                    chat.sendMeCommand(player, "достал(а) дефибрилятор из сумки медика");
                }, 9000);
                setTimeout(() => {
                    let targetId = user.getId(target);
                    chat.sendMeCommand(player, "использовал(а) дефибриллятор");
        ...
        default:
        {
            ...
            inventory.useItem(target, id, itemId, true, ownerType, ownerId);
        }
    }
};

inventory.useItem = function(player, id, itemId, isTargetable = false, ownerType, ownerId) {
    methods.debug('inventory.useItem', id, itemId, isTargetable);
    if (!user.isLogin(player))
        return;
    try {
        ...
        mysql.executeQuery(sql, function (err, rows, fields) {
            ...
            if (rows.length === 0) {
                player.notify('~r~Предмет вам не принадлежит');
                return;
            }
            ...
            try {
                switch (itemId)
                {
                    ...
                    case 253:
                    {
                        // chat.sendDiceCommand(player); TODO: нет на сервере такой функции
                        break;
                    }
                    case 477:
                    {
                    ...
                    case 4:
                    {
                        ...
                        if(user.get(player,'isGreenZone')){
                            player.notify("~r~Нельзя взламывать транспорт в GreenZone");
                            return;
                        }
                        ...
                        setTimeout(function () {
                            let trust = user.get(player,'stats_lockpick');

                            methods.debug('inventory.useItem.setTimeout case 4');
                            try {
                                ...
                                if (methods.getRandomInt(methods.parseInt(trust), 102) >= 90)
                                {
                                    if(methods.parseInt(trust) > 100){
                                        user.set(player, 'stats_lockpick', 100)
                                        user.removeRep(player, 5);
                                        veh.locked = false;
                                        player.notify("~g~Вы открыли транспорт");
                                    } else if(methods.parseInt(trust) < 100){
                                        user.set(player, 'stats_lockpick', trust + 1)
                                        user.removeRep(player, 5);
                                        veh.locked = false;
                                        player.notify("~g~Вы открыли транспорт");
                                    }
                    ...
                    case 5: // todo
                    {
                        ...
                        setTimeout(function () {
                            if (!user.isLogin(player))
                                return;
                            methods.debug('inventory.useItem.setTimeout1 case 5');
                        ...
                        setTimeout(function () {
                            ...
                            methods.debug('inventory.useItem.setTimeout2 case 5');
                    ...
                    case 262: // TODO
                    {
                        ...
                        setTimeout(function () {
                            ...
                            setTimeout(function () {
                                try {
                                    methods.debug('inventory.useItem.setTimeout case 262');
                                    dispatcher.sendPos("Код 0", "В банке сработала сигнализация", player.position);
                ...
                methods.error('inventory.useItem', e.toString());
            }
        });
    } catch(e) {
        methods.debug(e);
