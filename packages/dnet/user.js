...
user.createAccount = function (player, login, pass, email) {
    ...
    if (!/^[a-zA-Z0-9]+$/.test(login)) {
        user.showCustomNotify(player, 'Логин должен быть только из английских букв и цифр', 1);
        return;
    }
    ...
    user.doesExistAccount(login, email, player.socialClub, function (cb) {
        ...
        let sql = "INSERT INTO accounts (login, email, social, serial, rgscid, password, reg_ip, reg_timestamp) VALUES ('" + login +
            "', '" + email + "', '" + social + "', '" + player.serial + "', '" + player.rgscId + "', '" + methods.sha256(pass) + "', '" + player.ip + "', '" + methods.getTimeStamp() + "')";
...
user.createUser = function (player, name, surname, age, promocode, referer, national) {
    if (!user.exists(player))
        return;
    methods.debug('user.createUser', name, surname, age, promocode, referer, national);

    let regName = /^[a-zA-Z]+ [a-zA-Z]+$/;
    if (!regName.test(name + ' ' + surname)) {
        user.showCustomNotify(player, 'Имя и Фамилия должна состоять только из английских букв', 1);
        return;
    }
    if (!/^[a-zA-Z0-9]+$/.test(promocode)) {
        user.showCustomNotify(player, 'Промокод должен быть только из английский букв и цифр', 1);
        return;
    }
    if (!/^[а-яА-Яa-zA-Z0-9]+$/.test(national))
        return;
...
user.loadAccount = function (player, callback) {
...
user.loginUser = function (player, name, spawn = 'Стандарт') {
    ...
    methods.debug('user.loginUser', name, spawn);
    if (!user.exists(player))
        return false;
...
user.save = function (player, withReset = false) {
    methods.debug('user.saveAccount', withReset);
    try {
        return new Promise(resolve => {
            methods.debug('user.saveAccount 2', withReset);
...
user.saveName = function (player, newName) {
    methods.debug('user.saveName', newName);
    try {
        return new Promise(resolve => {
            methods.debug('user.saveName 2', newName);
...
user.loadUser = function (player, name, spawn = 'Стандарт') {
    try {
        methods.debug('user.loadUser', name, spawn);
        if (!user.exists(player))
            return false;
        let selectSql = 'id';
        enums.userData.forEach(function (element) {
        ...
        mysql.executeQuery(`SELECT ${selectSql} FROM users WHERE name = ? LIMIT 1`, name, function (err, rows, fields) {
            ...
            enums.userData.forEach(function (element) {
                try {
                    ...
                    methods.error(`catch in user.loadUser_3`, e.toString())
                }
            });
            ...
            setTimeout(function () {
                ...
                if (!user.exists(player))
                    return false;
                methods.debug('user.loadUser.setTimeout');
                ...
                if (user.get(player, 'date_ban') > methods.getTimeStamp()) {
                    ...
                    user.showCustomNotify(player, 'Вы забанены, подробности смотреть на сайте: appi-rp.ru/banlist', 1, 5, 60000);
                ...
                if (user.get(player, 'social') !== player.socialClub) {
                    user.resetAll(player);
                    user.kick(player, 'Вы были кикнуты');
                    return;
                }
                ...
                user.set(player, 'mommy_is_dead', methods.UUID());
                ...
                setTimeout(function () {
                    ...
                    methods.debug('user.loadUser.setTimeout1');
                    try {
                        ...
                        setTimeout(function () {
                            user.set(player, 'canSave', true);
                        }, 5000);
                        user.updateClientCache(player);

                        let data = {
                            idLabel: user.get(player, 'id'),
                            name: user.get(player, 'name'),
                            /*walkie: user.get(player, 'walkie_' + (user.get(player, 'walkie_current') + 1)),
                            walkieBuy: user.get(player, 'walkie_buy'),*/
                        };

                        player.setVariable('info', JSON.stringify(data));
                        ...
                        player.dimension = 0;
                        player.datingList = new Map();
                        user.set(player, 'a_t', methods.sha256(methods.replaceAll(user.get(player, 'name'), ' ', '') + '|SLAFY'));
                        user.set(player, 'a_l', player.a_l);
                        user.set(player, 'a_p', player.a_p);
                        ...
                        setTimeout(function () {
                            try {
                                methods.debug('user.loadUser.setTimeout2');
                                //user.setArmour(player, user.get(player, 'ap'));
                                user.setHealth(player, user.get(player, 'hp'));
                                ...
                                methods.error(`catch in user.loadUser_4`, e.toString())
                            }
                        }, 2000);
                        ...
                        mysql.executeQuery(`SELECT * FROM user_dating WHERE user_owner = '${userId}'`, function (err, rowsD, fields) {
                            ...
                            try {
                                ...
                                list.forEach(item => {
                                    try {
                                        player.datingList.set(item.uId, item.uName)
                                    }
                                    catch (e) {
                                        methods.debug(e);
                                    }
                                ...
                                methods.debug(`catch in user.loadUser_5`, e.toString());
                            }
                        });
                        ...
                        setTimeout(function () {
                            try {
                                methods.debug('user.loadUser.setTimeout3');
                                mp.players.forEach(async p => {
                                    try {
                                        if (user.isLogin(p)) {
                                            if (user.get(p, 'id') === user.get(player, 'id') && p.id !== player.id) {
                                ...
                                methods.error(`catch in user.loadUser_6`, e.toString())
                            }
                        ...
                        methods.error(`catch in user.loadUser_2`, e.toString())
                    }
        ...
        methods.error(`catch in user.loadUser_1`, e.toString())
    }
};
...
user.spawnByName = function (player, spawn = 'Стандарт') {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return false;
        methods.debug('user.spawnByName.setTimeout');
        ...
        setTimeout(function () {
            try {
                methods.debug('user.spawnByName.setTimeout2');
                if (user.isLogin(player)) {
                    player.dimension = 0;
...
user.updateCharacterCloth = function (player) {
    ...
    try {
        ...
        setTimeout(function () {
            ...
            methods.debug('user.updateCharacterCloth.setTimeout');
            //TODO GET CURRENT
            ...
            try {
                if (user.hasById(user.getId(player), 'uniform'))
                    user.giveUniform(player, user.getById(user.getId(player), 'uniform'));
            }
            catch (e) { }
        }, 1000); //TODO
...
user.doesExistUser = function (name, callback) {
    methods.debug('user.doesExistUser', name);
...
user.doesLimitUser = function (serial, callback) {
    methods.debug('user.doesLimitUser', serial);
    mysql.FexecuteQuery(`SELECT id FROM users WHERE social = ?`, serial, function (err, rows, fields) {
        if (err) {
            methods.debug('[DATABASE | ERROR]');
            methods.debug(err);
            return callback(true);
...
user.validateAccount = function (player, login, pass, callback) {
    methods.debug('user.validateAccount', login, pass);
    mysql.executeQuery(`SELECT password, social, login FROM accounts WHERE login = ? LIMIT 1`, login, function (err, rows, fields) {
        ...
        rows.forEach(function (item) {
            ...
            if (user.exists(player)) {
                ...
                player.a_l = item['login'];
                player.a_p = item['password'];
            }
            return callback(true);
        });
    });
...
user.validateAccountBySocial = function (player, login, callback) {
    ...
    mysql.executeQuery(`SELECT password, social, login FROM accounts WHERE login = ? LIMIT 1`, login, function (err, rows, fields) {
        ...
        if (rows.length === 0)
            return callback(false);
        rows.forEach(function (item) {
            if (item['social'] !== player.socialClub)
                return callback(false);
            if (user.exists(player)) {
                player.accSocial = item['social'];
                player.a_l = item['login'];
...
user.doesExistAccount = function (login, email, social, callback) {
    methods.debug('user.doesExistAccount', login, email, social);
...
user.setOnlineStatus = function (player, isOnline) {
    methods.debug('user.setOnlineStatus', isOnline);
    if (user.isLogin(player))
...
user.getGlovesOffset = function (player, handId) {
    methods.debug('user.getGlovesOffset', handId);
    if (user.isLogin(player)) {
...
user.setProp = function (player, slot, type, color) {
    methods.debug('user.setProp', slot, type, color);
    if (!user.exists(player))
        return false;
    try {
...
user.setDecoration = function (player, slot, overlay) {
    methods.debug('user.setDecoration', slot, overlay);
    if (!user.exists(player))
        return false;
    try {
...
user.addExplode = function (player, x, y, z, explosionType, damageScale, isAudible, isInvisible, cameraShake, timeout = 1000) {
    methods.debug('user.showLoadDisplay', x, y, z, explosionType, damageScale, isAudible, isInvisible, cameraShake, timeout);
    setTimeout(function () {
        try {
            if (!user.exists(player))
                return false;
...
user.callCef = function (player, name, params) {
    methods.debug('user.callCef', name, params);
    if (!user.exists(player))
        return false;
    try {
...
user.cuff = function (player) {
    if (player.getVariable('enableAdmin')) return;
    ...
    try {
        ...
        setTimeout(function () {
            try {
                if (!user.exists(player))
                    return false;
                methods.debug('user.cuff.setTimeout');
...
user.getRepColorName = function (player) {
    methods.debug('user.getRepColorName');
    if (!user.isLogin(player))
        return 'Нейтральная';
    let rep = user.get(player, 'rep');
    if (rep > 900)
...
user.getFractionName2 = function (player) {
    methods.debug('user.getFractionName2');
    try {
        if (!user.isLogin(player))
            return 'Отсуствует';
...
user.getFamilyName = function (player) {
    methods.debug('user.getFamilyName');
    try {
        if (!user.isLogin(player))
            return 'Отсуствует';
...
user.getDepartmentName = function (player) {
    methods.debug('user.getDepartmentName');
    try {
        if (!user.isLogin(player))
            return 'Отсуствует';
...
user.getFractionHash = function (player) {
    methods.debug('user.getFractionHash');
    try {
        if (!user.isLogin(player))
            return 'none';
...
user.ready = function (player) {
    ...
    try {
        ...
        let uuid = methods.hashList[methods.getRandomInt(0, methods.hashList.length - 1)];
        player.hashList.push(uuid);
        player.call('playerReadyDone', [uuid]);
    }
    catch (e) {
...
user.updateVehicleInfo = function (player) {
    methods.debug('user.updateVehicleInfo');
    if (!user.exists(player))
        return false;

    try {
        if(enums.vehicleInfo.length > 0){
    ...
    try {
        if(enums.vehicleInfoEngine.length){
            for (let i = 0; i < parseInt(enums.vehicleInfoEngine.length / 250) + 1; i++) {
                let from = i * 250 - 1;
                let to = i * 250 + 249;
                player.call('client:updateVehicleInfoEngine', [i, enums.vehicleInfoEngine.slice(from < 0 ? 0 : from, to)]);
            }
        }
...
function serializeComponentSetDate(dataSet) {
    return (Array.from(dataSet).map((hash) => (hash.toString()))).join("|");
}
user.setWeaponComponened = function (player, wpNamed, value) {

    methods.debug('user.setWeaponComponened', wpNamed, value);
    try {
        if (user.exists(player)) {
            if (typeof wpNamed === "string"){
                wpNamed = mp.joaat(wpNamed)
            }
            if (!player._weaponComponented.hasOwnProperty(wpNamed)) player._weaponComponented[wpNamed] = new Set();
            //player.call('client:user:setWeaponComponened', [wpNamed, value]);
            player._weaponComponented[wpNamed].add(value);
            player.setVariable("weaponComponented" + wpNamed.toString(), wpNamed.toString() + "." + serializeComponentSetDate(player._weaponComponented[wpNamed]));
        }
    }
    catch (e) { methods.debug(e) }
};
user.deleteWeaponComponened = function (player, wpNamed, value) {
    methods.debug('user.deleteWeaponComponened', wpNamed, value);
    try {
        if (user.exists(player) && player._weaponComponented.hasOwnProperty(wpNamed)) {
            //player.call('client:user:deleteWeaponComponened', [wpNamed, value]);
            player._weaponComponented[wpNamed].delete(value);
            player.setVariable("weaponComponented" + wpNamed.toString(), wpNamed.toString() + "." + serializeComponentSetDate(player._weaponComponented[wpNamed]))
        }
    }
    catch (e) { methods.debug(e) }
...
user.setDating = function (player, key, value) {
    methods.debug('user.setDating', key, value);
    try {
        if (user.exists(player)) {
...
user.setById = function (id, key, val) {
    //methods.debug('user.setById');
    Container.Data.Set(id - 88888888, key, val);
};

user.hasById = function (id, key) {
    //methods.debug('user.hasById');
    return Container.Data.Has(id - 88888888, key);
};

user.resetById = function (id, key) {
    //methods.debug('user.resetById');
    return Container.Data.Reset(id - 88888888, key);
};

user.getById = function (id, key) {
    //methods.debug('user.getById');
    try {
        return Container.Data.Get(id - 88888888, key);
    } catch (e) {
        methods.debug(e);
    }
    return null;
...
user.getId = function (player) {
    ...
    if (!user.exists(player))
        return -1;
    if (user.has(player, 'id'))
        return methods.parseInt(user.get(player, 'id'));
    return -1;
};

user.getSvId = function (player) {
    //methods.debug('user.getsvid');
    if (!user.exists(player))
        return -1;
    return player.id;
};
...
user.getLastName = function (player) {
    //methods.debug('user.getLastName');
    if (!user.exists(player))
        return 'NO_NAME';
    try {
        if (user.has(player, 'name'))
            return user.get(player, 'name').split(' ')[1];
    }
    catch (e) { }
    return player.socialClub;
};
...
user.getPlayerById = function (id) {
    //methods.debug('user.getPlayerById');
    try {
        let player = null;
...
user.getVehicleDriver = function (vehicle) {
    methods.debug('user.getVehicleDriver');
    let driver = null;
    if (!vehicles.exists(vehicle))
        return driver;
    try {
        vehicles.getOccupants(vehicle).forEach((p) => {
            try {
                if (user.exists(p) && p.seat == 0)
                    driver = p;
            }
            catch (e) {
                methods.error('user.getVehicleDriver.forEach', e);
            }
        });
    }
    catch (e) {
        methods.error('user.getVehicleDriver', e);
    }
    return driver;
};

user.getVehiclesInLspd = function (player, type = 0) {
    methods.debug('user.getVehiclesInLspd', type);
    if (!user.isLogin(player))
        return;
...
user.getCasinoMoney = function (player) {
    methods.debug('user.getCasinoMoney');
    if (user.has(player, 'money_casino'))
        return methods.parseInt(user.get(player, 'money_casino')) || 0;
    return 0;
};

user.addCasinoMoney = function (player, money, text = 'Финансовая операция') {
    methods.debug('user.addCasinoMoney');
    user.addCasinoHistory(player, text, methods.parseInt(money), user.getCasinoMoney(player));
    user.setCasinoMoney(player, user.getCasinoMoney(player) + methods.parseInt(money));
};

user.removeCasinoMoney = function (player, money, text = 'Финансовая операция') {
    methods.debug('user.removeCasinoMoney');
    user.addCasinoHistory(player, text, methods.parseInt(money) * -1, user.getCasinoMoney(player));
    user.setCasinoMoney(player, user.getCasinoMoney(player) - methods.parseInt(money));
};

user.setCasinoMoney = function (player, money) {
    methods.debug('user.setCasinoMoney');
    user.set(player, 'money_casino', methods.parseInt(money));
    console.log(`new money = ${user.getCasinoMoney(player)}`);
    user.updateClientCache(player);
};

user.addCasinoHistory = function (player, text, price, was = 0) {
    methods.debug('user.addCasinoHistory');
    if (!user.isLogin(player))
        return;
    ...
    mysql.executeQuery(`INSERT INTO log_casino (user_id, text, price, was, timestamp, rp_datetime) VALUES ('${userId}', '${text}', '${price}', '${was}', '${timestamp}', '${rpDateTime}')`);
};

user.addMoney = function (player, money, text = 'Финансовая операция', payType = 0) {
    methods.debug('user.addMoney');
    if (payType === 1)
...
user.removeMoney = function (player, money, text = 'Финансовая операция', payType = 0) {
    methods.debug('user.removeMoney');
    if (payType === 1)
...
user.addCashMoney = function (player, money, text = 'Финансовая операция') {
    methods.debug('user.addCashMoney');
...
user.removeCashMoney = function (player, money, text = 'Финансовая операция') {
    methods.debug('user.removeCashMoney');
...
user.addCryptoMoney = function (player, money, text = 'Финансовая операция') {
    methods.debug('user.addCryptoMoney');
...
user.removeCryptoMoney = function (player, money, text = 'Финансовая операция') {
    methods.debug('user.removeCryptoMoney');
...
user.addBankMoney = function (player, money, text = "Операция со счетом") {
    methods.debug('user.addBankMoney');
...
user.removeBankMoney = function (player, money, text = "Операция со счетом") {
    methods.debug('user.removeBankMoney');
...
user.addPayDayMoney = function (player, money) {
    methods.debug('user.addPayDayMoney');
...
user.addBonusMoney = function (player, money) {
    methods.debug('user.addBonusMoney');
    if (user.isLogin(player)) {
...
user.achiveDoneAllById = function (player, id) {
    methods.debug('user.achiveDoneAllById', id);
    try {
        if (user.isLogin(player))
...
user.achiveDoneDailyById = function (player, id) {
    methods.debug('user.achiveDoneDailyById', id);
    try {
        if (user.isLogin(player))
...
user.setFractionId = function (player, id) {
    methods.debug('user.setFractionId', id);
    try {
        if (!user.isLogin(player))
            return;
...
user.setFractionId2 = function (player, id) {
    methods.debug('user.setFractionId2', id);
    try {
        if (!user.isLogin(player))
            return;
        id = methods.parseInt(id);
...
user.setFamilyId = function (player, id) {
    methods.debug('user.setFamilyId', id);
    try {
        if (!user.isLogin(player))
            return;
        id = methods.parseInt(id);
...
user.setDatingName = function (player, value) {
    methods.debug('user.setDatingName', value);
    try {
        if (!user.isLogin(player))
            return;
...
user.getBankCardPrefix = function (player, bankCard = 0) {
    methods.debug('bank.getBankCardPrefix', bankCard);
    if (!user.isLogin(player))
        return;
    if (bankCard == 0)
...
user.addBankHistory = function (player, text, price, was = 0) {
    methods.debug('user.addBankHistory');
    if (!user.isLogin(player))
        return;
...
user.addCashHistory = function (player, text, price, was = 0) {
    methods.debug('user.addCashHistory');
    if (!user.isLogin(player))
        return;
...
user.addCryptoHistory = function (player, text, price, was = 0) {
    methods.debug('user.addCryptoHistory');
    if (!user.isLogin(player))
        return;
...
user.addHistoryById = function (id, type, reason) {
    methods.debug('user.addHistoryById');
    let rpDateTime = weather.getRpDateTime();
    let timestamp = methods.getTimeStamp();
...
user.sendSms = function (player, sender, title, text, pic) {
    ...
    return; //TODO
    // TODO:  mysql.executeQuery(`INSERT INTO log_player (user_id, datetime, type, do) VALUES ('${user.getId(player)}', '${rpDateTime} (( ${dateTime} ))', '${type}', '${reason}')`);
};
...
user.setMaxSpeed = function (player, speed) {
    methods.debug('user.setMaxSpeed');
    if (!user.exists(player))
        return false;
    try {
...
user.teleportVeh = function (player, x, y, z, rot = 0.1) {
    ...
    if (!vehicles.exists(player.vehicle)) {
        user.teleport(player, x, y, z, rot);
        return false;
    }
    try {
...
user.putInVehicle = function (player, veh, seat) {
    ...
    if (!vehicles.exists(veh))
        return false;
    try {
        player.putIntoVehicle(veh, seat + 1);
        player.call('client:putInVehicle');
        vehicles.addOccupant(veh, player);
    }
    catch (e) { }
};

user.removeFromVehicle = function (player) {
    methods.debug('user.removeFromVehicle');
    if (!user.exists(player))
        return false;
    try {
...
user.setWaypoint = function (player, x, y) {
    methods.debug('user.setWaypoint', x, y);
    try {
        if (!user.exists(player))
            return false;
        player.call('client:user:setWaypoint', [x, y]);
...
user.setClipset = function (player, style) {
    try {
        methods.debug('user.setClipset', style);
        if (!user.exists(player))
            return false;
...
user.setClipsetW = function (player, style) {
    try {
        methods.debug('user.setClipsetW', style);
        if (!user.exists(player))
            return false;
...
user.getDating = function (player, item) {
    methods.debug('user.getDating');
    try {
        if (player.datingList.has(item))
            return player.datingList.get(item);
        return item;
    }
    catch (e) {
        methods.debug(e);
...
user.sendPhoneNotify = function (player, sender, title, message, pic = 'CHAR_BLANK_ENTRY') {
    methods.debug('user.sendPhoneNotify');
    try {
        if (!user.isLogin(player))
            return;
...
user.sendPhoneSms = function (player, sender, title, message, pic = 'CHAR_BLANK_ENTRY') {
    methods.debug('user.sendPhoneSms');
    try {
        if (!user.isLogin(player))
            return;
...
user.sendSmsBankOperation = function (player, text, title = 'Операция со счётом', pref = 0) {
    methods.debug('bank.sendSmsBankOperation', text, title, pref);
    if (!user.isLogin(player))
        return;
...
user.kick = function (player, reason, title = 'Вы были кикнуты.') {
    if (!user.exists(player))
        return;
    try {
        methods.debug('user.kick', reason, title);
...
user.kickAntiCheat = function (player, reason, title = 'Вы были кикнуты.', isAdmin = false) {
    methods.debug('user.kickAntiCheat', reason, title, isAdmin);
    try {
        if (user.isLogin(player)) {
            if (isAdmin) {
                chat.sendToAdmin(`Anti-Cheat Protection кикнул ${user.getRpName(player)} (${player.id})`, reason);
            }
            else
...
user.blockKeys = function (player, enable) {
    methods.debug('user.blockKeys', enable);
    try {
        if (!user.exists(player))
            return false;
...
user.freeze = function (player, enable) {
    methods.debug('user.freeze', enable);
    try {
        if (!user.exists(player))
            return false;
...
user.heading = function (player, rot) {
    methods.debug('user.headingToCoord', rot);
    if (!user.exists(player))
        return false;
    let pos = player.position;
    mp.players.forEach((p) => {
        try {
            if (user.exists(player) && user.exists(p) && methods.distanceToPos(pos, p.position) < 100)
...
user.headingToCoord = function (player, x, y, z) {
    methods.debug('user.headingToCoord', x, y, z);
    if (!user.exists(player))
        return false;
    let pos = player.position;
    mp.players.forEach((p) => {
        try {
            if (user.exists(player) && user.exists(p) && methods.distanceToPos(pos, p.position) < 300)
...
user.headingToTarget = function (player, targetId) {
    methods.debug('user.headingToCoord', targetId);
    if (!user.exists(player))
        return false;
    let pos = player.position;
    mp.players.forEach((p) => {
...
user.playScenario = function (player, name) {
    methods.debug('user.playScenario', name);
    if (!user.exists(player))
        return false;
    let pos = player.position;
    mp.players.forEach((p) => {
...
user.playAnimationWithUser = function (player, target, animId) {
    methods.debug('user.stopScenario', animId);
    if (!user.exists(player))
        return false;
    ...
    try {
        ...
        setTimeout(function () {
            try {
                methods.debug('user.playAnimationWithUser.setTimeout');
...
user.playAnimation = function (player, dict, anim, flag = 49) {
    methods.debug('user.playAnimation', dict, anim, flag);
    if (!user.exists(player))
        return false;
    let pos = player.position;
    mp.players.forEach((p) => {
...
user.stopAnimationNow = function (player) {
    methods.debug('user.stopAnimationNow');
    if (!user.exists(player))
        return false;
    let pos = player.position;
    mp.players.forEach((p) => {
...
user.setRagdoll = function (player, timeout) {
    methods.debug('user.setRagdoll', timeout);
    if (!user.exists(player))
        return false;
    let pos = player.position;
    mp.players.forEach((p) => {
...
user.playEatAnimation = function (player) {
    methods.debug('user.playEatAnimation');
...
user.addDrugLevel = function (player, drugType, level) {
    methods.debug('user.addDrugLevel', drugType, level);
    try {
        if (user.isLogin(player))
...
user.removeDrugLevel = function (player, drugType, level) {
    methods.debug('user.removeDrugLevel', drugType, level);
    try {
        if (user.isLogin(player))
...
user.setDrugLevel = function (player, drugType, level) {
    methods.debug('user.setDrugLevel', drugType, level);
    try {
        if (user.isLogin(player))
...
user.stopAllScreenEffects = function (player) {
    methods.debug('user.stopAllScreenEffects');
    try {
        if (user.isLogin(player))
...
user.addWaterLevel = function (player, level) {
    if (!user.isLogin(player))
        return;
    methods.debug('user.addWaterLevel', level);
    try {
...
user.removeWaterLevel = function (player, level) {
    methods.debug('user.removeWaterLevel', level);
    try {
        if (!user.isLogin(player))
            return;
...
user.setWaterLevel = function (player, level) {
    methods.debug('user.setWaterLevel', level);
    try {
        if (!user.isLogin(player))
            return;
...
user.getWaterLevel = function (player) {
    methods.debug('user.getWaterLevel');
...
user.addEatLevel = function (player, level) {
    methods.debug('user.addEatLevel', level);
    try {
        if (!user.isLogin(player))
            return;
...
user.removeEatLevel = function (player, level) {
    methods.debug('user.removeEatLevel', level);
    try {
        if (!user.isLogin(player))
            return;
...
user.setEatLevel = function (player, level) {
    methods.debug('user.setEatLevel', level);
    try {
        if (!user.isLogin(player))
            return;
...
user.getEatLevel = function (player) {
    methods.debug('user.getEatLevel');
    return user.get(player, "eat_level");
};

user.giveWeapon = function (player, weapon, pt) {
    methods.debug('user.giveWeapon');
    if (!user.exists(player))
        return false;
    player.call('client:user:giveWeapon', [weapon, pt])
};

user.giveWeaponComponent = function (player, weapon, component, wpName) {
    methods.debug('user.giveWeaponComponent');
    try {
        if (!user.exists(player))
            return false;
        methods.debug('user.giveWeaponComponent', weapon, component, wpName);
        player.giveWeaponComponent(weapon, component, wpName);
    }
    catch (e) { }
};

user.removeWeaponComponent = function (player, weapon, component, wpNamed) {
    methods.debug('user.removeWeaponComponent');
    try {
        if (!user.exists(player))
            return false;
        methods.debug('user.removeWeaponComponent', weapon, component, wpNamed);
...
user.removeAllWeaponComponents = function (player, weapon) {
    methods.debug('user.removeAllWeaponComponents');
    try {
        if (!user.exists(player))
            return false;
...
user.setWeaponTint = function (player, weapon, tint, wpNamed) {
    methods.debug('user.setWeaponTint');
    try {
        if (!user.exists(player))
            return false;
        user.setWeaponComponened(player, mp.joaat(wpNamed), tint + 1)
...
user.giveJobSkill = function (player, jobNew = 0) {
    methods.debug('user.giveJobSkill', jobNew);
    if (user.isLogin(player)) {
        try {
...
user.giveJobMoney = function (player, money, jobId = 0) {
    methods.debug('user.giveJobMoney', money, jobId);
    if (!user.isLogin(player))
        return;
...
user.jail = function (player, sec, withIzol = 0) {
    methods.debug('user.jail', sec, withIzol);
    try {
        if (!user.isLogin(player))
            return false;
...
user.arrest = function (player) {
    ...
    try {
        ...
        user.jail(player, methods.parseInt(user.get(player, 'wanted_level')) * 60);
...
user.giveWanted = function (player, level, reason, officer = 'Система') {
    methods.debug('user.giveWanted', level, reason, officer);
    if (!user.isLogin(player))
        return false;
...
user.giveLic = function (player, lic, monthEnd = 12, desc = '') {
    methods.debug('user.giveLic', lic, monthEnd, desc);
    if (!user.isLogin(player))
        return;
...
user.buyLicense = function (player, type, price, month, typePay = 0) {
    if (!user.isLogin(player))
        return;
    methods.debug('licenseCenter.buy', type, price, month, typePay);
...
user.toLspdSafe = function (player, time = 28, target = null) {
    methods.debug('user.toLspdSafe', time);
    try {
        if (!user.exists(player))
            return false;
        ...
        user.set(player, "hand", 0);
        user.set(player, "hand_color", 0);
...
user.createBlip = function (player, id, x, y, z, blipId = 1, blipColor = 0, route = false, shortRange = false, name = 'Цель', rot = 0, scale = 0.8) {
    methods.debug('user.createBlip', id, x, y, z, blipId, blipColor, route, shortRange, name, rot, scale);
    try {
        if (!user.exists(player))
            return false;
...
user.deleteBlip = function (player, id) {
    methods.debug('user.deleteBlip', id);
    try {
        if (!user.exists(player))
            return false;
...
user.createBlipByRadius = function (player, id, x, y, z, radius, blipId = 1, blipColor = 0, route = false) {
    methods.debug('user.createBlipByRadius', id, x, y, z, radius, blipId, blipColor, route);
    try {
        if (!user.exists(player))
            return false;
...
user.deleteBlipByRadius = function (player, id) {
    methods.debug('user.createBlipByRadius', id);
    try {
        if (!user.exists(player))
            return false;
...
user.flashBlipByRadius = function (player, id, flash = false) {
    methods.debug('user.flashBlipByRadius', id, flash);
    try {
        if (!user.exists(player))
            return false;
...
user.createBlip1 = function (player, x, y, z, blipId = 1, blipColor = 0, route = false) {
    methods.debug('user.createBlip1', x, y, z, blipId, blipColor, route);
    try {
        if (!user.exists(player))
            return false;
...
user.deleteBlip1 = function (player) {
    methods.debug('user.deleteBlip1');
    try {
        if (!user.exists(player))
            return false;
...
user.createBlip2 = function (player, x, y, z, blipId = 1, blipColor = 0, route = false) {
    methods.debug('user.createBlip2', x, y, z, blipId, blipColor, route);
    try {
        if (!user.exists(player))
            return false;
...
user.deleteBlip2 = function (player) {
    methods.debug('user.deleteBlip2');
    try {
        if (!user.exists(player))
            return false;
...
user.createBlip3 = function (player, x, y, z, blipId = 1, blipColor = 0, route = false) {
    methods.debug('user.createBlip3', x, y, z, blipId, blipColor, route);
    try {
        if (!user.exists(player))
            return false;
...
user.deleteBlip3 = function (player) {
    methods.debug('user.deleteBlip3');
    try {
        if (!user.exists(player))
            return false;
...
user.payDay = async function (player) {
    ...
    if (user.get(player, 'online_time') === 169) {
        if (user.get(player, 'referer') !== "") {
            ...
            user.addCashMoney(player, 5000, 'Бонус от государства');
            player.notify(`~g~Вы получили $5,000 по реферальной системе`);
            player.notify(`~g~Пригласивший ${user.get(player, 'referer')} получил 250bp на личный счёт`);
            mysql.executeQuery(`UPDATE users SET money_donate = money_donate + '250' WHERE name ='${user.get(player, 'referer')}'`);
            mysql.executeQuery(`INSERT INTO log_referrer (name, referrer, money, timestamp) VALUES ('${user.getRpName(player)}', '${user.get(player, 'referer')}', '250', '${methods.getTimeStamp()}')`);
        }
        ...
        if (user.get(player, 'promocode') !== "") {
            ...
            mysql.executeQuery(`SELECT * FROM promocode_top_list WHERE promocode = '${promocode}' LIMIT 1`, function (err, rows, fields) {
                if (rows.length >= 1) {
                    ...
                    player.notify(`~g~Вы получили ~s~$10000 ~g~по промокоду ~s~${user.get(player, 'promocode')}`);
                    user.addCashMoney(player, 10000, 'Бонус от государства');
                }
    ...
    if (user.get(player, 'bank_card') > 0) {
        ...
        if (player.getVariable('isAfk') === true) {
            ...
            if (!user.hasById(user.getId(player), 'uniform') && !user.isNews(player) && !user.isFib(player) || !(user.get(player, 'rank_type') == 2 && user.isSapd(player)) || !(user.get(player, 'rank_type') == 4 && user.isSheriff(player))) {
                ...
                if (currentCofferMoney < nalog) {
                    ...
                    let ben = coffer.getBenefit(frId);
                    if (ben > 0)
                    ...
                    coffer.removeMoney(frId, nalog + ben)
                }
            }
        }
...
user.isPolice = function (player) {
    return user.isLogin(player) && (user.isSapd(player) || user.isFib(player) || user.isUsmc(player) || user.isSheriff(player) || user.isGov(player));
};
...
user.getVehicleFreeSlot = function (player) {
    methods.debug('user.getVehicleFreeSlot');
    if (!user.isLogin(player))
        return 0;
    let freeSlot = 0;
    try {
        ...
        else if (user.get(player, 'car_id3') === 0 && user.get(player, 'house_id') > 0 && hData && hData.get('price') >= 100000) {
            freeSlot = 3;
        }
        else if (user.get(player, 'car_id4') === 0 && user.get(player, 'house_id') > 0 && hData && hData.get('price') >= 500000) {
            freeSlot = 4;
        }
        else if (user.get(player, 'car_id5') === 0 && user.get(player, 'house_id') > 0 && hData && hData.get('price') >= 1000000) {
            freeSlot = 5;
        }
        else if (user.get(player, 'car_id3') === 0 && user.get(player, 'condo_id') > 0 && cData && cData.get('price') >= 100000) {
            freeSlot = 3;
        }
        else if (user.get(player, 'car_id4') === 0 && user.get(player, 'condo_id') > 0 && cData && cData.get('price') >= 500000) {
            freeSlot = 4;
        }
        else if (user.get(player, 'car_id5') === 0 && user.get(player, 'condo_id') > 0 && cData && cData.get('price') >= 1000000) {
            freeSlot = 5;
        }
...
user.warn = function (player, count, reason, notify = true) {
    methods.debug('user.warn', count, reason, notify);
    if (!user.isLogin(player))
        return;
...
user.ban = function (player, count, reason) {
    methods.debug('user.ban', count, reason);
    if (!user.isLogin(player))
        return;
...
user.giveVip = function (player, days = 7, type = 2, withChat = false, desc = '') {
    methods.debug('user.giveVip', days, type, withChat, desc);
    if (!user.isLogin(player))
        return;
...
user.giveRandomMask = function (player, proc = 50, withChat = false, desc = '') {
    methods.debug('user.giveRandomMask', proc, withChat, desc);
    if (!user.isLogin(player))
        return;
...
user.giveMask = function (player, maskId, withChat = false, desc = '') {
    methods.debug('user.giveMask', maskId, withChat, desc);
    if (!user.isLogin(player))
        return;
...
user.giveVehicle = function (player, vName, withDelete = 1, withChat = false, desc = '', isBroke = false, isFree = false, slot = 10) {
    methods.debug('user.giveVehicle', vName, withDelete, withChat, desc, isBroke, isFree, slot);
    if (!user.isLogin(player))
        return;
    ...
    if (user.get(player, 'car_id' + slot) === 0 && vInfo.display_name !== 'Unknown') {
        ...
        setTimeout(function () {
            try {
                methods.debug('user.giveVehicle.setTimeout');
...
user.giveUniform = function (player, id = 0) {
    methods.debug('user.giveUniform', id);
    if (!user.isLogin(player))
        return;
    ...
    if (id === 0) { //default
        ...
        if (user.getSex(player) == 1) {
            ...
            user.setComponentVariation(player, 8, 58, 0);
            user.setComponentVariation(player, 10, 0, 0);
            user.setComponentVariation(player, 11, 425, 0);
        }
        ...
        if (user.getSex(player) == 1) {
            ...
            user.setComponentVariation(player, 4, 157, 0);
            user.setComponentVariation(player, 5, 0, 0);
            user.setComponentVariation(player, 6, 25, 0);
            user.setComponentVariation(player, 7, 0, 0);
            ...
            user.setComponentVariation(player, 9, 0, 0);
            user.setComponentVariation(player, 10, 0, 0);
            user.setComponentVariation(player, 11, 442, 0);
        ...
        if (user.getSex(player) == 1) {
            ...
            user.setComponentVariation(player, 9, 0, 0);
            user.setComponentVariation(player, 10, 0, 0);
            user.setComponentVariation(player, 11, 424, 3);
        ...
        if (user.getSex(player) == 1) {
            ...
            user.setComponentVariation(player, 4, 155, 5);
            user.setComponentVariation(player, 5, 0, 0);
            user.setComponentVariation(player, 6, 10, 0);
            user.setComponentVariation(player, 7, 115, 2);
            user.setComponentVariation(player, 8, 0, 18);
            user.setComponentVariation(player, 9, 0, 0);
            user.setComponentVariation(player, 10, 0, 0);
            user.setComponentVariation(player, 11, 438, 0);
        }
    }
        ...
        if (user.getSex(player) == 1) {
            ...
            user.setComponentVariation(player, 0, 6, 0);
            user.setComponentVariation(player, 1, 0, 0);
            user.setComponentVariation(player, 3, 11, 0);
            user.setComponentVariation(player, 4, 25, 6);
            user.setComponentVariation(player, 5, 0, 0);
            ...
            user.setComponentVariation(player, 9, 0, 0);
            user.setComponentVariation(player, 10, 0, 0);
            user.setComponentVariation(player, 11, 425, 3);
        }
    }
        ...
        if (user.getSex(player) == 1) {
            ...
            user.setComponentVariation(player, 0, 1, 0);
            user.setComponentVariation(player, 1, 0, 0);
            user.setComponentVariation(player, 3, 6, 0);
            user.setComponentVariation(player, 4, 25, 6);
            user.setComponentVariation(player, 5, 0, 0);
            user.setComponentVariation(player, 6, 54, 0);
            user.setComponentVariation(player, 7, 0, 0);
            ...
            user.setComponentVariation(player, 9, 0, 0);
            user.setComponentVariation(player, 10, 0, 0);
            user.setComponentVariation(player, 11, 423, 0);
        }
    }
        ...
        if (user.getSex(player) == 1) {
            ...
            user.setProp(player, 0, 58, 1);
            user.setComponentVariation(player, 3, 17, 0);
            user.setComponentVariation(player, 4, 157, 1);
            user.setComponentVariation(player, 5, 0, 0);
            user.setComponentVariation(player, 6, 25, 0);
            user.setComponentVariation(player, 7, 0, 0);
            user.setComponentVariation(player, 8, 122, 0);
            user.setComponentVariation(player, 9, 0, 0);
            user.setComponentVariation(player, 10, 0, 0);
            user.setComponentVariation(player, 11, 442, 1);
        }
    }
    else if (id === 21) { //BCSD 10 (Tactical)
        //user.clearAllProp(player);
        user.setComponentVariation(player, 7, 0, 0);
        if (user.getSex(player) == 1) {
...
///
user.stopSpectate = function (player, positionTarget) {
    try {
        if (!player.spectatePosition) return;
        player.call('admin:spectate:stop');
        let pos = positionTarget ? positionTarget : player.spectatePosition
        user.teleport(player, pos.x, pos.y, pos.z);
        player.heading = player.spectatePosition.h;
        player.dimension = player.spectateTarget ? player.spectateTarget.dimension : player.spectatePosition.d;
...
user.startSpectate = function (player, target) {
    ...
    player.call('admin:spectate', [target.id]);
...
user.exists = (player) => {
    //methods.debug('user.exists');
    try {
        if (player && mp.players.exists(player)) {
            player.getVariable('exists');
            return true;
        }
    }
    catch (e) {
        methods.error(`catch in user.exists`, e.toString())
        return false;
    }
    return false
};
