...
const offsetContainer = enums.offsets.vehicleCont;
const offsetFr = -5000;
...
vehicles.processVehicleManager = () => {
    //methods.debug('vehicles.processVehicleManager1')
    ...
    try {
        ...
        if (creationQueue.length) {
            for (let creation of creationQueue) {
                try {
                    ...
                    //methods.debug('processVehicleManager2', creation.args);
        ...
        if (cbs.length) {
            for (let cb of cbs) {
                try {
                    //methods.debug('vehicles.processVehicleManager4')
                    if (cb[1] && cb[0] && cb)
                        cb[1](cb[0]);
                }
                catch (e) {
        ...
        //methods.debug('vehicles.processVehicleManager5')
    } catch (e) {
        methods.debug(e)
    }
...
vehicles.loadAllUserVehicles = (userId) => {
    methods.debug('vehicles.loadAllUserVehicles', userId);
...
vehicles.loadUserById = (id) => {
    methods.debug('vehicles.loadUserById', id);
...
vehicles.loadAllOldVehShop = () => {
    methods.debug('vehicles.loadAllOldVehShop');
    mysql.executeQuery(`SELECT * FROM cars WHERE sell_price > '0' AND user_id = '-1'`, function (err, rows, fields) {
        rows.forEach((item, i) => {
            try {
                setTimeout(() => {
                    vehicles.loadUserVehicleByRow(item)
                ...
                methods.debug('vehicles.loadAllOldVehShop', e)
            }
        });
    });
};
...
vehicles.loadUserVehicleByRow = (row) => {
    methods.debug('vehicles.loadUserVehicleByRow', row);
    ...
    vehicles.set(row['id'], 's_whel_1', row['s_whel_1']);
    vehicles.set(row['id'], 's_whel_2', row['s_whel_2']);
    vehicles.set(row['id'], 's_whel_3', row['s_whel_3']);
    vehicles.set(row['id'], 's_whel_4', row['s_whel_4']);
    vehicles.set(row['id'], 's_whel_5', row['s_whel_5']);
    vehicles.set(row['id'], 's_whel_6', row['s_whel_6']);
    vehicles.set(row['id'], 's_elec', row['s_elec']);
    vehicles.set(row['id'], 's_break', row['s_break']);
    vehicles.set(row['id'], 's_h_break', row['s_h_break']);

    vehicles.set(row['id'], 's_body_door_0', row['s_body_door_0']);
    vehicles.set(row['id'], 's_body_door_1', row['s_body_door_1']);
    vehicles.set(row['id'], 's_body_door_2', row['s_body_door_2']);
    vehicles.set(row['id'], 's_body_door_3', row['s_body_door_3']);
    vehicles.set(row['id'], 's_body_door_4', row['s_body_door_4']);
    vehicles.set(row['id'], 's_body_door_5', row['s_body_door_5']);
    vehicles.set(row['id'], 's_body_door_6', row['s_body_door_6']);
    vehicles.set(row['id'], 's_body_door_7', row['s_body_door_7']);

    vehicles.set(row['id'], 's_body_window_0', row['s_body_window_0']);
    vehicles.set(row['id'], 's_body_window_1', row['s_body_window_1']);
    vehicles.set(row['id'], 's_body_window_2', row['s_body_window_2']);
    vehicles.set(row['id'], 's_body_window_3', row['s_body_window_3']);
    vehicles.set(row['id'], 's_body_window_4', row['s_body_window_4']);
    vehicles.set(row['id'], 's_body_window_5', row['s_body_window_5']);
    vehicles.set(row['id'], 's_body_window_6', row['s_body_window_6']);
    vehicles.set(row['id'], 's_body_window_7', row['s_body_window_7']);
    ...
    vehicles.set(row['id'], 'winter_tyre', row['winter_tyre']);
    vehicles.set(row['id'], 'ticket', row['ticket']);
...
vehicles.getFreePolicePos = () => {
    methods.debug('vehicles.getFreePolicePos');
...
vehicles.getFreeSellAutoPos = () => {
    methods.debug('vehicles.getFreeSellAutoPos');
...
vehicles.getFreeSellAutoNearestPos = function () {
    methods.debug('vehicles.getFreeSellAutoNearestPos');
    let prevPos = new mp.Vector3(9999, 9999, 9999);
    let pos = new mp.Vector3(-2132.262451171875, 1091.923095703125, 25.622211456298828);
    let freeItem = [0, 0, 0, 0];
    enums.autoSell.forEach(function (item) {
        let spawnPos = new mp.Vector3(item[0], item[1], item[2]);
        if (vehicles.exists(methods.getNearestVehicleWithCoords(spawnPos, 2)))
            return;
        if (methods.distanceToPos(spawnPos, pos) < methods.distanceToPos(prevPos, pos)) {
            prevPos = spawnPos;
            freeItem = item;
        }
    });
...
vehicles.spawnPlayerCar = (id) => {
    ...
    methods.debug('vehicles.spawnPlayerCar', id);
    ...
    vehicles.spawnCarCb(veh => {
        ...
        try {
            ...
            if (vehicles.has(id, 'removeNumber') || vehicles.get(id, 'number_type') < 0)
                veh.numberPlate = '';
            ...
            if (vehicles.get(id, 's_h_break'))
                veh.setVariable('sbreak', true);
...
vehicles.getParkPosition = (className) => {
    methods.debug('vehicles.getParkPosition');
    let array = [];
    switch (className) {
        case 'Helicopters':
...
vehicles.updateAllFractionNumbers = () => {
    methods.debug('vehicles.updateAllFractionNumbers');
...
vehicles.loadAllFractionVehicles = () => {
    methods.debug('vehicles.loadAllFractionVehicles');
...
vehicles.getFractionAllowCarList = function (fractionId, rankType) {
    methods.debug('vehicles.getFractionAllowCarList', fractionId, rankType);
    let carAllowList = [];
...
vehicles.getFractionVehicleInfo = (id) => {
    methods.debug('vehicles.getFractionVehicleInfo');
    let veh = undefined;
...
vehicles.getFractionVehicleIdx = (id) => {
    methods.debug('vehicles.getFractionVehicleIdx');
    let vId = -1;
    id = methods.parseInt(id);
...
vehicles.updateFractionVehicleInfo = (id, newItem) => {
    methods.debug('vehicles.updateFractionVehicleInfo');
...
vehicles.spawnFractionCar = (id) => {
    ...
    methods.debug('vehicles.spawnFractionCar', id);
...
vehicles.loadAllTestVehicles = () => {
    methods.debug('vehicles.loadAllTestVehicles');
...
vehicles.removePlayerVehicle = (userId) => {
    methods.debug('vehicles.removePlayerVehicle', userId);
    mp.vehicles.forEach(async function (v) {
...
vehicles.save = (id) => {
    ...
    return new Promise((resolve) => {
        try {
            methods.debug('vehicles.save2', id);
            ...
            sql = sql + ", winter_tyre = '" + vehicles.get(id, "winter_tyre") + "'";
            sql = sql + ", ticket = '" + vehicles.get(id, "ticket") + "'";
            ...
            sql = sql + ", s_whel_1 = '" + methods.parseInt(vehicles.get(id, "s_whel_1")) + "'";
            sql = sql + ", s_whel_2 = '" + methods.parseInt(vehicles.get(id, "s_whel_2")) + "'";
            sql = sql + ", s_whel_3 = '" + methods.parseInt(vehicles.get(id, "s_whel_3")) + "'";
            sql = sql + ", s_whel_4 = '" + methods.parseInt(vehicles.get(id, "s_whel_4")) + "'";
            sql = sql + ", s_whel_5 = '" + methods.parseInt(vehicles.get(id, "s_whel_5")) + "'";
            sql = sql + ", s_whel_6 = '" + methods.parseInt(vehicles.get(id, "s_whel_6")) + "'";
            sql = sql + ", s_elec = '" + methods.parseInt(vehicles.get(id, "s_elec")) + "'";
            sql = sql + ", s_break = '" + methods.parseInt(vehicles.get(id, "s_break")) + "'";
            sql = sql + ", s_h_break = '" + methods.parseInt(vehicles.get(id, "s_h_break")) + "'";

            sql = sql + ", s_body_door_0 = '" + methods.parseInt(vehicles.get(id, "s_body_door_0")) + "'";
            sql = sql + ", s_body_door_1 = '" + methods.parseInt(vehicles.get(id, "s_body_door_1")) + "'";
            sql = sql + ", s_body_door_2 = '" + methods.parseInt(vehicles.get(id, "s_body_door_2")) + "'";
            sql = sql + ", s_body_door_3 = '" + methods.parseInt(vehicles.get(id, "s_body_door_3")) + "'";
            sql = sql + ", s_body_door_4 = '" + methods.parseInt(vehicles.get(id, "s_body_door_4")) + "'";
            sql = sql + ", s_body_door_5 = '" + methods.parseInt(vehicles.get(id, "s_body_door_5")) + "'";
            sql = sql + ", s_body_door_6 = '" + methods.parseInt(vehicles.get(id, "s_body_door_6")) + "'";
            sql = sql + ", s_body_door_7 = '" + methods.parseInt(vehicles.get(id, "s_body_door_7")) + "'";

            sql = sql + ", s_body_window_0 = '" + methods.parseInt(vehicles.get(id, "s_body_window_0")) + "'";
            sql = sql + ", s_body_window_1 = '" + methods.parseInt(vehicles.get(id, "s_body_window_1")) + "'";
            sql = sql + ", s_body_window_2 = '" + methods.parseInt(vehicles.get(id, "s_body_window_2")) + "'";
            sql = sql + ", s_body_window_3 = '" + methods.parseInt(vehicles.get(id, "s_body_window_3")) + "'";
            sql = sql + ", s_body_window_4 = '" + methods.parseInt(vehicles.get(id, "s_body_window_4")) + "'";
            sql = sql + ", s_body_window_5 = '" + methods.parseInt(vehicles.get(id, "s_body_window_5")) + "'";
            sql = sql + ", s_body_window_6 = '" + methods.parseInt(vehicles.get(id, "s_body_window_6")) + "'";
            sql = sql + ", s_body_window_7 = '" + methods.parseInt(vehicles.get(id, "s_body_window_7")) + "'";
            ...
            methods.error(`catch in vehicles.save`, e.toString())
        }
    });
};
...
vehicles.resetAll = function (id) {
    //methods.debug('vehicles.get');
    return Container.Data.ResetAll(offset + methods.parseInt(id));
};
...
vehicles.getContainer = function (id, key) {
    //methods.debug('vehicles.get');
    return Container.Data.Get(offsetContainer + methods.parseInt(id), key);
};

vehicles.hasContainer = function (id, key) {
    //methods.debug('vehicles.get');
    return Container.Data.Has(offsetContainer + methods.parseInt(id), key);
};
...
vehicles.resetContainerAll = function (id) {
    Container.Data.ResetAll(offsetContainer + methods.parseInt(id));
};

vehicles.setVeh = function (veh, key, val) {
    //methods.debug(`vehicles.set ${id} ${key} ${val} | `);
    if (vehicles.exists(veh))
        Container.Data.Set(offset + methods.parseInt(veh.getVariable('container')), key, val);
};

vehicles.getVeh = function (veh, key) {
    //methods.debug('vehicles.get');
    if (vehicles.exists(veh))
        return Container.Data.Get(offset + methods.parseInt(veh.getVariable('container')), key);
    return null;
};

vehicles.hasVeh = function (veh, key) {
    //methods.debug('vehicles.get');
    if (vehicles.exists(veh))
        return Container.Data.Has(offset + methods.parseInt(veh.getVariable('container')), key);
    return false;
};

vehicles.resetVeh = function (veh, key) {
    //methods.debug('vehicles.get');
    if (vehicles.exists(veh))
        return Container.Data.Reset(offset + methods.parseInt(veh.getVariable('container')), key);
    return true;
};

vehicles.resetVehAll = function (veh) {
    if (vehicles.exists(veh))
        Container.Data.ResetAll(offset + methods.parseInt(veh.getVariable('container')));
};
...
vehicles.getFuelLabel = function (id) {
    methods.debug('vehicles.getFuelLabel');
    switch (id) {
        case 1:
            return 'Бензин';
        case 2:
...
vehicles.getFuelPostfix = function (id) {
    methods.debug('vehicles.getFuelPostfix');
    switch (id) {
        case 1:
        case 2:
        case 4:
...
vehicles.parkFraction = function (id, x, y, z, rot) {
    methods.debug('vehicles.parkFraction', id, x, y, z, rot);
...
vehicles.setColorFraction = function (id, color) {
    methods.debug('vehicles.setColorFraction', id, color);
...
vehicles.updatePrice = function (id, newPrice) {
    try {
        methods.debug('vehicles.updatePrice', id, newPrice);
...
vehicles.destroy = (vehicle) => {
    try {
        methods.debug('vehicles.destroy');
        if (vehicles.exists(vehicle)) {
            vehicle.setVariable('destroy', true);
            try {
                vehicle._attachments.forEach(attach => {
                    try {
                        ...
                        methods.error('vehicles.destroy.addAttachment', e);
                    }
                });
            }
            catch (e) {
                methods.error('vehicles.destroy._attachments', e);
            }
            ...
            try {
                ...
                else if (vehicle.getVariable('fraction_id')) {}
                else vehicles.resetAll(vehicle.getVariable('container'));
            }
            catch (e) {}

            /*try {
                vehicles.resetContainerAll(vehicle.id);
                vehicles.resetVehAll(vehicle);
            }
            catch (e) {}*/

            setTimeout(function () {
                try {
                    methods.debug('vehicles.respawn.setTimeout1');
                    if (!vehicles.exists(vehicle))
                        return;
                    vehicle.dimension = 1410065407;
                    vehicle.alpha = 0;
                    vehicle.position = new mp.Vector3(9999, 9999, 9999);
                    setTimeout(function () {
                        try {
                            methods.debug('vehicles.respawn.setTimeout2');
                            if (!vehicles.exists(vehicle))
                                return;
                            vehicle.destroy()
                        }
                        catch (e) { methods.error('vehicles.respawn.setTimeout.destroy', e); }
                    }, 500)
                }
                catch(e) {
                    methods.error(`catch in timeout2 vehicles.destroy`, e.toString())
                }
        ...
        methods.error('catch in vehicles.destroy', e.toString())
    }
...
vehicles.respawn = (vehicle) => {
    try {
        ...
        if (!vehicles.exists(vehicle))
            return;
        if(vehicle.getVariable('destroy')) return;
        ...
        methods.error('catch in vehicles.respawn', e)
    }

};

vehicles.repair = (vehicle, fixSynced = true) => {
    methods.debug('vehicles.repair', fixSynced);
    if (!vehicles.exists(vehicle))
        return;
    vehicle.repair();
    try {
        let syncData = vSync.getVehicleSyncData(vehicle);
        if (fixSynced) {
            syncData.Window = vSync.VehicleSyncData.Window;
            syncData.Doors = vSync.VehicleSyncData.Doors;
            syncData.Tyre = vSync.VehicleSyncData.Tyre;
        }
        else {
            for (let i = 0; i < 6; i++) {
                if (syncData.Tyre[i] > 0)
                    vSync.setVehicleTyreState(vehicle, i, syncData.Tyre[i]);
            }
            for (let i = 0; i < 8; i++) {
                if (syncData.Doors[i] > 0)
                    vSync.setVehicleDoorState(vehicle, i, syncData.Doors[i]);
            }
            for (let i = 0; i < 8; i++) {
                if (syncData.Window[i] > 0)
                    vSync.setVehicleWindowState(vehicle, i, syncData.Window[i]);
            }
        }
        if (syncData.Dirt)
            vSync.setVehicleDirt(vehicle, syncData.Dirt);
    }
    catch (e) {
        methods.error('vehicles.repair', e);
    }
};
...
vehicles.respawn2 = (vehicle, player) => {
    try {
        ...
        if (!user.isLogin(player))
            return;
        methods.debug('vehicles.respawn2');
...
vehicles.respawnById = (id) => {
    methods.debug('vehicles.respawnById', id);
    mp.vehicles.forEach(function (v) {
        try {
            ...
            methods.error(`catch in vehicles.respawnById`, e.toString())
        }
    })
};

vehicles.findVehicleByNumber = (number) => {
    methods.debug('vehicles.findVehicleByNumber', number);
...
vehicles.getOccupants = (vehicle) => {
    try {
        ...
        if (vehicles.exists(vehicle))
            return vehicle._occupants;
    } catch (e) {
        methods.debug(`error in vehicles.getOccupants`, e.toString())
    }
    return [];
};

vehicles.addOccupant = (vehicle, player) => {
    try {
        methods.debug('vehicles.setOccupant');
        if (vehicles.exists(vehicle) && user.exists(player))
            vehicle._occupants.push(player);
    } catch (e) {
        methods.debug(`error in vehicles.getOccupants`, e.toString())
    }
};

vehicles.removeOccupant = (vehicle, player) => {
    try {
        methods.debug('vehicles.removeOccupant');
        if (vehicles.exists(vehicle)) {
            let players = [];
            vehicle._occupants.forEach(p => {
                try {
                    if (user.exists(player) && player.id !== p.id)
                        players.push(p);
                }
                catch (e) {}
            })
            vehicle._occupants = players;
        }
    } catch (e) {
        methods.debug(`error in vehicles.getOccupants`, e.toString())
    }
...
vehicles.getOccupantsOld = (vehicle) => {
    try {
        methods.debug('vehicles.getOccupants');
        let players = [];
        mp.players.forEach((p) => {
            try {
                ...
                methods.error('vehicles.getOccupants', e);
            }
        });
        return players;
    } catch (e) {
...
vehicles.setFuel = (veh, fuel) => {
    methods.debug('vehicles.setFuel', fuel);
    if (!vehicles.exists(veh))
        return;
    ...
    try {
        ...
        veh.setVariable('fuel', methods.parseFloat(fuel));
    }
    catch (e) {
...
vehicles.setNumberPlate = (veh, number) => {
    methods.debug('vehicles.setNumberPlate', number);
    if (!vehicles.exists(veh))
        return;
    try {
...
vehicles.getNumberPlate = (veh) => {
    methods.debug('vehicles.getNumberPlate');
    if (!vehicles.exists(veh))
        return '';
    return veh.getVariable('numberPlate') || veh.numberPlate;
};

vehicles.addFuel = (veh, fuel) => {
    methods.debug('vehicles.addFuel', fuel);
    if (!vehicles.exists(veh))
        return;
...
vehicles.checkVehiclesFuel = () => {
    ...
    mp.vehicles.forEach(function (veh) {
        try {
            ...
            methods.error(`error in checkVehiclesFuel`, e.toString())
        }
    });

    setTimeout(vehicles.checkVehiclesFuel, 8000);
};

vehicles.generateNumber = function (length = 8) {
    methods.debug('vehicles.generateNumber', length);
    let text = "";
...
vehicles.updateOwnerInfo = function (id, userId, userName) {
    methods.debug('vehicles.updateOwnerInfo', id, userId, userName, vehicles.get(id, 'name'));
    id = methods.parseInt(id);
    ...
    if (userId == 0) {
        ...
        mysql.executeQuery("UPDATE cars SET user_name = '" + userName + "', user_id = '" + userId + "', number = '" + vehicles.generateNumber() + "', tax_money = '0', is_neon = '0', neon_r = '0', neon_g = '0', neon_b = '0', extra = '0', cop_park_name = '', ticket = 0, winter_tyre = 0, is_cop_park = '0', sell_price = '0', s_km = '0', s_eng = '100', s_trans = '100', s_fuel = '100', s_whel_1 = '100', s_whel_2 = '100', s_whel_3 = '100', s_whel_4 = '100', s_whel_5 = '100', s_whel_6 = '100', s_elec = '100', s_break = '100', s_h_break = '0', s_body_door_0 = '1', s_body_door_1 = '1', s_body_door_2 = '1', s_body_door_3 = '1', s_body_door_4 = '1', s_body_door_5 = '1', s_body_door_6 = '1', s_body_door_7 = '1', s_body_window_0 = '1', s_body_window_1 = '1', s_body_window_2 = '1', s_body_window_3 = '1', s_body_window_4 = '1', s_body_window_5 = '1', s_body_window_6 = '1', s_body_window_7 = '1', upgrade = '{\"18\":-1}' where id = '" + id + "'");

        try {
            let vehInfo = methods.getVehicleInfo(vehicles.get(id, 'name'));
            if (vehInfo.display_name !== 'Unknown')
                discord.sendMarketVehicles(`${vehInfo.display_name}`, `Гос. стоимость: ${methods.moneyFormat(vehInfo.price)}\nМакс. скорость: ~${vehInfo.sm}км/ч\nКласс: ${vehInfo.class_name}`, `https://appi-rp.ru/client/images/cars/360/${vehInfo.display_name.toLowerCase()}_1.png`);
        }
        catch (e) { }
    }
    else {
...
vehicles.delete = function (id) {
    methods.debug('vehicles.delete', id);
    id = methods.parseInt(id);
...
vehicles.sell = function (player, slot) {
    methods.debug('vehicles.sell', slot);
    if (!user.isLogin(player))
        return;
    ...
    mp.vehicles.forEach(function (veh) {
        ...
        if (veh.getVariable('container') == containerId) {
            ...
            setTimeout(function () {
                if (!user.isLogin(player))
                    return;
                methods.debug('vehicles.sell.setTimeout');
...
vehicles.setTunning = (veh) => {
    methods.debug('vehicles.setTunning');
    setTimeout(function () {
        try {
            methods.debug('vehicles.setTunning.setTimeout');
            if (vehicles.exists(veh)) {
                try {
                    ...
                    if (car.get('s_whel_1') < 5)
                        vSync.setVehicleTyreState(veh, 0, 2);
                    if (car.get('s_whel_2') < 5)
                        vSync.setVehicleTyreState(veh, 1, 2);
                    if (car.get('s_whel_3') < 5)
                        vSync.setVehicleTyreState(veh, 4, 2);
                    if (car.get('s_whel_4') < 5)
                        vSync.setVehicleTyreState(veh, 5, 2);
                    if (car.get('s_whel_5') < 5)
                        vSync.setVehicleTyreState(veh, 2, 2);
                    if (car.get('s_whel_6') < 5)
                        vSync.setVehicleTyreState(veh, 3, 2);

                    for (let i = 0; i < 8; i++) {
                        if (!car.get('s_body_door_' + i))
                            vSync.setVehicleDoorState(veh, i, 1);
                        if (!car.get('s_body_window_' + i))
                            vSync.setVehicleWindowState(veh, i, 2);
                    }
                    ...
                    if (car.has('upgrade')) {
                        ...
                        for (let tune in upgrade) {
                            ...
                            if (methods.parseInt(tune) >= 100 && upgrade[tune] > -10) {
                                if (methods.parseInt(tune) === 101)
                                    vSync.setCamberState(veh, upgrade[tune]);
                                if (methods.parseInt(tune) === 102)
                                    vSync.setSuspensionRaiseState(veh, upgrade[tune]);
                                if (methods.parseInt(tune) === 103)
                                    vSync.setSteeringLockState(veh, upgrade[tune]);
                            }
                        }

                        setTimeout(async function () {
                            try {
                                methods.debug('vehicles.setTunning.setTimeout2');
                                if (!vehicles.exists(veh))
                                    return;
                                for (let tune in upgrade) {
            ...
            methods.error(`catch in vehicles.setTuning`)
        }
    }, 1000);
};

vehicles.addNew = (model, count) => {
    methods.debug('vehicles.addNew', model, count);
...
vehicles.setHeading = (vid, rot = 0) => {
    methods.debug('vehicles.setHeading', vid, rot);
    mp.players.call('vSync:setHeading', [vid, rot]);
};

let CountAllCars = 0;
vehicles.spawnCar = (position, heading, nameOrModel, number = undefined, dim = 0) => {
    methods.debug('vehicles.spawnCar', position, heading, nameOrModel, number, dim);
...
vehicles.spawnCarCb = (cb, position, heading, nameOrModel, color1 = -1, color2 = -1, liv = -1, number = undefined) => {
    methods.debug('vehicles.spawnCarCb', position, heading, nameOrModel, color1, color2, liv, number);
    if (typeof nameOrModel == 'string')
...
vehicles.spawnJobCar = (cb2, position, heading, nameOrModel, jobId = 0) => {
    methods.debug('vehicles.spawnJobCar', position, heading, nameOrModel, jobId);
...
vehicles.lockStatus = (player, vehicle) => {
    ...
    try {
        ...
        if (!player.vehicle) {
            ...
            setTimeout(function () {
                try {
                    methods.debug('vehicles.lockStatus.setTimeout');
                    if (!user.isLogin(player))
                        return;
...
vehicles.engineStatus = (player, vehicle, status = null) => {
    methods.debug('vehicles.engineStatus', status);
    if (!user.isLogin(player))
        return;
    if (!vehicles.exists(vehicle))
        return;
...
/*vehicles.exists = (vehicle) => {
    ...
    return vehicle && vehicles.exists(vehicle);
};*/

vehicles.exists = (vehicle) => {
    methods.debug('vehicles.exists');
    try {
        if (vehicle && mp.vehicles.exists(vehicle) && vehicle.id) {
            vehicle.getVariable('exists');
            //methods.debug('vehicles.exists', true);
            return true;
        }
    }
    catch (e) {
        methods.error(`catch in vehicles.exists`, e.toString())
        //methods.debug('vehicles.exists', false);
        return false;
    }
    //methods.debug('vehicles.exists', false);
    return false
