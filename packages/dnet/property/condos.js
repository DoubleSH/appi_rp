...
condos.loadBigAll = function() {
    methods.debug('condos.loadBigAll');
...
condos.loadBigLast = function() {
    methods.debug('condos.loadBigLast');
...
condos.getCountLiveUser = function(id, cb) {
    methods.debug('business.getCountLiveUser', id);
...
condos.updateOwnerInfo = function (id, userId, userName) {
    methods.debug('condos.updateOwnerInfo', id, userId, userName);
    id = methods.parseInt(id);
...
condos.updatePin = function (id, pin) {
    methods.debug('condos.updatePin', id, pin);
    id = methods.parseInt(id);
...
condos.updateSafe = function (id, pin, isLoad = false) {
    methods.debug('condos.updateSafe', id, pin, isLoad);
    ...
    if (pin > 0) {
        ...
        try {
            if (mp.objects.exists(hBlips.get(id).safe)) {
                ...
                setTimeout(function () {
                    try {
                        methods.debug('condos.updateSafe.setTimeout1');
            ...
            if (mp.objects.exists(hBlips.get(id).safeDoor)) {
                ...
                setTimeout(function () {
                    try {
                        methods.debug('condos.updateSafe.setTimeout2');
...
condos.lockStatus = function (id, lockStatus) {
    methods.debug('condos.lockStatus', id, lockStatus);
    id = methods.parseInt(id);
...
condos.sell = function (player) {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('condos.sell.setTimeout');
...
condos.buy = function (player, id) {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('condos.buy.setTimeout');
