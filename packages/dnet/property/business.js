...
business.BusinessNewOfficePos = new mp.Vector3(-1032.9556884765625, -434.76702880859375, 62.8613166809082);
business.BusinessNewStreetPos = new mp.Vector3(-1032.4761962890625, -421.5011291503906, 38.616111755371094);
business.BusinessNewGaragePos = new mp.Vector3(-1027.834716796875, -409.0787048339844, 32.41249084472656);
business.BusinessNewRoofPos = new mp.Vector3(-1023.8412475585938, -433.0587463378906, 76.36907958984375);
business.BusinessNewPanelPos = new mp.Vector3(-1013.0193481445312, -419.6404113769531, 66.66131591796875);
...
business.addMoney = function(id, money, name = "Операция со счетом") {
    methods.debug('business.addMoney', id, money);
...
business.removeMoney = function(id, money, name = "Операция со счетом") {
    ...
    methods.debug('business.removeMoney', id, money);
    if (id === 0 || business.get(id, 'user_id') === 0) {
...
business.addMoneyTax = function(id, money) {
    methods.debug('business.addMoneyTax', id, money);
    if (id === 0 || business.get(id, 'user_id') === 0)
        return;
...
business.removeMoneyTax = function(id, money) {
    methods.debug('business.removeMoneyTax', id, money);
    if (id === 0 || business.get(id, 'user_id') === 0)
        return;
...
business.addHistory = function(id, name, price) {
    methods.debug('business.addHistory', id, name, price);
    id = methods.parseInt(id);
...
business.addHistoryTax = function(id, price) {
    methods.debug('business.addHistoryTax');
    id = methods.parseInt(id);
    price = methods.parseFloat(price);
...
business.sell = function (player) {
    ...
    setTimeout(function () {
        ...
        methods.debug('business.sell.setTimeout');
...
business.buy = function (player, id) {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('business.buy.setTimeout');
