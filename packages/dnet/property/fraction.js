...
fraction.resetAll = function () {
    methods.debug('fraction.resetAll');
    mysql.executeQuery(`SELECT * FROM fraction_list`, function (err, rows, fields) {
        rows.forEach(function (item) {
            fraction.set(item['id'], 'order_lamar', 0);
            fraction.set(item['id'], 'order_lamar_m', 0);
            fraction.set(item['id'], 'order_atm', 0);
            fraction.set(item['id'], 'order_fuel', 0);
            fraction.set(item['id'], 'order_drug', 0);
        });
    });
};
...
fraction.loadAll = function () {
    ...
    mysql.executeQuery(`SELECT * FROM fraction_list`, function (err, rows, fields) {
        rows.forEach(function (item) {
            ...
            fraction.set(item['id'], 'interior', item['interior']);
            fraction.set(item['id'], 'sc_alpha', item['sc_alpha']);
            fraction.set(item['id'], 'sc_font', item['sc_font']);
            fraction.set(item['id'], 'sc_color', item['sc_color']);
            fraction.set(item['id'], 'price', item['price']);
            fraction.set(item['id'], 'tax_money', item['tax_money']);
            fraction.set(item['id'], 'tax_score', item['tax_score']);

            fraction.set(item['id'], 'order_lamar', item['order_lamar']);
            fraction.set(item['id'], 'order_lamar_m', item['order_lamar_m']);
            fraction.set(item['id'], 'order_atm', item['order_atm']);
            //fraction.set(item['id'], 'order_fuel', item['order_fuel']); //Да-да, закоменти
            fraction.set(item['id'], 'order_drug', item['order_drug']);
...
fraction.createCargoWar = function (count = 3) {
    ...
    methods.debug('fraction.createCargoWar', count);
    if (isCargo)
        return;
    ...
    spawnList.forEach((item, i) => {
        ...
        mp.players.forEach(p => {
            if (!user.isLogin(p))
                return;
            if (user.get(p, 'fraction_id2') > 0 || user.isAdmin(p) || user.isPolice(p)) {
...
fraction.getNearSpawn = function (pos) {
    methods.debug('fraction.getNearSpawn', pos);
...
fraction.getNearSpawnGarage = function (pos) {
    methods.debug('fraction.getNearSpawnGarage', pos);
    let prevPos = new mp.Vector3(9999, 9999, 9999);
...
fraction.getRandomSpawnGarage = function () {
    methods.debug('stocks.getRandomSpawnGarage');
...
fraction.spawnNearCargo = function (player, isDrug = false, name = 'Speedo4', count = 9) {
    ...
    methods.debug('fraction.spawnNearCargo', isDrug, name, count);
...
fraction.spawnCargo = function (name, boxes, x, y, z, heading = 0, cargoId = 999, color = -1) {
    methods.debug('fraction.spawnCargo', name, boxes, x, y, z, heading, cargoId, color);
    vehicles.spawnCarCb(veh => {
        if (!vehicles.exists(veh))
            return;
        try {
...
fraction.spawnNearBank = function (player, type = 0) {
    ...
    methods.debug('fraction.spawnNearBank', type);
    if (!user.isLogin(player))
        return;
    ...
    vehicles.spawnCarCb(veh => {
        if (!vehicles.exists(veh))
            return;
        methods.debug('fraction.spawnNearBank.setTimeout');
        ...
        try {
            ...
            veh.setColor(color, color);
            veh.windowTint = 1;
            vehicles.setNumberPlate(veh, 'LAM' + user.get(player, 'id') + veh.getVariable('vid'))
            if (type === 0)
                inventory.addItem(262, 1, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
            if (type === 1) {
                inventory.addAmmoItem(280, 5, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addAmmoItem(281, 5, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addAmmoItem(282, 5, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addAmmoItem(283, 5, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addAmmoItem(284, 5, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
            }
            if (type === 2) {
                inventory.addAmmoItem(216, 10, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addAmmoItem(215, 5, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
            }
            if (type === 3) {
                inventory.addAmmoItem(5, 20, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
            }
        }
        catch (e) {
            methods.debug(e);
...
fraction.spawnNearCanabis = function (player) {
    ...
    methods.debug('fraction.spawnNearCanabis');
    if (!user.isLogin(player))
        return;
    ...
    vehicles.spawnCarCb(veh => {
        ...
        try {
            ...
            veh.windowTint = 1;
            vehicles.setNumberPlate(veh, 'LAM' + user.get(player, 'id') + veh.getVariable('vid'))
            inventory.addItem(3, countZones * 6, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
        }
        catch (e) {
            methods.debug(e);
        }
...
fraction.spawnNearGuns = function (player, type = 0) {
    ...
    methods.debug('fraction.spawnNearGuns', type);
    if (!user.isLogin(player))
        return;
    ...
    vehicles.spawnCarCb(veh => {
        ...
        try {
            ...
            vehicles.setNumberPlate(veh, 'LAM' + user.get(player, 'id') + veh.getVariable('vid'))
            if (type === 1) {
                inventory.addItem(280, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addItem(281, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addItem(282, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addItem(283, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addItem(284, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
                inventory.addItem(285, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
            }
            if (type === 2) {
                inventory.addItem(106, 1, inventory.types.Vehicle, inventory.getVehicleHash(veh), 100, 0, "{}");
                inventory.addItem(108, 1, inventory.types.Vehicle, inventory.getVehicleHash(veh), 100, 0, "{}");
                inventory.addItem(110, 1, inventory.types.Vehicle, inventory.getVehicleHash(veh), 100, 0, "{}");
                inventory.addItem(113, 1, inventory.types.Vehicle, inventory.getVehicleHash(veh), 100, 0, "{}");
                inventory.addItem(284, 8, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");

            }
            if (type === 3) {
                inventory.addItem(71, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 100, 0, "{}");
                inventory.addItem(280, 2, inventory.types.Vehicle, inventory.getVehicleHash(veh), 1, 0, "{}");
...
fraction.createCargoMafiaWar = function () {
    ...
    methods.debug('fraction.createCargoMafiaWar');
    if (isCargoMafia)
        return;
    ...
    spawnList.forEach((item, i) => {
        ...
        vehicles.spawnCarCb(veh => {
            ...
            try {
                ...
                veh.locked = false;
                veh.setColor(color, color);
                let boxes = [50, 50, 50, 50, 50, 50, 50, 50, 51, 52, 52];
                veh.setVariable('box', JSON.stringify(boxes));
                veh.setVariable('cargoId', b1);
...
fraction.createCargoBigWar = function () {
    ...
    methods.debug('fraction.createCargoBigWar');
    if (isCargoBig)
        return;
...
fraction.createCargoArmyWar = function () {
    methods.debug('fraction.createCargoArmyWar');
    if (isCargoArmy)
        return;
...
fraction.stopCargoWar = function () {
    ...
    methods.debug('fraction.stopCargoWar');
    isCargo = false;
    timer = 0;
...
fraction.stopCargoMafiaWar = function () {
    ...
    methods.debug('fraction.stopCargoMafiaWar');
    isCargoMafia = false;
    timerMafia = 0;
...
fraction.stopCargoBigWar = function () {
    ...
    methods.debug('fraction.stopCargoBigWar');
    isCargoBig = false;
    timerBig = 0;
...
fraction.stopCargoArmyWar = function () {
    methods.debug('fraction.stopCargoArmyWar');
    isCargoArmy = false;
    mp.players.forEach(p => {
        if (!user.isLogin(p))
            return;
...
fraction.timerCargoWar = function () {
    methods.debug('fraction.timerCargoWar');
    try {
        timer--;
...
fraction.timerCargoBigWar = function () {
    methods.debug('fraction.timerCargoBigWar');
    timerBig--;
...
fraction.timerCargoArmyWar = function () {
    methods.debug('fraction.timerCargoArmyWar');
    let pos1 = null;
    let pos2 = null;
...
fraction.getShopGang = function (player) {
    methods.debug('fraction.getShopGang');
    if (!user.isLogin(player))
        return;
    ...
    setTimeout(function () {
        try {
            methods.debug('fraction.getShopGang.setTimeout');
...
fraction.startGrabShopGang = function (player, itemId = 0) {
    methods.debug('fraction.startGrabShopGang', itemId);
    if (!user.isLogin(player))
        return;
    ...
    shopItem.pos.forEach((pos, i) => {
        if (methods.distanceToPos(new mp.Vector3(pos[0], pos[1], pos[2]), player.position) < 1.5) {
            ...
            setTimeout(function () {
                ...
                methods.debug('fraction.startGrabShopGang.setTimeout');
...
fraction.save = function (id) {
    ...
    return new Promise((resolve) => {
        ...
        sql = sql + ", interior = '" + methods.removeQuotes(fraction.get(id, "interior")) + "'";
        sql = sql + ", sc_font = '" + methods.removeQuotes(fraction.get(id, "sc_font")) + "'";
        sql = sql + ", sc_color = '" + methods.removeQuotes(fraction.get(id, "sc_color")) + "'";
        sql = sql + ", sc_alpha = '" + methods.removeQuotes(fraction.get(id, "sc_alpha")) + "'";

        sql = sql + ", order_lamar = '" + methods.parseInt(fraction.get(id, "order_lamar")) + "'";
        sql = sql + ", order_lamar_m = '" + methods.parseInt(fraction.get(id, "order_lamar_m")) + "'";
        sql = sql + ", order_atm = '" + methods.parseInt(fraction.get(id, "order_atm")) + "'";
        sql = sql + ", order_drug = '" + methods.parseInt(fraction.get(id, "order_drug")) + "'";
...
fraction.addMoney = function (id, money, name = "Операция со счетом") {
    methods.debug('fraction.addMoney');
...
fraction.addHistory = function (name, doName, text, fractionId) {
    methods.debug('fraction.addHistory');
...
fraction.editFractionName = function (player, text) {
    methods.debug('fraction.editFractionName');
    if (!user.isLogin(player))
        return;
...
fraction.editFractionLeader = function (player, text) {
    methods.debug('fraction.editFractionLeader');
    if (!user.isLogin(player))
        return;
...
fraction.editFractionSubLeader = function (player, text) {
    methods.debug('fraction.editFractionSubLeader');
    if (!user.isLogin(player))
        return;
...
fraction.editFractionRank = function (player, text, rankId, depId) {
    methods.debug('fraction.editFractionRank');
    if (!user.isLogin(player))
        return;
...
fraction.deleteFractionRank = function (player, rankId, depId) {
    methods.debug('fraction.deleteFractionRank');
    if (!user.isLogin(player))
        return;
...
fraction.editFractionDep = function (player, text, depId) {
    methods.debug('fraction.editFractionDep');
    if (!user.isLogin(player))
        return;
...
fraction.deleteFractionDep = function (player, depId) {
    methods.debug('fraction.deleteFractionDep');
    if (!user.isLogin(player))
        return;
...
fraction.addFractionRank = function (player, text, depId) {
    methods.debug('fraction.addFractionRank');
    if (!user.isLogin(player))
        return;
...
fraction.createFractionDep = function (player, text) {
    methods.debug('fraction.createFractionDep');
    if (!user.isLogin(player))
        return;
...
fraction.create = function (player, id) {
    ...
    if (user.getCashMoney(player) < 250000) {
        player.notify('~r~У Вас не достаточно средств для создания организации');
        return;
    }
    ...
    fraction.set(id, "name", user.getLastName(player) + ' Family LTD');
    user.removeCashMoney(player, 250000, 'Создание организации');

    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('fraction.create.setTimeout');
        user.save(player);
        fraction.save(id);
...
fraction.destroy = function (player, id) {
    ...
    id = methods.parseInt(id);
    methods.debug('fraction.destroy', id);

    if (!user.isLeader2(player)) {
        player.notify('~r~Эта организация вам не принадлежит');
        return;
    }
...
fraction.destroyJust = function (id) {
    id = methods.parseInt(id);
    methods.debug('fraction.destroyJust', id);

    fraction.set(id, "name", 'Офис');
