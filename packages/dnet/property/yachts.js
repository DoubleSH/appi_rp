...
yachts.updateOwnerInfo = function (id, userId, userName) {
    methods.debug('yachts.updateOwnerInfo', id, userId, userName);
...
yachts.updateName = function (id, name) {
    methods.debug('yachts.updateName', id, name);
    id = methods.parseInt(id);
...
yachts.sell = function (player) {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('yachts.sell.setTimeout');
...
yachts.buy = function (player, id) {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('yachts.buy.setTimeout');
