...
houses.garageList = [
    ...
    [-1072.5390625, -66.4981918334961, -90.53367614746094, 179.13075256347656, -1072.5390625, -66.4981918334961, -90.53367614746094, 179.13075256347656], //7
    [-1357.4638671875, 163.92323303222656, -99.4342041015625, -165.08839416503906, -1357.4638671875, 163.92323303222656, -99.4342041015625, -165.08839416503906], //8
...
houses.updateOwnerInfo = function (id, userId, userName) {
    methods.debug('houses.updateOwnerInfo', id, userId, userName);
    id = methods.parseInt(id);
...
houses.updateSafe = function (id, pin, isLoad = false) {
    try {
        methods.debug('houses.updateSafe', id, pin, isLoad);
        ...
        if (pin > 0) {
            ...
            try {
                if (mp.objects.exists(hBlips.get(id).safe)) {
                    hBlips.get(id).safe.dimension = 1410065407;
                    setTimeout(function () {
                        methods.debug('houses.updateSafe.setTimeout1');
                        hBlips.get(id).safe.destroy();
                    }, 1000)
                }
                if (mp.objects.exists(hBlips.get(id).safeDoor)) {
                    hBlips.get(id).safeDoor.dimension = 1410065407;
                    setTimeout(function () {
                        methods.debug('houses.updateSafe.setTimeout2');
...
houses.updatePin = function (id, pin) {
    methods.debug('houses.updatePin', id, pin);
    id = methods.parseInt(id);
    pin = methods.parseInt(pin);
...
houses.lockStatus = function (id, lockStatus) {
    methods.debug('houses.lockStatus', id, lockStatus);
    id = methods.parseInt(id);
...
houses.sell = function (player) {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('houses.sell.setTimeout');
...
houses.buy = function (player, id) {
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        methods.debug('houses.buy.setTimeout');
...
houses.enterv = function (player, id) {
    ...
    if (garageId == -1) {
        user.showCustomNotify(player, 'Вы отошли слишком далеко', 1, 9);
        return;
    }
    ...
    if (vehicles.exists(v) && player.vehicle) {
        ...
        if (vehicles.exists(v2) && player.vehicle) {
            user.showCustomNotify(player, 'К сожалению, сейчас у ворот уже стоит транспорт, необходимо чтобы он отъехал', 1, 9);
            return;
        }

        if (vehicles.exists(player.vehicle)){
            player.vehicle.position = new mp.Vector3(houses.garageList[garageId][0], houses.garageList[garageId][1], houses.garageList[garageId][2])
            player.vehicle.rotation = new mp.Vector3(0,0, houses.garageList[garageId][3])
            player.vehicle.dimension = id;
            vehicles.getOccupants(player.vehicle).forEach(function (p) {
                try {
                    if(p.seat == 0) return
                    p.dimension = id;
                    p.position = new mp.Vector3(houses.garageList[garageId][0], houses.garageList[garageId][1], houses.garageList[garageId][2])
                }
                catch (e) {
                    methods.debug(e);
                }
            ...
            player.position = new mp.Vector3(houses.garageList[garageId][0], houses.garageList[garageId][1], houses.garageList[garageId][2])
            player.dimension = id;
        }
...
houses.exitv = function (player, id) {
    ...
    if (vehicles.exists(player.vehicle)){
        player.vehicle.position = new mp.Vector3(hInfo.get('gx' + garageId), hInfo.get('gy' + garageId), hInfo.get('gz' + garageId))
        player.vehicle.rotation = new mp.Vector3(0,0,hInfo.get('grot' + garageId))
        ...
        vehicles.getOccupants(player.vehicle).forEach(function (p) {
            try {
                if(p){
                    if(p.seat == 0) return
                    p.dimension = 0;
                    p.position = new mp.Vector3(hInfo.get('gx' + garageId), hInfo.get('gy' + garageId), hInfo.get('gz' + garageId))   
                }
        ...
        player.position = new mp.Vector3(hInfo.get('gx' + garageId), hInfo.get('gy' + garageId), hInfo.get('gz' + garageId))
        player.dimension = 0;
    }
};