...
weather.load = function() {
    ...
    setInterval(weather.timeTimer, 8571);
    setInterval(weather.timeSyncTimer, 1000);
    setInterval(weather.saveTimer, 60 * 1000);
    setInterval(weather.weatherTimer, 30 * 60 * 1000);
};
...
weather.trafficLightTimer = function() {
    methods.debug('weather.trafficLightTimer');
    ...
    if (isBrokeLight) {
        ...
        if (currentLightIdx === 1 || currentLightIdx === 2 || currentLightIdx === 4 || currentLightIdx === 5)
            setTimeout(weather.trafficLightTimer, 500);
        else
            setTimeout(weather.trafficLightTimer, 5000);
    }
...
weather.setBrokeLight = function() {
    methods.debug('weather.setBrokeLight');
    isBrokeLight = true;
    setTimeout(function () {
        methods.debug('weather.setBrokeLight.setTimeout');
        isBrokeLight = false;
    }, 20000);
};
...
weather.timeSyncTimer = function() {
    ...
    mp.players.forEach(function (p) {
       try {
           ...
           //realHour, realTime, realDate, realDateTime, min, hour, day, month, year, windSpeed, windDir, temp, hash

           let uuid = methods.hashList[methods.getRandomInt(0, methods.hashList.length - 1)];
           p.hashList.push(uuid);
           p.call('client:managers:weather:sync',
               [
                   dateTime.getHours(),
                   `${methods.digitFormat(dateTime.getHours())}:${methods.digitFormat(dateTime.getMinutes())}`,
                   `${methods.digitFormat(dateTime.getDate())}/${methods.digitFormat(dateTime.getMonth()+1)}`,
                   ...
                   _minute, _hour, _day, _month, _year,
                   _windSpeed, _windDir, Math.round(_tempNew),
                   uuid
               ]
           );

           if (p.hashList.length > 10)
               p.hashList.shift();
           if (p.eventValidate.length > 20)
               p.eventValidate.splice(0, p.eventValidate.length - 20);
       }
       catch (e) {}
    });
};

weather.timeTimer = function() {
    ...
    try {
        ...
        /*if (dateTime.getHours() === 20 && dateTime.getMinutes() === 1) {*if (!isCreateVeh) {
                isCreateVeh = true;
                try {
        ...
        if (dateTime.getHours() == 4 && dateTime.getMinutes() == 45) //TODO
            methods.notifyToAll('Рестарт сервера через 15 минут');
        if (dateTime.getHours() == 4 && dateTime.getMinutes() == 55)
            methods.notifyToAll('Рестарт сервера через 5 минут');
        if (dateTime.getHours() == 4 && dateTime.getMinutes() == 58) {
            methods.saveAll();
            fraction.resetAll();
        }
...
weather.setWeather = function(weatherName) {
    methods.debug('weather.setWeather', weatherName);
...
weather.nextRandomWeatherByType = function(weatherType) {
    methods.debug('weather.nextRandomWeatherByType', weatherType);
...
weather.changeTime = function(time = 0) {
    try {
        _hour = time > 23 ? 0 : time;
        methods.notifyWithPictureToAll('Смена времени', 'Администрация', 'Администратор изменил время на сервере на ' + _hour + ':' + _minute, 'CHAR_ACTING_UP');
    }
    catch (e) {}
};