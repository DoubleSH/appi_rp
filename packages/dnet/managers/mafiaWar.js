...
mafiaWar.loadAll = function() {
    ...
    setTimeout(function () {
        setInterval(mafiaWar.timer, 1000);
    }, 10000);
};

mafiaWar.save = function(id, ownerId) {
    methods.debug('mafiaWar.save', id, ownerId);
...
mafiaWar.getZoneId = function(position) {
    methods.debug('mafiaWar.getZoneId', position);
...
mafiaWar.startWar = function(id) {
    methods.debug('mafiaWar.startWar', id);
...
mafiaWar.timerMoney = function() {
    methods.debug('mafiaWar.timerMoney')
...
mafiaWar.getMaxCounterFractionId = function(rm, trd, lcn) {
    methods.debug('mafiaWar.getMaxCounterFractionId')
...
mafiaWar.isInZone = function(player, zone) {
    methods.debug('mafiaWar.isInZone')
    try {
        let list = [];
        zone.forEach(item => {
