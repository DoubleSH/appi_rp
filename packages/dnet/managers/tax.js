...
tax.loadAll = function() {
    ...
    mysql.executeQuery("UPDATE fraction_list SET tax_score = (RAND(90000000) * 10000000) + 60000000");
};
...
tax.sell = function() {
    ...
    mysql.executeQuery("SELECT * FROM stocks WHERE tax_money <= (round(price * '" + _currentTax + "' + '" + _taxMin + "', 0) * '" + _taxDays + "') * '-1' AND user_id > '0' LIMIT 20", function (err, rows, fields) {
        rows.forEach(row => {
            ...
            let price = methods.parseInt(row['price'] / 2);

            if (methods.parseInt(row["tax_money"]) < -100000)
                price = methods.parseInt(row['price']);
    ...
    //============Организации=============
    //=============================
    mysql.executeQuery("SELECT * FROM fraction_list WHERE tax_money <= (round(price * '" + _currentTax + "' + '" + _taxMin + "', 0) * '7') * '-1' AND owner_id > '0' LIMIT 20", function (err, rows, fields) {
        rows.forEach(row => {

            fraction.destroyJust(row['id']);
            tax.adLiveInvader(`Организация ${row["name"]} поступил в продажу`);
            mysql.executeQuery("UPDATE fraction_list SET tax_money = '0' WHERE id = '" + row['id'] + "'");

            methods.saveLog('log_sell_inactive',
                ['text'],
                [`USER: ${row["owner_id"]} FRACTION ${row["id"]}`],
            );
        });
...
tax.removeTax = function() {
    ...
    mysql.executeQuery("UPDATE fraction_list SET tax_money = tax_money - (round((price * '" + _currentTax + "' + '" + _taxMin +  "') / '7', 0)) WHERE owner_id > 0 AND is_war = 0");
...
tax.updateTax = function() {
    ...
    mysql.executeQuery(`SELECT * FROM fraction_list WHERE owner_id > '0'`, function (err, rows, fields) {
        rows.forEach(item => {
