...
graffiti.loadAll = function () {
    methods.debug('graffiti.loadAll');
...
graffiti.createWar = function() {
    ...
    if (isWar)
        return;
    methods.debug('graffiti.createWar');
...
graffiti.stopWar = function() {
    methods.debug('graffiti.stopWar');
    try {
        isWar = false;
        timer = 0;
        ...
        graffiti.position.forEach((item, idx) => {
            try {
                ...
                if (frId > 0) {
                    fraction.setMoney(frId, fraction.getMoney(frId) + 0.5);
                    if (moneyToUser.has(frId.toString()))
                        moneyToUser.set(frId.toString(), moneyToUser.get(frId.toString()) + 0.1);
                    else
                        moneyToUser.set(frId.toString(), 0.1);
                }
            }
            catch (e) {
                methods.error(`catch in graffiti position foreach stopwar`, e.toString())
            }
        });
        ...
        mp.players.forEach(async p => {
            try {
                ...
                methods.error(`catch in graffity stopwar foreach `, e.toString())
            }
        });
        ...
        methods.error(`catch in graffiti.stopWar`, e.toString())
    }
};

graffiti.timerWar = function() {
    methods.debug('graffiti.timerWar')
    timer--;
    if (timer < 1) {
        graffiti.stopWar();
        return;
...
graffiti.save = function(id) {
    ...
    return new Promise((resolve) => {
        methods.debug('graffiti.save2', id);
...
graffiti.changeGraffiti = function(player) {
    methods.debug('graffiti.changeGraffiti');
    ...
    graffitiList.forEach((obj, idx) => {
        if (methods.distanceToPos(obj.position, player.position) < 4) {
            ...
            setTimeout(function () {
                if (!user.isLogin(player))
                    return;
                methods.debug('graffiti.changeGraffiti.setTimeout');
                try {
                    ...
                    try {
                        ...
                        {
                            obj.dimension = 1410065407;
                            setTimeout(function () {
                                try {
                                    methods.debug('graffiti.changeGraffiti.setTimeout2');
                                    obj.destroy();
                                }
                                catch (e) {}
                            }, 1000);
...
graffiti.getType = function(type, fraction = 0) {
    methods.debug('graffiti.getType', type, fraction);
