...
dispatcher.sendPos = function (title, desc, pos, withCoord = true, phone = 0) {
    ...
    methods.debug('dispatcher.sendPos', title, desc, pos, withCoord, phone);
...
dispatcher.sendLocalPos = function (title, desc, pos, fractionId, withCoord = true) {
    ...
    methods.debug('dispatcher.sendLocalPos', title, desc, pos, fractionId, withCoord);
    let time = methods.digitFormat(weather.getHour()) + ':' + methods.digitFormat(weather.getMin());
    mp.players.forEach(function (player) {
...
dispatcher.getTaxiMenu = function (player) {
    methods.debug('dispatcher.getTaxiMenu');
    if (methods.isBlackout())
        return;
...
dispatcher.getMechMenu = function (player) {
    methods.debug('dispatcher.getMechMenu');
    if (methods.isBlackout())
        return;
    if (!user.isLogin(player))
        return;
...
dispatcher.sendTaxiPos = function (title, desc, pos, wpos, price, phone = 0) {
    methods.debug('dispatcher.sendTaxiPos', title, desc, pos, wpos, price, phone);
    if (methods.isBlackout()) {
        return;
    }
    countTaxi++;
...
dispatcher.sendMechPos = function (title, desc, pos, phone = 0) {
    methods.debug('dispatcher.sendMechPos', title, desc, pos, phone);
    if (methods.isBlackout()) {
        return;
    }
    countMeh++;
...
dispatcher.acceptTaxi = function (player, id) {
    methods.debug('dispatcher.acceptTaxi', id);
    if (!user.isLogin(player))
        return;
    ...
    phone.sendMessageByNumber(user.get(player, 'phone'), order.phone, `Здравствуйте. Таксист принял ваш заказ. Транспорт: ${vInfo.display_name} | ${player.vehicle.numberPlate}`, 's/GTAVCAnniversary/n/GTAVC52.png');

    player.notify('~y~Вы приняли заказ');
};

dispatcher.acceptMech = function (player, id) {
    methods.debug('dispatcher.acceptMech', id);
    if (!user.isLogin(player))
        return;
    ...
    user.setWaypoint(player, order.pos.x, order.pos.y);
    phone.sendMessageByNumber(user.get(player, 'phone'), order.phone, `Здравствуйте. Механик принял ваш заказ, ожидайте.`, 's/GTAVCAnniversary/n/GTAVC52.png');
    player.notify('~y~Вы приняли заказ');
};

mp.events.add("playerEnterVehicle", (player, vehicle, seat) => {
    methods.debug('playerEnterVehicle123')
    try {
...
mp.events.add("playerExitVehicle", (player, vehicle) => {
    methods.debug('playerExitVehicle123')
    try {
        if (!vehicles.exists(vehicle))
            return;
        ...
        methods.error('dispatcher.playerExitVehicle', e);
    }
});