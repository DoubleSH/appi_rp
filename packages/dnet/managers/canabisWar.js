...
canabisWar.loadAll = function() {
    ...
    setTimeout(function () {
        methods.debug('canabisWar.loadAll.setTimeout');
        canabisWar.timer();
        setInterval(canabisWar.timer, 1000);
...
canabisWar.save = function(id, ownerId, name) {
    ...
    methods.debug('canabisWar.save', id, ownerId, name);
...
canabisWar.startWar = function(zoneId, attack, def, isArmor, count) {
    ...
    methods.debug('canabisWar.startWar', zoneId, attack, def, isArmor, count);
...
canabisWar.addWar = function(player, zoneId, count, armorIndex, gunIndex, timeIndex) {
    methods.debug('canabisWar.addWar', zoneId, count, armorIndex, gunIndex, timeIndex);
    ...
    try {
        ...
        mp.players.forEach(async p => {
            try {
                ...
                methods.error(`catch in canabisWar.timerMoney.foreach`)
            }
        });
    }
    catch (e) {
        methods.error(`catch in canabisWar.timerMoney`, e.toString())
    }
};

canabisWar.getCountZones = function(fractionId = -1) {
    methods.debug('canabisWar.getCountZones');
    let count = 0;
    gangList.forEach(item => {
...
mp.events.add("playerDeath", (player, reason, killer) => {
    methods.debug('playerDeath3124231244')
    if (!isStartTimer)
        return;
