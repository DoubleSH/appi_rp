...
gangZone.currentWeapon = function() {
    methods.debug('gangZone.currentWeapon ')
    return currentWeapon;
};

gangZone.currentLobby = function() {
    methods.debug('gangZone.currentLobby ')
    let count = 0;
...
gangZone.playerToLobby = function(player) {
    ...
    if (user.isLogin(player)) {
        ...
        player.setVariable('gangzone', true)
        player.dimension = 99999;
        user.set(player, 'blockDeath', true);
        ...
        setTimeout(function () {
            try {
                methods.debug('gangZone.playerToLobby.setTimeout');
                let spawnIdx = methods.getRandomInt(0, zoneSpawnList[currentZone].length);
...
gangZone.playerExitLobby = function(player) {
    ...
    if (user.isLogin(player)) {
        ...
        player.setVariable('gangzone', false)
        user.removeAllWeapons(player);
        player.dimension = 0;
        user.reset(player, 'blockDeath');
...
mp.events.add("playerDeath", (player, reason, killer) => {
    methods.debug('playerDeath12')
    try {
        if (user.isLogin(player.lastDamage) && user.isLogin(player) && user.has(player, 'gangZoneKills')) {
                ...
                if (user.has(player, 'gangZoneKills') && user.has(killer, 'gangZoneKills')) {
                    ...
                    setTimeout(function () {
                        try {
                            if (!user.isLogin(player))
                                return;
                            methods.debug('gangZone.playerDeath.setTimeout');
                    ...
                    setTimeout(function () {
                        try {
                            methods.debug('gangZone.playerDeath.setTimeout2');
                            if (user.has(player, 'gangZoneKills'))
                                user.giveWeapon(player, currentWeapon, 9999);
                        }
                        catch (e) {}
                    ...
                    mp.players.forEach(async p => {
                        try {
                            ...
                            methods.error(`catch in gangZone111`, e.toString())
                        }
                    });
                }
            } catch (e) {
        ...
        methods.error(`catch in gangZone playerDeath`, e.toString())
    }
});