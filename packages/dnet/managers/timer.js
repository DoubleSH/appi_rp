...
timer.loadAll = function() {
    setInterval(timer.min60Timer, 1000 * 60 * 60);
    setInterval(timer.min59Timer, 1000 * 60 * 59);
    //setInterval(timer.min2hTimer, 1000 * 60 * 120);
    setInterval(timer.min30Timer, 1000 * 60 * 30);
    setInterval(timer.min10Timer, 1000 * 60 * 10);
    setInterval(timer.min1Timer, 1000 * 60);
    setInterval(timer.sec10Timer, 1000 * 10);
    setInterval(timer.sec5Timer, 1000 * 5);
};

timer.min30Timer = function() {
    methods.debug('timer.min30Timer');
...
timer.min10Timer = function() {
    methods.debug('timer.min10Timer');
    try {
        stocks.removeState();
    }
    catch (e) {}
    ...
    mp.players.forEach(async function (p) {
        if (user.isLogin(p)) {
            try {
...
timer.min1Timer = async function() {
    methods.debug('timer.min1Timer');
    try {
        for (let i = 0; i < 50; i++) {
        ...
        methods.error(`catch in min1timer`, e.toString())
    }
};

timer.min60Timer = function() {
    methods.debug('timer.min60Timer');
    ...
    mp.players.forEach(async function (p) {
        try {
            ...
            methods.error(`catch in foreach min60timer`)
        }
    });
...
timer.min59Timer = async function() {
    methods.debug('timer.min59Timer');
    ...
    try {
        timer.containerCounter().then();
    }
    catch (e) {}
};


timer.containerCounter = function() {
    return new Promise(resolve => {
        methods.debug('timer.containerCounter');

        try {
            let data = Container.Data.GetDataAll();
            let array = [];
            data.forEach((value, key) => {
                array.push({key: key, count: value.size});
            });
            let jsonObj = {
                countAll: data.size,
                data: array,
            };
            methods.saveFile(`containerCounter-${methods.getDate2()}-${methods.getTime()}`, `${JSON.stringify(jsonObj)}`);
        }
        catch (e) {
...
timer.min2hTimer = function() {
    methods.debug('timer.min2hTimer');
    /*try {
        let player = methods.getRandomPlayer();
        if (user.isLogin(player)) {
            user.giveRandomMask(player, 0, true);
...
timer.sec10Timer = function() {
    methods.debug('timer.sec10Timer');
    mp.players.forEach(async function (p) {
        try {
            if (user.isLogin(p)) {

                if (user.has(p, 'spawnName') && p.dimension === 0) {
            ...
            methods.error(`catch in sec10timer`, e.toString())
        }
    });
    ...
    mp.vehicles.forEach(async function (v) {
        if (vehicles.exists(v)) {
            try {
                if (v.getVariable('fraction_id') || v.getVariable('user_id') || v.getVariable('useless') || v.getVariable('prolog') || v.prolog || vehicles.getOccupants(v).length > 0)
                    return;
