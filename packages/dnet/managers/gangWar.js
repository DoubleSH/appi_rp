...
gangWar.loadAll = function() {
    ...
    setTimeout(function () {
        setInterval(gangWar.timer, 1000);
    }, 10000);
};

gangWar.save = function(id, ownerId, name) {
    methods.debug('gangWar.save', id, ownerId, name);
...
gangWar.getZoneList = function() {
    methods.debug('canabisWar.getZoneList');
    let list = [];
...
gangWar.getNearZoneId = function(pos) {
    methods.debug('canabisWar.getNearZoneId');
    let zoneId = -1;
...
gangWar.getZoneWarList = function() {
    methods.debug('canabisWar.getZoneWarList');
    return warPool;
};

gangWar.hasWar = function(idx) {
    methods.debug('canabisWar.hasWar');
    if(!warPool) return
    return warPool.has(idx.toString());
};

gangWar.getWar = function(idx) {
    methods.debug('canabisWar.getWar');
    if(!warPool) return
...
gangWar.startWar = function(zoneId, attack, def, isArmor, count) {
    ...
    methods.debug('gangWar.startWar', zoneId, attack, def, isArmor, count);
...
gangWar.addWar = function(player, zoneId, count, armorIndex, gunIndex, timeIndex) {
    methods.debug('gangWar.addWar', zoneId, count, armorIndex, gunIndex, timeIndex);
...
gangWar.timer = function() {
    methods.debug('canabisWar.timer');
    if (isStartTimer) {
        timerCounter--;
...
gangWar.timerMoney = function() {
    methods.debug('canabisWar.timerMoney');
    try {
        ...
        for (let i = 1; i <= gangList.length; i++) {
            if (gangWar.get(i, 'fraction_id') > 0) {
                ...
                let money = methods.getRandomInt(20, 40) / 1000;
                ...
                if (moneyToUser.has(gangWar.get(i, 'fraction_id').toString())) {
                    ...
                    moneyToUser.set(gangWar.get(i, 'fraction_id').toString(), methods.getRandomInt(20, 40) / 1000);
                }
        ...
        mp.players.forEach(async p => {
            try {
                ...
                methods.error(`catch in gangWar.timerMoney foreach`, e.toString())
            }
        });
    }
    catch (e) {
        methods.error(`catch in gangWar.timerMoney`, e.toString())
    }

};

gangWar.getMaxCounterFractionId = function(at, def) {
    methods.debug('canabisWar.getMaxCounterFractionId');
...
gangWar.isInZone = function(player, id) {
    methods.debug('canabisWar.isInZone');
    try {
        /*let list = [];
...
gangWar.getZone = function(id) {
    methods.debug('canabisWar.getZone');
    try {
        let list = [];
...
gangWar.changeZoneColor = function(id, fractionId) {
    methods.debug('canabisWar.changeZoneColor');
