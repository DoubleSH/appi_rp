...
pickups.QuestBotDetective = new mp.Vector3(-810.9228515625, -993.209228515625, 12.432669639587402);
...
pickups.BotLspd4 = new mp.Vector3(1540.8948974609375, 816.7403564453125, 76.65579223632812);
...
pickups.SellVehicle = new mp.Vector3(-2132.11181640625, 1097.13134765625, 24.66229248046875);
...
pickups.CasinoMoneyBuy = new mp.Vector3(1115.73, 220.01, -50.43);
...
pickups.DispatcherPos5 = new mp.Vector3(1542.8997802734375, 827.9708862304688, 76.65573120117188);
pickups.DispatcherPos6 = new mp.Vector3(1543.9739990234375, 829.8317260742188, 76.65581512451172);
...
pickups.Drift1Pos = new mp.Vector3(-224.581298828125, -270.901611328125, 47.89645004272461);
pickups.Drift2Pos = new mp.Vector3(-2127.963134765625, 1105.9530029296875, -28.48540687561035);
pickups.Drift3Pos = new mp.Vector3(-2127.968017578125, 1105.9530029296875, 24.543169021606445);
...
pickups.Garage31Pos = new mp.Vector3(-1079.1270751953125, -83.37319946289062, -91.19998168945312);
pickups.Garage32Pos = new mp.Vector3(-1078.9368896484375, -83.38263702392578, -95.59982299804688);
pickups.Garage33Pos = new mp.Vector3(-1078.8330078125, -83.67906951904297, -99.99977111816406);
...
pickups.Bcsd1StockPos = new mp.Vector3(1540.625, 810.4092407226562, 76.6557846069336);
...
pickups.SheriffGarderobPos = new mp.Vector3(1538.3870849609375, 810.810791015625, 76.65585327148438);
...
pickups.SheriffArsenalPos = new mp.Vector3(1549.541015625, 837.256103515625, 76.65579986572266);
...
pickups.SheriffClearPos = new mp.Vector3(1543.3280029296875, 817.1266479492188, 76.65577697753906);
pickups.SheriffClearPos2 = new mp.Vector3(370.0141906738281, -1585.6617431640625, 28.29206085205078);
pickups.SheriffArrestPos = new mp.Vector3(1556.091064453125, 836.8241577148438, 76.65575408935547);
...
pickups.SapdKeyPos = new mp.Vector3(458.533935546875, -1008.1021728515625, 27.275184631347656);
pickups.SheriffKeyPos = new mp.Vector3(1527.5404052734375, 793.7092895507812, 76.45545196533203);
...
pickups.SheriffInfo1Pos = new mp.Vector3(1539.2261962890625, 815.8829345703125, 81.13055419921875);
...
//Island
pickups.IslandPos1 = new mp.Vector3(134.6911163330078, -3335.5224609375, 5.021927356719971);
pickups.IslandPos1.rot = -107;
pickups.IslandPos2 = new mp.Vector3(5076.19287109375, -4648.87451171875, 1.310251235961914);
pickups.IslandPos2.rot = 57;

//AutoRepair Shop
pickups.AutoRepairShopPos1 = new mp.Vector3(-967.8649291992188, -2069.7802734375, 8.405892372131348);
pickups.AutoRepairShopPos1.rot = 128;
pickups.AutoRepairShopPos2 = new mp.Vector3(970.6948852539062, -2990.9794921875, -40.646976470947266);
pickups.AutoRepairShopPos2.rot = -155;
...
pickups.GangVehPos1 = new mp.Vector3(7.4240, -1809.3, 25.01075);
...
pickups.checkPressLAlt = function (player) {
    ...
    if (methods.distanceToPos(business.BusinessStreetPos, playerPos) < distanceCheck ||
        ...
        player.call('client:menuList:showBusinessOfficeTeleportMenu');

    if (methods.distanceToPos(business.BusinessNewStreetPos, playerPos) < distanceCheck ||
        methods.distanceToPos(business.BusinessNewRoofPos, playerPos) < distanceCheck ||
        methods.distanceToPos(business.BusinessNewGaragePos, playerPos) < distanceCheck ||
        methods.distanceToPos(business.BusinessNewOfficePos, playerPos) < distanceCheck)
    ...
    if (methods.distanceToPos(pickups.Drift1Pos, playerPos) < 4 ||
        methods.distanceToPos(pickups.Drift2Pos, playerPos) < 4 ||
        methods.distanceToPos(pickups.Drift3Pos, playerPos) < 4)
        player.call('client:menuList:showDriftTeleportMenu');
    ...
    if (methods.distanceToPos(pickups.Garage31Pos, playerPos) < 3 ||
        methods.distanceToPos(pickups.Garage32Pos, playerPos) < 3 ||
        methods.distanceToPos(pickups.Garage33Pos, playerPos) < 3)
        player.call('client:menuList:showGarage3TeleportMenu');
    ...
    if (methods.distanceToPos(pickups.AutoRepairShopPos1, playerPos) < distanceCheck)
        user.teleportVeh(player, pickups.AutoRepairShopPos2.x, pickups.AutoRepairShopPos2.y, pickups.AutoRepairShopPos2.z + 1, pickups.AutoRepairShopPos2.rot);
    if (methods.distanceToPos(pickups.AutoRepairShopPos2, playerPos) < distanceCheck)
        user.teleportVeh(player, pickups.AutoRepairShopPos1.x, pickups.AutoRepairShopPos1.y, pickups.AutoRepairShopPos1.z + 1, pickups.AutoRepairShopPos1.rot);
...
pickups.checkPressE = function (player) {
    ...
    if (methods.distanceToPos(pickups.WheelLuckyPos, playerPos) < distanceCheck) {
        setTimeout(function () {
            try {
                methods.debug('pickups.checkPressE.setTimeout');
                wheel.start(player);
            }
            catch (e) { }
        }, methods.getRandomInt(500, 1000));
    }

    /*if (methods.distanceToPos(pickups.CasinoMoneyBuy, playerPos) < distanceCheck) {
        player.call('client:casino:showCasinoMoneyMenu');
    }*/
    ...
    if (player.dimension > 0) {
        if (methods.distanceToPos(business.BusinessBotPos, playerPos) < distanceCheck)
            player.call('client:menuList:showOfficeMenu', [Array.from(fraction.getData(player.dimension))]);
        if (methods.distanceToPos(business.BusinessNewPanelPos, playerPos) < distanceCheck)
...
pickups.createAll = function () {
    ...
    //methods.createCpVector(pickups.CasinoMoneyBuy, 'Нажмите ~g~E~s~ чтобы открыть меню', 1, -1, pickups.Yellow)
    ...
    methods.createCpVector(pickups.AutoRepairShopPos1, "Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом", 4, -1, pickups.Blue100, 0.3);
    methods.createCpVector(pickups.AutoRepairShopPos2, "Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом", 4, -1, pickups.Blue100, 0.3);
    ...
    methods.createCpVector(business.BusinessNewOfficePos, 'Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом', 1, -1, pickups.Blue100);
    methods.createCpVector(business.BusinessNewStreetPos, 'Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом', 1, -1, pickups.Blue100);
    methods.createCpVector(business.BusinessNewRoofPos, 'Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом', 1, -1, pickups.Blue100);
    methods.createCpVector(business.BusinessNewGaragePos, 'Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом', 1, -1, pickups.Blue100);
    methods.createCpVector(business.BusinessNewPanelPos, 'Нажмите ~g~E~s~ чтобы открыть меню', 1, -1, pickups.Blue);
    ...
    methods.createCpVector(pickups.Drift1Pos, 'Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом', 4, -1, pickups.Blue100, 0.3);
    methods.createCpVector(pickups.Drift2Pos, 'Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом', 4, -1, pickups.Blue100, 0.3);
    methods.createCpVector(pickups.Drift3Pos, 'Нажмите ~g~Left Alt~s~ чтобы воспользоваться пикапом', 4, -1, pickups.Blue100, 0.3);
