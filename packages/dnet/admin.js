...
admin.giveLeader = function(player, type, id, listIndex) {
    try {
        ...
        methods.debug('admin.giveLeader', type, id, listIndex);
...
admin.blacklist = function(player, type, id, reason) {
    try {
        methods.debug('admin.blacklist', type, id, reason);
        if (!user.isAdmin(player))
            return;
        ...
        if (type === 0) {
            ...
            mysql.executeQuery(`INSERT INTO black_list (social, serial, rgsc_id, address, reason) VALUES ('${target.socialClub}', '${target.serial}', '${target.rgscId}', '${target.ip}', '${reason}')`);
...
admin.ban = function(player, type, id, listIndex, reason) {
    try {
        methods.debug('admin.ban', type, id, listIndex, reason);
        if (!user.isAdmin(player))
            return;
        ...
        if (type === 0) {
            ...
            player.notify('~r~Игрок был забанен');
...
admin.banByAnticheat = function(type, id, listIndex, reason) {
    try {
        methods.debug('admin.banByAnticheat', type, id, listIndex, reason);
        id = methods.parseInt(id);
...
admin.unban = function(player, type, id, reason) {
    try {
        methods.debug('admin.unban', type, id, reason);
        if (!user.isAdmin(player))
            return;
        ...
        if (type === 0) {
            ...
            mysql.executeQuery(`SELECT * FROM users WHERE id = '${methods.parseInt(id)}'`, (err, rows, fields) => {
                rows.forEach(row => {
                    ...
                    player.notify('~r~Игрок был разбанен');
                    if (reason === '0')
                        return;
...
admin.warn = function(player, type, id, reason) {
    try {
        methods.debug('admin.warn', type, id, reason);
        if (!user.isAdmin(player))
            return;
        ...
        if (type === 0) {
            ...
            methods.saveLog('log_admin', ['name', 'type', 'do'], [`${user.getRpName(player)}`, 'USER_WARN', `${user.getRpName(target)}`]);
...
admin.unwarn = function(player, type, id, reason) {
    try {
        ...
        if (type === 0) {
            ...
            user.warn(target, -1, reason, false);
            methods.saveLog('log_admin', ['name', 'type', 'do'], [`${user.getRpName(player)}`, 'USER_UNWARN', `${user.getRpName(target)}`]);
...
admin.untimer = function(player, type, id, reason) {
    try {
        methods.debug('admin.untimer', type, id, reason);
        if (!user.isAdmin(player))
            return;
...
admin.kick = function(player, type, id, reason) {
    try {
        methods.debug('admin.kick', type, id, reason);
        if (!user.isAdmin(player))
            return;
        ...
        if (type === 0) {
            ...
            methods.saveLog('log_admin', ['name', 'type', 'do'], [`${user.getRpName(player)}`, 'USER_KICK', `${user.getRpName(target)}`]);
            if (reason === '0')
                return;
...
admin.jail = function(player, type, id, min, reason) {
    try {
        methods.debug('admin.jail', type, id, min, reason);
        if (!user.isAdmin(player))
            return;
        ...
        if (type === 0) {
            ...
            methods.saveLog('log_admin', ['name', 'type', 'do'], [`${user.getRpName(player)}`, 'USER_JAIL', `${user.getRpName(target)}`]);
            if (reason === '0')
                return;
            ...
            mysql.executeQuery(`SELECT * FROM users WHERE id = '${methods.parseInt(id)}'`, (err, rows, fields) => {
                rows.forEach(row => {
                    ...
                    methods.saveLog('log_admin', ['name', 'type', 'do'], [`${user.getRpName(player)}`, 'USER_JAIL', `${row['name']}`]);
                    if (reason === '0')
                        return;
...
admin.setArmorById = function(player, type, id, num) {
    try {
        methods.debug('admin.setArmorById', type, id, num);
        if (!user.isAdmin(player))
            return;
...
admin.setHpById = function(player, type, id, num) {
    try {
        methods.debug('admin.setHpById', type, id, num);
        if (!user.isAdmin(player))
            return;
...
admin.setSkinById = function(player, type, id, skin) {
    try {
        methods.debug('admin.setSkinById', type, id, skin);
        if (!user.isAdmin(player))
            return;
...
admin.resetSkinById = function(player, type, id) {
    try {
        methods.debug('admin.resetSkinById', type, id);
        if (!user.isAdmin(player))
            return;
...
admin.adrenalineById = function(player, type, id) {
    try {
        methods.debug('admin.adrenalineById', type, id);
        if (!user.isAdmin(player))
            return;
...
admin.freeHospById = function(player, type, id) {
    try {
        methods.debug('admin.freeHospById', type, id);
        if (!user.isAdmin(player))
            return;
...
admin.tpToAdmin = function(player, type, id) {
    try {
        methods.debug('admin.tpToAdmin', type, id);
        if (!user.isAdmin(player))
            return;
...
admin.tpToUser = function(player, type, id) {
    try {
        methods.debug('admin.tpToUser', type, id);
        if (!user.isAdmin(player))
            return;
...
admin.changeDimension = function(player, type, id, dim) {
    try {
        methods.debug('admin.changeDimension', type, id, dim);
        if (!user.isAdmin(player))
            return;
...
admin.getDimension = function(player, type, id) {
    try {
        methods.debug('admin.getDimension', type, id);
        if (!user.isAdmin(player))
            return;
