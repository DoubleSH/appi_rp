...
phone.getUserInfo = function(player, text) {
    ...
    methods.debug('phone.getUserInfo', text);
...
phone.getVehInfo = function(player, text) {
    ...
    methods.debug('phone.getVehInfo', text);
...
phone.getGunInfo = function(player, text) {
    ...
    methods.debug('phone.getGunInfo', text);
...
phone.memberAction = function(player, id) {
    ...
    methods.debug('phone.memberAction', id);
        ...
        rows.forEach(row => {
            ...
            if (user.isLeader(player) || user.isSubLeader(player)) {
                items.push(phone.getMenuItemModal(
                    ...
                    '',
                    'Премия',
                    `Вы точно хотите выписать премию ${row['name']}?`,
                    'Выписать',
                    'Отмена',
                    { name: 'fractionBonus', memberId: row['id'], named: row['name']},
                    '',
                    true
                ));
            }
...
phone.memberAction2 = function(player, id) {
    ...
    methods.debug('phone.memberAction2', id);
...
phone.memberActionF = function(player, id) {
    ...
    methods.debug('phone.memberActionF');
...
phone.fractionVehicleAction = function(player, id) {
    ...
    methods.debug('phone.fractionVehicleAction', id);
    let fractionId = user.get(player, 'fraction_id');
...
phone.fractionVehicleAction2 = function(player, id) {
    ...
    methods.debug('phone.fractionVehicleAction2', id);
    let fractionId = user.get(player, 'fraction_id2');
...
phone.fractionMoney = function(player) {
    ...
    if(fraction.isGos(user.get(player, 'fraction_id'))){
        items.push(phone.getMenuItemModalInput(
            'Снять средства',
            '',
            'Введите значение',
...
phone.fractionListF = function(player, showStats = false) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.fractionListF', showStats);
...
phone.fractionVehicles = function(player) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.fractionVehicles');
...
phone.fractionVehicles2 = function(player) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.fractionVehicles2');
...
phone.fractionVehiclesBuyList2 = function(player) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.fractionVehiclesBuyList2');
...
phone.userNewsList = function(player) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.userNewsList');
...
phone.bankHistory = function(player, bankCard) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.bankHistory', bankCard);
...
phone.openInvaderStatsList = function(player, days = 30) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.openInvaderStatsList', days);
    try {
...
phone.userHistory = function(player, id) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.userHistory', id);
...
phone.userTickets = function(player, id) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.userTickets', id);
...
phone.changeBg = function(player, str) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.changeBg', str);
...
phone.userVehicleAppMenu = function(player) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.userVehicleAppMenu');
...
phone.fractionVehicleBuyInfo = function(player, id) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.fractionVehicleBuyInfo', id);
...
phone.fractionVehicleBuyInfo2 = function(player, id) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.fractionVehicleBuyInfo2', id);
...
phone.updateContactList = function(player) {
    ...
    methods.debug('phone.updateContactList');
    try {
...
phone.updateDialogList = function(player) {
    ...
    methods.debug('phone.updateDialogList');
    let myPhone = user.get(player, 'phone').toString();
    try {
        mysql.executeQuery(`SELECT *, pa1.number as number1, pa2.number as number2, pa1.avatar as avatar1, pa2.avatar as avatar2 FROM phone_sms LEFT JOIN phone_avatars as pa1 ON phone_sms.number_from=pa1.number  LEFT JOIN phone_avatars as pa2 ON phone_sms.number_to=pa2.number  WHERE number_from = '${myPhone}' OR number_to = '${myPhone}' GROUP BY number_from, number_to ORDER BY phone_sms.id DESC LIMIT 100`, (err, rows, fields) => {
...
phone.selectChat = function(player, phoneNumber, chat) {
    ...
    methods.debug('phone.selectChat', phoneNumber, chat);
...
phone.deleteChat = function(player, phoneNumber) {
    ...
    methods.debug('phone.deleteChat', phoneNumber);
...
phone.sendMessageByNumber = function(numberFrom, phoneNumber, message, avatar) {
    try {
        methods.debug('phone.sendMessageByNumber', numberFrom, phoneNumber, message, avatar);
        if (methods.parseInt(phoneNumber) === 0)
            return;
...
phone.showMenu = function(player, uuid, title, items) {
    if (!user.isLogin(player))
        return;
    methods.debug('phone.showMenu', uuid, title, items);
