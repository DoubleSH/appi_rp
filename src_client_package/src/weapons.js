...
weapons.getDamageHeadByHash = function(hash) {
    let damage = 1;
    weapons.getMapList().forEach(item => {
        if (item[1] / 2 == hash)
            damage = item[3];
...
weapons.getDamageByHashName = function(hash) {
    let damage = 1;
    weapons.getMapList().forEach(item => {
        if (mp.game.joaat(item[0]) == hash)
            damage = item[2];
    });
    return damage;
};

weapons.getDamageHeadByHashName = function(hash) {
    let damage = 1;
    weapons.getMapList().forEach(item => {
        if (mp.game.joaat(item[0]) == hash)
            damage = item[3];
