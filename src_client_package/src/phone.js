...
phone.showBrowser = async function() {
    phone.ingameBrowser = mp.browsers.new(`https://appi-rp.ru/browser?login=${await user.get('a_l')}&password=${await user.get('a_p')}`);
    phone.ingameBrowser.validate = true;
    //phone.ingameBrowser = mp.browsers.new(`https://appi-rp.ru/browser`);
...
phone.destroyBrowser = function() {
    try {
        ...
        setTimeout(function() {
            if(phone && phone.ingameBrowser)
                phone.ingameBrowser = null;
        }, 500);
    }
    catch (e) {
        methods.error('catch in phone.destroyBrowser', e.toString())
    }
};

phone.showSlafy = async function() {
    phone.ingameBrowser = mp.browsers.new(`https://game.slafy.ru/?login=${mp.players.local.userName}&token=${await user.get('a_t')}`);
    phone.ingameBrowser.validate = true;
    //phone.ingameBrowser = mp.browsers.new(`https://appi-rp.ru/browser`);
    mp.gui.chat.activate(false);
    mp.gui.cursor.show(true, true);
...
phone.apps = function(action) {
    ...
    switch (action) {
        ...
        case 'settings':
            ...
            break;
        case 'uveh':
            methods.callRemote('server:phone:userVehicleAppMenu');
            phone.showLoad();
            break;
        case 'sms':
            methods.callRemote('server:phone:updateDialogList');
            chat.activate(false);
            mp.gui.cursor.show(true, true);
            break;
        case 'cont':
            methods.callRemote('server:phone:updateContactList');
            chat.activate(false);
            mp.gui.cursor.show(true, true);
            break;
        default:
...
phone.updateMainAppList = function() {
    let appList = [
        ...
        { link: "/phone/android/umenu", action: 'slafy', img: 'slafy', name: 'Slafy' },
    ];
    let appListBottom = [
        ...
        items: [
            ...
            },
            {
                ...
                umenu: [
                    {
                        title: 'Время и дата',
                        text: weather.getFullRpTime() + ' · ' + weather.getFullRpDate(),
                        type: 1,
                        params: { name: 'none' }
                    },
                    {
                        title: 'Температура',
                        text: '' + weather.getWeatherTempFormat(),
                        type: 1,
                        params: { name: 'none' }
                    },
                    {
                        title: 'Ветер',
                        text: `${methods.parseFloat(weather.getWindSpeed() * 1.4).toFixed(1)}m/s`,
                        type: 1,
                        params: { name: 'none' }
                    },
                ],
            },
            {
                ...
                umenu: [
                    {
                        title: 'Район',
                        text: ui.getCurrentZone(),
                        type: 1,
                        params: { name: 'none' }
                    },
                    {
                        title: 'Улица',
                        text: ui.getCurrentStreet(),
                        type: 1,
                        params: { name: 'none' }
                    },
                ],
...
phone.showAppListService = function() {
...
phone.showAppFraction2 = async function() {
    ...
    let order_lamar = await fraction.get(user.getCache('fraction_id2'), 'order_lamar');
    let order_lamar_m = await fraction.get(user.getCache('fraction_id2'), 'order_lamar_m');
    let order_atm = await fraction.get(user.getCache('fraction_id2'), 'order_atm');
    //let order_fuel = await fraction.get(user.getCache('fraction_id2'), 'order_fuel');
    let order_drug = await fraction.get(user.getCache('fraction_id2'), 'order_drug');
    ...
    if (order_lamar > 5000) {
        ...
        titleMenu.umenu.push(
            {
                title: "Грузы Ламара (Speedo)",
                text: `Перевезите ${order_lamar}/50 грузов ламара и получите Speedo`,
                type: 1,
                clickable: true,
    ...
    if (order_lamar_m > 5000) {
        ...
        titleMenu.umenu.push(
            {
                title: "Грузы Ламара (Mule)",
                text: `Перевезите ${order_lamar_m}/100 грузов ламара и получите Mule`,
                type: 1,
                clickable: true,
    ...
    if (order_atm > 5000) {
        ...
        titleMenu.umenu.push(
            {
                title: "Кража банкоматов",
                text: `Взломайте ${order_atm}/5 банкоматов и получите 100btc на счет организации`,
                type: 1,
                clickable: true,
                params: { name: "getAtm" }
            },
    ...
    if (order_drug > 5000) {
        ...
        titleMenu.umenu.push(
            {
                title: "Закладки",
                text: `Разнесите ${order_drug}/75 закладок и получите груз с наркотиками`,
                type: 1,
                clickable: true,
                params: { name: "getDrug" }
            },
    ...
    /*if (fData.get('is_mafia')) {
        ...
        if (user.isLeader2() || user.isSubLeader2()) {
            ...
            if (await fraction.has(user.getCache('fraction_id2'), 'order_drug_marg')) {
                titleMenu.umenu.push(
                    {
...
phone.showAppFractionUpgrade2 = async function() {
    ...
    if (!fData.get('is_mafia'))  {
        if (fData.get('money') >= 5000) {
            menu.items.push(
                {
                    title: 'Улучшение Синдиката',
                    umenu: [
                        {
                            title: `Улучшить за ${methods.cryptoFormat(5000, 0)}`,
                            text: `Для улучшение необходимо иметь 25 человек в организации`,
                            type: 1,
                            clickable: true,
                            params: { name: "setMafia" }
                        }
                    ],
                }
            );
            ...
            menu.items.push(
                {
                    ...
                    umenu: [
                        {
                            title: `Не хватает ${methods.cryptoFormat(5000 - fData.get('money'), 0)} для улучшения`,
                            text: `Для улучшение необходимо иметь 25 человек в организации`,
                            type: 1,
                            params: { name: "none" }
                        }
...
phone.consoleCallback = async function(command) {
    ...
    try {
        ...
        if (cmd === 'help') {
            ...
            else if (args[0] === 'https://appi-rp.ru/' || args[0] === 'https://appi-rp.ru') {
            ...
            else if (args[0] === 'https://appi-rp.ru/publicApi' || args[0] === 'https://appi-rp.ru/publicApi') {
                if (user.getCache('stats_darknet') < 35) {
                    phone.addConsoleCommand('Look up');
                    phone.addConsoleCommand('Access Denied');
                    return;
            ...
            if (args.length === 0 || args[0] === '-h') {
                ...
                if (args[1] === 'AABABBAAABABBABAAABAAAABBABBABBAABABAAABAABBB') {
                    ...
                    methods.callRemote('server:phone:getUserInfo', methods.parseInt(args[2]));
                }
                else {
                    phone.addConsoleCommand('Access denied');
                }
                ...
                if (args[1] === 'WARISGONE') {
                    ...
                    phone.showLoad();
                    await methods.sleep(100);
                    methods.callRemote('server:phone:getGunInfo', methods.removeQuotes2(methods.removeQuotes(args[2])));
                }
                else {
                    phone.addConsoleCommand('Access denied');
                }
                ...
                if (args[1] === 'MR.DEADINSIDE') {
                    ...
                    if (args[2] === 'LGBT') {
                        ...
                        methods.callRemote('server:phone:getVehInfo', methods.removeQuotes2(methods.removeQuotes(args[2])));
                    }
                }
                else {
                    phone.addConsoleCommand('Access denied');
                ...
                if (args[1] === 'APPIRPTOP') {
                    ...
                    phone.showLoad();
                    await methods.sleep(100);
                    methods.callRemote('server:business:log', methods.parseInt(args[2]));
                }
                else {
                    phone.addConsoleCommand('Access denied');
                }
                ...
                /*if (!weather.isNight()) {
                    phone.addConsoleCommand('Access denied. You can use this command only Night: 23:00 to 6:00');
                    return;
                }*/
                ...
                if (methods.getRandomInt(0, 99) < user.getCache('stats_darknet')) {
                    ...
                    methods.callRemote('server:trafficDestroy');
                }
                else {
                ...
                if (methods.getRandomInt(0, 101) < user.getCache('stats_darknet')) {
                    ...
                    methods.callRemote('server:cityDestroy')
                }
                else {
                    user.giveWanted(50, 'Попытка взлома системы управления городом');
                    phone.addConsoleCommand('Error, try again');
                ...
                if (methods.getRandomInt(0, 101) < user.getCache('stats_darknet')) {
                    ...
                    methods.callRemote('server:networkDestroy')
                }
                else {
                    user.giveWanted(50, 'Попытка взлома системы управления городом');
                    phone.addConsoleCommand('Error, try again');
                        ...
                        if (vInfo.fuel_type === 3 || vInfo.class_name === "Super") {
                            ...
                            if (methods.getRandomInt(0, 100) < 30 + user.getCache('stats_darknet')) {
                                if (v.getVariable('fraction_id') > 0 || v.getVariable('isAdmin') || v.getVariable('useless')) {
                                    ...
                                    methods.callRemote('server:vehicle:lockStatus:hack', v.remoteId);
                                    phone.addConsoleCommand('Success');
                    ...
                    if (methods.md5(v.remoteId.toString()).slice(0, 6) === hash) {
                        ...
                        if (vInfo.fuel_type === 3 || vInfo.class_name === "Super") {
                            ...
                            if (methods.getRandomInt(0, 100) < 10 + user.getCache('stats_darknet')) {
                                methods.callRemote('server:vehicle:engineStatus:hack', v.remoteId);
                                phone.addConsoleCommand('Success');
                    ...
                    for (let i = 0; i < 25; i++) {
                        if (mp.vehicles.exists(currentVeh)) {
                            ...
                            if (dist > 30) {
                                ...
                                methods.callRemote('server:vehicles:speedLimit', currentVeh.remoteId, 0);
                                return;
                            }
                            ...
                            methods.callRemote('server:vehicles:speedLimit', currentVeh.remoteId, speed);
            ...
            if (args.length === 0 || args[0] === '-h') {
                ...
                /*if (args[1] === '-fraction') {
                    ...
                    methods.callRemote('server:crypto:transferMoney', args[1], sum);
                }
                else {
                    phone.addConsoleCommand('Usage: ecorp -send [coin number] [sum]');
                }
            ...
            /*else if (args[0] === '-fraction') {
                if (args[1] === '-create') {
                    if (user.getCache('rep') < 100 && user.getCache('fraction_id2') == 0) {
                        methods.callRemote('server:phone:createFraction');
                        phone.toMainUMenu();
                        phone.showLoad();
                    }
                    else {
                    ...
                    methods.callRemote('server:phone:fractionAll');
                    phone.toMainUMenu();
                    phone.showLoad();
                }
                else {
                ...
                if (args[1] === '-getpos') {
                    ...
                    if (loader.isStart()) {
                        mp.game.ui.notifications.show('~r~Это действие недоступно сейчас');
                        return;
                    }
                ...
                if (args[1] === '-rifle') {
                    ...
                    methods.callRemote('server:fraction:getLamarRifle');
                }
                else if (args[1] === '-ammo') {
                    ...
                    methods.callRemote('server:fraction:getLamarAmmo');
                }
                    ...
                    methods.callRemote('server:fraction:getLamarPistol');
                }
                else {
                    phone.addConsoleCommand('Usage: ecorp -money -clear');
                }
...
phone.callBackRadio = function(checked, id, ...args) {
    ...
    try {
        ...
        if (params.name == 'memberNewRank') {
            methods.callRemote('server:user:newRank', params.memberId, params.rankId);
            phone.showAppFraction();
        }
        if (params.name == 'memberNewDep') {
            methods.callRemote('server:user:newDep', params.memberId, params.depId);
            phone.showAppFraction();
        }
        if (params.name == 'memberNewRank2') {
            methods.callRemote('server:user:newRank2', params.memberId, params.rankId);
            phone.showAppFraction2();
        }
        if (params.name == 'memberNewDep2') {
            methods.callRemote('server:user:newDep2', params.memberId, params.depId);
            phone.showAppFraction2();
        }
        if (params.name == 'memberNewRankF') {
            methods.callRemote('server:user:newRankF', params.memberId, params.rankId);
            phone.showAppFamily();
        }
        if (params.name == 'memberNewDepF') {
            methods.callRemote('server:user:newDepF', params.memberId, params.depId);
            phone.showAppFamily();
        }
        if (params.name == 'vehicleNewRank') {
            methods.callRemote('server:fraction:vehicleNewRank', params.memberId, params.rankId);
            phone.showAppFraction();
        }
        if (params.name == 'vehicleNewDep') {
            methods.callRemote('server:fraction:vehicleNewDep', params.memberId, params.depId);
            phone.showAppFraction();
        }
    }
    catch(e) {
...
phone.callBackModal = function(paramsJson) {
    try {
        ...
        if (params.name == 'memberUninvite') {
            methods.callRemote('server:user:uninvite', params.memberId);
            phone.showAppFraction();
        }
        if (params.name == 'fractionBonus') {
            let menu = {
                UUID: 'fraction',
                title: user.getFractionNameL(),
                items: [
                    {
                        ...
                        umenu: [
                            {
                                title: "Введите размер премии до 5.000$",
                                modalTitle: 'Введите размер',
                                modalButton: ['Отмена', 'Отправить'],
                                type: 8,
                                clickable: true,
                                params: { name: "fractionBonus2", memberId: params.memberId, named: params.named}
                            }
                        ],
                    },
                ],
        ...
        if (params.name == 'memberGiveSubLeader') {
            methods.callRemote('server:user:giveSubLeader', params.memberId);
            phone.showAppFraction();
        }
        if (params.name == 'memberTakeSubLeader') {
            methods.callRemote('server:user:takeSubLeader', params.memberId);
            phone.showAppFraction();
        }
        if (params.name == 'memberUninvite2') {
            methods.callRemote('server:user:uninvite2', params.memberId);
            phone.showAppFraction2();
        }
        if (params.name == 'memberGiveSubLeader2') {
            methods.callRemote('server:user:giveSubLeader2', params.memberId);
            phone.showAppFraction2();
        }
        if (params.name == 'memberGiveLeader2') {
            methods.callRemote('server:user:giveLeader2', params.memberId);
            phone.showAppFraction2();
        }
        if (params.name == 'memberTakeSubLeader2') {
            methods.callRemote('server:user:takeSubLeader2', params.memberId);
            phone.showAppFraction2();
        }
        if (params.name == 'memberUninviteF') {
            methods.callRemote('server:user:uninviteF', params.memberId);
            phone.showAppFamily();
        }
        if (params.name == 'memberGiveSubLeaderF') {
            methods.callRemote('server:user:giveSubLeaderF', params.memberId);
            phone.showAppFamily();
        }
        if (params.name == 'memberGiveLeaderF') {
            methods.callRemote('server:user:giveLeaderF', params.memberId);
            phone.showAppFamily();
        }
        if (params.name == 'memberTakeSubLeaderF') {
            methods.callRemote('server:user:takeSubLeaderF', params.memberId);
            phone.showAppFamily();
        }
        if (params.name == 'fractionVehicleBuy') {
            methods.callRemote('server:fraction:vehicleBuy', params.vehId, params.price);
            phone.showAppFraction();
        }
        if (params.name == 'fractionVehicleBuy2') {
            methods.callRemote('server:fraction:vehicleBuy2', params.vehId, params.price);
            phone.showAppFraction2();
        }
        if (params.name == 'fractionVehicleSell') {
            methods.callRemote('server:fraction:vehicleSell', params.vehId, params.price);
            phone.showAppFraction();
        }
        if (params.name == 'fractionVehicleSell2') {
            methods.callRemote('server:fraction:vehicleSell2', params.vehId, params.price);
            phone.showAppFraction2();
        }
        if (params.name == 'deleteFractionDep') {
            methods.callRemote('server:phone:deleteFractionDep', params.depId);
        ...
        if (params.name == 'deleteFractionRank') {
            ...
            methods.callRemote('server:phone:deleteFractionRank', params.rankId, params.depId);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
    }
    catch(e) {
...
phone.callBackModalInput = async function(paramsJson, text) {
    try {
        ...
        if (params.name == 'getUserInfo') {
            methods.callRemote('server:phone:getUserInfo', text);
        }
        if (params.name == 'getVehInfo') {
            methods.callRemote('server:phone:getVehInfo', text);
        }
        if (params.name == 'getGunInfo') {
            methods.callRemote('server:phone:getGunInfo', text);
        }
        if (params.name == 'changeBg') {
            ...
            phone.updateBg(text);
            phone.toMainPage();
            methods.callRemote('server:phone:changeBg', text);
        }
        if (params.name == 'inviteFraction2') {
            methods.callRemote('server:phone:inviteFraction2', methods.parseInt(text));
        }
        if (params.name == 'inviteFamily') {
            methods.callRemote('server:phone:inviteFamily', methods.parseInt(text));
        }
        ...
        if (params.name == 'mafiaClearWanted') {
            methods.callRemote('server:phone:mafiaClearWanted', methods.parseInt(text));
        }
        if (params.name == 'mafiaGiveShop') {
            methods.callRemote('server:phone:mafiaGiveShop', methods.parseInt(text));
        }
        if (params.name == 'destroyFraction') {
            if (text.toLowerCase() === 'да')
                methods.callRemote('server:phone:destroyFraction');
            else
                mp.game.ui.notifications.show(`~r~Вы не ввели слово "да"`);
        }
        if (params.name == 'destroyFamily') {
            if (text.toLowerCase() === 'да')
                methods.callRemote('server:phone:destroyFamily');
            else
                mp.game.ui.notifications.show(`~r~Вы не ввели слово "да"`);
        }
        if (params.name == 'editFractionName') {

            methods.callRemote('server:phone:editFractionName', text);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
        if (params.name == 'createFractionDep') {
            methods.callRemote('server:phone:createFractionDep', text);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
        if (params.name == 'editFractionLeader') {
            methods.callRemote('server:phone:editFractionLeader', text);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
        if (params.name == 'editFractionSubLeader') {
            methods.callRemote('server:phone:editFractionSubLeader', text);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
        if (params.name == 'editFractionRank') {
            methods.callRemote('server:phone:editFractionRank', text, params.rankId, params.depId);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
        if (params.name == 'editFractionDep') {
            methods.callRemote('server:phone:editFractionDep', text, params.depId);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
        if (params.name == 'addFractionRank') {
            methods.callRemote('server:phone:addFractionRank', text, params.depId);
            setTimeout(phone.showAppFractionHierarchy2, 300);
        }
        if (params.name == 'createFractionDepF') {
            methods.callRemote('server:phone:createFractionDepF', text);
            setTimeout(phone.showAppFractionHierarchyF, 300);
        }
        if (params.name == 'editFractionLeaderF') {
            methods.callRemote('server:phone:editFractionLeaderF', text);
            setTimeout(phone.showAppFractionHierarchyF, 300);
        }
        if (params.name == 'editFractionSubLeaderF') {
            methods.callRemote('server:phone:editFractionSubLeaderF', text);
            setTimeout(phone.showAppFractionHierarchyF, 300);
        }
        if (params.name == 'editFractionRankF') {
            methods.callRemote('server:phone:editFractionRankF', text, params.rankId, params.depId);
            setTimeout(phone.showAppFractionHierarchyF, 300);
        }
        if (params.name == 'editFractionDepF') {
            methods.callRemote('server:phone:editFractionDepF', text, params.depId);
            setTimeout(phone.showAppFractionHierarchyF, 300);
        }
        if (params.name == 'addFractionRankF') {
            methods.callRemote('server:phone:addFractionRankF', text, params.depId);
            setTimeout(phone.showAppFractionHierarchyF, 300);
        }
        if (params.name == 'fractionBenefit') {
            ...
            methods.callRemote('server:phone:fractionMoney');
            phone.showLoad();
        }
        if (params.name == 'fractionTake') {
            if(user.isGos()){
                mp.game.ui.notifications.show(`~r~Гос фракциям запрещено снимать деньги с бюджета организации`);
                return;
            }
            let price = methods.parseFloat(text);
        ...
        if (params.name == 'sendAd') {
            ...
            methods.callRemote('server:invader:sendAdTemp', text);
        ...
        if (params.name == 'fractionBonus2') {
            ...
            if (price > 5000) {
                mp.game.ui.notifications.show(`~r~Значение не может быть больше 5.000$`);
                return;
            }

            // coffer.removeMoney(coffer.getIdByFraction(user.getCache('fraction_id')), methods.parseInt(text));
            // user.addBankMoney(methods.parseInt(text), 'Снял деньги с организации');
            // user.sendSmsBankOperation('Зачисление премии: ~g~' + methods.moneyFormat(methods.parseInt(text)));
            methods.callRemote('server:fractionBonus', methods.parseInt(text), params.memberId)
            phone.showAppFraction();

            methods.saveFractionLog(
                user.getCache('name'),
                `Выдал премию сотруднику ${params.named}`,
                `Сумма: ${methods.moneyFormat(methods.parseInt(text))}`,
                user.getCache('fraction_id')
            );
        }
        if (params.name == 'sendFractionNews') {
            ...
            switch (user.getCache('fraction_id')) {
                case 1:
                    ...
                    break;
                case 2:
                    methods.sendDiscordServerNews(title, 'Новости SAPD', text);
                ...
                case 4:
                    ...
                    break;
                case 5:
                    methods.sendDiscordServerNews(title, 'Новости SASD', text);
                    methods.notifyWithPictureToAll(title, 'Новости SASD', text, 'DIA_POLICE');
                    break;
                case 6:
    ...
    try {
        ...
        if (menu == 'fraction') {
            if (params.name == 'hierarchy')
                ...
                methods.callRemote('server:phone:fractionList');
                phone.showLoad();
            }
            else if (params.name == 'respawnVeh') {
                methods.callRemote('server:fraction:vehicleRespawn', params.memberId);
                phone.showAppFraction();
            }
            else if (params.name == 'log') {
                methods.callRemote('server:phone:fractionLog');
                phone.showLoad();
            }
            else if (params.name == 'getDrugVans') {
                methods.callRemote('server:fraction:getDrugVans');
            }
                ...
                methods.callRemote('server:phone:fractionVehicles');
                phone.showLoad();
            }
            else if (params.name == 'fractionVehicleBuyInfo') {
                methods.callRemote('server:phone:fractionVehicleBuyInfo', params.id);
                phone.showLoad();
            }
            else if (params.name == 'vehicleBuyList') {
                methods.callRemote('server:phone:fractionVehiclesBuyList');
                phone.showLoad();
            }
            else if (params.name == 'destroyVehicle') {
                if (Container.Data.HasLocally(mp.players.local.remoteId, "isCarRespawn"))
                {
                    mp.game.ui.notifications.show("~r~Таймаут на действие 3 секунды");
                    return;
                }
                Container.Data.SetLocally(mp.players.local.remoteId, "isCarRespawn", true);

                methods.callRemote('server:respawnNearstVehicle');

                setTimeout(function () {
                    Container.Data.ResetLocally(mp.players.local.remoteId, "isCarRespawn");
                }, 3000);
            }
                ...
                methods.callRemote('server:respawnNearstVehicle2');
                ...
                if (methods.parseInt(params.p))
                    user.sendMessageNumber('911', params.p.toString(), `Ваш вызов был принят сотрудником ${user.getFractionName()}`, 's/GTAO/n/gtao38.png')
            }
                ...
                methods.callRemote('server:phone:memberAction', params.memberId);
                phone.showLoad();
            }
            else if (params.name == 'showInvaderStats') {
                methods.callRemote('server:phone:openInvaderStatsList', params.days);
                phone.showLoad();
            }
            else if (params.name == 'fractionVehicleAction') {
                methods.callRemote('server:phone:fractionVehicleAction', params.vehId);
                phone.showLoad();
            }
                ...
                methods.callRemote('server:phone:showCanabisList');
                phone.showLoad();
            }
            else if (params.name == 'showCanabisWarList') {
                methods.callRemote('server:phone:showCanabisWarList');
                phone.showLoad();
            }
        }
        if (menu == 'uvehicle') {
            if (params.name == 'respawn'){
                ...
                methods.callRemote('server:phone:userRespawnById', params.id, params.price);
            ...
            if (params.name == 'getPos')
                methods.callRemote('server:phone:userGetPosById', params.id);
            if (params.name == 'lock')
                methods.callRemote('server:phone:userLockById', params.id);
            if (params.name == 'engine')
                methods.callRemote('server:phone:userEngineById', params.id);
            if (params.name == 'neon')
                methods.callRemote('server:phone:userNeonById', params.id);
        }
        if (menu == 'apps') {
            ...
            if (params.name == 'myHistory') {
                methods.callRemote('server:phone:userHistory', user.getCache('id'));
                phone.showLoad();
            }
            if (params.name == 'myTickets') {
                methods.callRemote('server:phone:userTickets', user.getCache('id'));
                phone.showLoad();
            }
        }
        if (menu == 'gps') {
            ...
            if (params.event)
                methods.callRemote(params.event);
        }
        if (menu == 'fishing') {
            if (params.x)
                user.setWaypoint(params.x, params.y);
        ...
        if (menu == 'invader') {
            if (params.name == 'adList') {
                methods.callRemote('server:phone:userAdList');
                phone.showLoad();
            }
            if (params.name == 'newsList') {
                methods.callRemote('server:phone:userNewsList');
                phone.showLoad();
            }
        }
        if (menu == 'adList') {
        ...
        if (menu == 'bank') {
            if (params.name == 'history') {
                methods.callRemote('server:phone:bankHistory');
                phone.showLoad();
            }
        }
        if (menu == 'userInfo') {
            if (params.name == 'history') {
                methods.callRemote('server:phone:userHistory', params.id);
                phone.showLoad();
            }
            if (params.name == 'tickets') {
                methods.callRemote('server:phone:userTickets', params.id);
                phone.showLoad();
            }
        }
        if (menu == 'ecorp') {
            ...
            if (params.name == 'buyFraction') {
                methods.callRemote('server:phone:buyFraction', params.id);
                phone.showAppList();
            }
        }
        if (menu == 'settings') {
            if (params.name == 'changeBg') {
                ...
                phone.toMainPage();
                methods.callRemote('server:phone:changeBg', params.img);
            }
        }
        if (menu == 'fraction2') {
            if (params.name == 'hierarchy')
                ...
                methods.callRemote('server:phone:fractionVehicles2');
                phone.showLoad();
            }
            else if (params.name == 'fractionVehicleBuyInfo') {
                methods.callRemote('server:phone:fractionVehicleBuyInfo2', params.id);
                phone.showLoad();
            }
            else if (params.name == 'vehicleBuyList') {
                methods.callRemote('server:phone:fractionVehiclesBuyList2');
                phone.showLoad();
            }
            else if (params.name == 'fractionVehicleAction2') {
                methods.callRemote('server:phone:fractionVehicleAction2', params.vehId);
                phone.showLoad();
            }
            else if (params.name == 'fractionVehicleFind2') {
                methods.callRemote('server:fraction:vehicleFind2', params.vehId);
                phone.showAppFraction2();
            }
            else if (params.name == 'respawnVeh') {
                methods.callRemote('server:fraction:vehicleRespawn', params.memberId);
                ...
                methods.callRemote('server:phone:fractionList2');
                phone.showLoad();
            }
            else if (params.name == 'slist') {
                methods.callRemote('server:phone:fractionStList2');
                phone.showLoad();
            }
            else if (params.name == 'log') {
                methods.callRemote('server:phone:fractionLog2');
                phone.showLoad();
            }
            else if (params.name == 'memberAction') {
                methods.callRemote('server:phone:memberAction2', params.memberId);
                phone.showLoad();
            }
            else if (params.name == 'showGangList') {
                methods.callRemote('server:phone:showGangList');
                phone.showLoad();
            }
            else if (params.name == 'showGangWarList') {
                methods.callRemote('server:phone:showGangWarList');
                phone.showLoad();
            }
            else if (params.name == 'showCanabisList') {
                methods.callRemote('server:phone:showCanabisList');
                ...
                methods.callRemote('server:phone:getShopGang');
                phone.showLoad();
            }
            else if (params.name == 'attackStreet') {
                //methods.callRemote('server:phone:attackStreet', params.zone);
                phone.hide();
            ...
            else if (params.name == 'setMafia') {
                /*fraction.set(user.getCache('fraction_id2'), 'is_mafia', 1);
                fraction.removeMoney(user.getCache('fraction_id2'), 1000, 'Улучшение синдиката');
                mp.game.ui.notifications.show(`~g~Вы успешно наладили связи и теперь вам доступны наводки на ограбления магазинов`);
                fraction.save(user.getCache('fraction_id2'));*/
                methods.callRemote('server:fraction:upgradeMafia');
                phone.showLoad();
                setTimeout(phone.showAppFraction2, 500);
            }
                ...
                methods.callRemote('server:fraction:getBankVeh', 1);
                ...
                methods.callRemote('server:fraction:getBankVeh', 2);
                ...
                methods.callRemote('server:fraction:getBankVeh', 3);
                ...
                methods.callRemote('server:fraction:getBankVeh', 99);
                ...
                if (order_drug < 75) {
                    mp.game.ui.notifications.show(`~r~Необходимо положить ${75 - order_drug} закладок, для выполнения контракта`);
                    return;
                }

                if (user.isLeader2() || user.isSubLeader2()) {
                    methods.callRemote('server:fraction:getDrugSpeedo');
                    fraction.set(user.getCache('fraction_id2'), 'order_drug', 9999);
                    phone.showLoad();
                    setTimeout(phone.showAppFraction2, 500);
                }
                else {
                ...
                let order_drug = await fraction.has(user.getCache('fraction_id2'), 'order_drug_marg');
                if (order_drug) {
                    mp.game.ui.notifications.show(`~r~Вы уже получали фургон сегодня`);
                    return;
                }
                ...
                if (weather.getRealHour() >= 6 && weather.getRealHour() < 17) {
                    if (user.isLeader2() || user.isSubLeader2()) {
                        methods.callRemote('server:fraction:getDrugCanabisSpeedo');
                        fraction.set(user.getCache('fraction_id2'), 'order_drug_marg', true);
                        phone.showLoad();
                        setTimeout(phone.showAppFraction2, 500);
                    }
                    else {
               ...
               if (order_lamar < 50) {
                   mp.game.ui.notifications.show(`~r~Необходимо перевести ${50 - order_lamar} грузов, для выполнения контракта`);
                   return;
               }

                if (user.isLeader2() || user.isSubLeader2()) {
                    methods.callRemote('server:fraction:getLamarSpeedo');
                    fraction.set(user.getCache('fraction_id2'), 'order_lamar', 9999);
                    phone.showLoad();
                    setTimeout(phone.showAppFraction2, 500);
                }
                else {
               ...
               let order_lamar_m = await fraction.get(user.getCache('fraction_id2'), 'order_lamar_m');
               if (order_lamar_m >= 9999) {
                   mp.game.ui.notifications.show(`~r~Вы уже получали фургон сегодня`);
                   return;
               }
               if (order_lamar_m < 100) {
                   mp.game.ui.notifications.show(`~r~Необходимо перевести ${100 - order_lamar_m} грузов, для выполнения контракта`);
                   return;
               }

                if (user.isLeader2() || user.isSubLeader2()) {
                    methods.callRemote('server:fraction:getLamarMule');
                    fraction.set(user.getCache('fraction_id2'), 'order_lamar_m', 9999);
                    phone.showLoad();
                    setTimeout(phone.showAppFraction2, 500);
                }
                else {
                ...
                if (order_atm < 5) {
                    mp.game.ui.notifications.show(`~r~Необходимо ограбить ${5 - order_atm} банкоматов, для выполнения контракта`);
                    return;
                }
                ...
                if (user.isLeader2() || user.isSubLeader2()) {
                    ...
                    fraction.set(user.getCache('fraction_id2'), 'order_atm', 9999);
                    phone.showLoad();
                    setTimeout(phone.showAppFraction2, 500);
                }
                else {
        ...
        if (menu == 'family') {
            ...
            if (params.name == 'avhive')
                ...
                methods.callRemote('server:phone:fractionListF');
                phone.showLoad();
            }
            else if (params.name == 'slist') {
                methods.callRemote('server:phone:fractionStListF');
                phone.showLoad();
            }
            else if (params.name == 'log') {
                methods.callRemote('server:phone:fractionLogF');
                phone.showLoad();
            }
            else if (params.name == 'memberActionF') {
                methods.callRemote('server:phone:memberActionF', params.memberId);
                phone.showLoad();
            }
            else if (params.name == 'uninviteMe') {
    ...
    try {
        ...
        if (phone.network > 0 && phone.getType() > 0) {
            notifyList.forEach(item => {
                if (item.pic === "CHAR_CALL911")
                    mp.game.ui.notifications.showWithPicture(item.sender.toString(), item.title.toString(), item.message.toString(), item.pic, 1, false, -1, 6);
                else
                    mp.game.ui.notifications.showWithPicture(item.sender.toString(), item.title.toString(), item.message.toString(), item.pic, 1);
            });
            notifyList = [];
        }
    }
