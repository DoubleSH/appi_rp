...
wheel.userRoll = async function() {
    try {
        ...
        methods.callRemote("server:casino:wheel:doRoll");
    } catch (e) {
...
new_events.add("client:casino:wheel:start", async () => {
    wheel.userRoll();
});

new_events.add("client:casino:wheel:doRoll", async (priceIndex, playerId) => {
    try {
        ...
        if (mp.players.local.remoteId === playerId)
            methods.callRemote('server:casino:wheel:finalRoll');
    }
    catch (e) {
