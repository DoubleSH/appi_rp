...
new_events.add('client:chat:sendMessage', function(message) {
    chat.sendLocal(message);
});

new_events.add('client:chatTyping', function(state) {
...
chat.sendBCommand = function(text) {
    methods.callRemote('server:chat:sendBCommand', text);
};

chat.sendTryCommand = function(text) {
    methods.callRemote('server:chat:sendTryCommand', text);
};

chat.sendDoCommand = function(text) {
    methods.callRemote('server:chat:sendDoCommand', text);
};

chat.sendMeCommand = function(text) {
    methods.callRemote('server:chat:sendMeCommand', text);
};

chat.sendDiceCommand = function() {
    methods.callRemote('server:chat:sendDiceCommand');
};

chat.send = function(text) {
    methods.callRemote('server:chat:send', text);
};

chat.sendToAll = function(sender, text, color = '2196F3') {
    methods.callRemote('server:chat:sendToAll', sender, text, color);
};

chat.sendToDep = function(sender, text, color = undefined) {
    methods.callRemote('server:chat:sendToDep', sender, text, color);
};

chat.sendToFraction = function(sender, text, color = undefined) {
    methods.callRemote('server:chat:sendToFraction', sender, text, color);
};

chat.sendToFamily = function(sender, text, color = undefined) {
    methods.callRemote('server:chat:sendToFamily', sender, text, color);
};
