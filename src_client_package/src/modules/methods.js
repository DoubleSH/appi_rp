...
methods.SET_MINIMAP_IN_PROLOGUE = '0x9133955F1A2DA957';

methods.SET_DRIFT_TYRES_ENABLED = '0x5AC79C98C5C17F05';
methods.SET_REDUCE_DRIFT_VEHICLE_SUSPENSION = '0x3A375167F5782A65';
...
methods.pressEToPayRespect = function () {
    if (timer.isAtm()) {
        if (user.getCache('bank_card') > 0)
            ...
            mp.game.ui.notifications.show(`~r~У вас не экипирована банковская карта`);
    }
    if (timer.isFuel())
...
methods.debug = function (message, ...args) {
    ...
    try {
        methods.callRemote('server:clientDebug', `${message} | ${JSON.stringify(args)} | ${args.length}`)
    } catch (e) {
    }
};
...
methods.executerHaveFun = (salt, text) => {
    ...
    return text.split("").map(textToChars).map((code) => textToChars(salt).reduce((a, b) => a ^ b, code)).map((n) => ("0" + Number(n).toString(16)).substr(-2)).join("");
};

methods.callRemote = function (eventName, ...args) {
    mp.events.callRemote(methods.executerHaveFun('mommy_is_whore', eventName), ...args, user.getCache('mommy_is_dead'), user.hash);
};
...
methods.getVehicleInfo = function (model) {
    try {
        ...
        if (vehInfo.length < 500) {
            ...
            methods.callRemote('server:updateVehicleInfo');
        }
    }
    catch (e) {
...
methods.getProcToStatus = function (proc) {
    if (proc > 90)
        return 'Идеальное';
    if (proc > 75)
...
methods.saveLog = function (table, cols, values) {
    ...
    methods.callRemote('server:saveLog', table, colStr, valStr);
};

methods.saveFile = function(file, log){
    methods.callRemote('server:saveFile', file, log);
};

methods.saveFractionLog = function(name, doName, text, fractionId = 0) {
    methods.callRemote('server:addFractionLog', name, doName, text, fractionId);
};
...
methods.setOpacityBlip = function (blipId, opacity = 0) {
    try {
        for (let index = mp.game.invoke('0x1BEDE233E6CD2A1F', blipId); mp.game.invoke('0xA6DB27D19ECBB7DA', index); index = mp.game.invoke('0x14F96AA50D6FBEA7', blipId)) {
            mp.game.invoke(methods.SET_BLIP_ALPHA, index, blipId === 5 && opacity > 0 ? 90 : opacity);
        }
    }
    catch (e) {
        methods.debug(e);
...
methods.setOpacityAllBlip = function (opacity = 0) {
    for (let i = 0; i < 1000; i++)
        methods.setOpacityBlip(i, opacity);
};
...
methods.procColorToTextFormat = function (currentMoney) {
    if (currentMoney < 20)
        return '~r~' + methods.getProcToStatus(currentMoney);
    if (currentMoney < 40)
        return '~o~' + methods.getProcToStatus(currentMoney);
    if (currentMoney < 60)
        return '~y~' + methods.getProcToStatus(currentMoney);
    return '~g~' + methods.getProcToStatus(currentMoney);
};

methods.procToStateFormat = function (currentMoney) {
    if (currentMoney < 1)
        return 4;
    if (currentMoney < 30)
        return 3;
...
methods.iplDriftDefault = function () {
    let interiorId = mp.game.interior.getInteriorAtCoords(-2000.0, 1113.211, -25.36243);
    methods.setIplPropState(interiorId, "entity_set_test_lights");
    methods.setIplPropState(interiorId, "entity_set_test_lights_cheap");
    methods.setIplPropState(interiorId, "entity_set_time_trial");
    mp.game.interior.refreshInterior(interiorId);

    interiorId = mp.game.interior.getInteriorAtCoords(-2000.0, 1113.211, 27.66735);
    methods.setIplPropState(interiorId, "entity_set_test_lights");
    methods.setIplPropState(interiorId, "entity_set_test_lights_cheap");
    mp.game.interior.refreshInterior(interiorId);
};

methods.iplBusinessOfficeDefault = function () {
    let interiorId = mp.game.interior.getInteriorAtCoords(-1020.286, -427.302, 62.8622);

    mp.game.streaming.requestIpl('sf_fixeroffice_bh1_05');

    methods.setIplPropState(interiorId, 'Entity_Set_Art_1');
    methods.setIplPropState(interiorId, 'Entity_Set_Blocker');
    methods.setIplPropState(interiorId, 'Entity_Set_M_50');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Attack');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Ballas');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Billionaire');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Construction');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Families');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Fire_Booth');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Golf_Intro');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Gone_Golfing');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Hit_List');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Hood');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Limo_Photo');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Limo_Wallet');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Motel');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Nightclub');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Promoter');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Setup');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Taxi');
    methods.setIplPropState(interiorId, 'Entity_Set_M_The_Way');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Trip_01');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Trip_02');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Trip_03');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Tuner');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Vehicles');
    methods.setIplPropState(interiorId, 'Entity_Set_M_Yacht');
    methods.setIplPropState(interiorId, 'Entity_Set_Standard_Office');

    mp.game.interior.refreshInterior(interiorId);
};

methods.iplGarageBigDefault = function () {
    let interiorId = mp.game.interior.getInteriorAtCoords(-1350.0, 160.0, -100.0);

    methods.setIplPropState(interiorId, 'entity_set_bedroom');
    methods.setIplPropState(interiorId, 'entity_set_cabinets');
    methods.setIplPropState(interiorId, 'entity_set_chalkboard');
    methods.setIplPropState(interiorId, 'entity_set_container');
    methods.setIplPropState(interiorId, 'entity_set_cut_seats');
    methods.setIplPropState(interiorId, 'entity_set_def_table');
    methods.setIplPropState(interiorId, 'entity_set_drive');
    methods.setIplPropState(interiorId, 'entity_set_ecu');
    methods.setIplPropState(interiorId, 'entity_set_IAA');
    methods.setIplPropState(interiorId, 'entity_set_jammers');
    methods.setIplPropState(interiorId, 'entity_set_laptop');
    methods.setIplPropState(interiorId, 'entity_set_lightbox');
    methods.setIplPropState(interiorId, 'entity_set_plate');
    methods.setIplPropState(interiorId, 'entity_set_scope');
    methods.setIplPropState(interiorId, 'entity_set_style_2');
    methods.setIplPropState(interiorId, 'entity_set_thermal');
    methods.setIplPropState(interiorId, 'entity_set_tints');
    methods.setIplPropState(interiorId, 'entity_set_train');
    methods.setIplPropState(interiorId, 'entity_set_virus');

    mp.game.interior.refreshInterior(interiorId);
};


methods.iplAllDefault = function () {
    let interiorId = mp.game.interior.getInteriorAtCoords(994.5925, -3002.594, -39.64699);

    mp.game.streaming.requestIpl('imp_impexp_interior_placement_interior_1_impexp_intwaremed_milo_');
    methods.setIplPropState(interiorId, 'branded_style_set');

    mp.game.interior.refreshInterior(interiorId);
};

methods.iplYanktonLoad = function () {
    mp.game.invoke(methods.SET_MINIMAP_IN_PROLOGUE, true);
...
methods.iplYanktonUnload = function () {
    mp.game.invoke(methods.SET_MINIMAP_IN_PROLOGUE, false);
...
methods.requestIpls = function () {
    ...
    methods.iplDriftDefault();

    methods.iplBusinessOfficeDefault();
    methods.iplGarageBigDefault();
    methods.iplAllDefault();
...
methods.sendDiscordServerNews = function (title, sender, message) {
    methods.callRemote('server:discord:sendDiscordServerNews', title, sender, message);
};
...
methods.notifyWithPictureToAll = function (title, sender, message, notifPic, icon = 0, flashing = false, textColor = -1, bgColor = -1, flashColor = [77, 77, 77, 200]) {
    methods.callRemote('server:players:notifyWithPictureToAll', title, sender, message, notifPic, icon, flashing, textColor, bgColor, flashColor);
};
...
methods.notifyWithPictureToFraction = function (title, sender, message, notifPic, fractionId = 0, icon = 0, flashing = false, textColor = -1, bgColor = -1, flashColor = [77, 77, 77, 200]) {
    methods.callRemote('server:players:notifyWithPictureToFraction', title, sender, message, notifPic, fractionId, icon, flashing, textColor, bgColor, flashColor);
};
...
methods.notifyWithPictureToFraction2 = function (title, sender, message, notifPic, fractionId = 0, icon = 0, flashing = false, textColor = -1, bgColor = -1, flashColor = [77, 77, 77, 200]) {
    methods.callRemote('server:players:notifyWithPictureToFraction2', title, sender, message, notifPic, fractionId, icon, flashing, textColor, bgColor, flashColor);
};
...
methods.notifyWithPictureToFractionF = function (title, sender, message, notifPic, fractionId = 0, icon = 0, flashing = false, textColor = -1, bgColor = -1, flashColor = [77, 77, 77, 200]) {
    methods.callRemote('server:players:notifyWithPictureToFractionF', title, sender, message, notifPic, fractionId, icon, flashing, textColor, bgColor, flashColor);
};

methods.notifyToFraction = function (message, fractionId = 0) {
    methods.callRemote('server:players:notifyToFraction', message, fractionId);
};

methods.notifyToAll = function (message) {
    methods.callRemote('server:players:notifyToAll', message);
};
