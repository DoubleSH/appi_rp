...
new_events.add('client:modalinput:callBack', (data) => {
    _isShowInput = false;
    chat.activate(true);
...
new_events.add("client:menuList:callBack:btn", async (menuName, id, jparams) => {
    try {
...
new_events.add("client:menuList:onClose", async () => {
    try {
        Menu.OnClose.Emit();
...
new_events.add("client:menuList:callBack:check", async (menuName, id, jparams, checked) => {
    try {
...
new_events.add("client:menuList:callBack:list", async (menuName, id, jparams, index) => {
    try {
...
new_events.add("client:menuList:callBack:select", async (menuName, idx) => {
    try {
        if (user.getCache('s_menu_sound'))
            mp.game.audio.playSoundFrontend(-1, "HIGHLIGHT_NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
