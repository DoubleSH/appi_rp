...
let isBlackHud = false;

let maxStringLength = 50;

new_events.add('guiReady', () => {
    user.showLoadDisplay(1);
    new_events.add('browserDomReady', (browser) => {
        if (browser === uiBrowser) {
            ...
            try {
                if (mp.storage.data.name) {
                    ...
                    setTimeout(function () {
                        //methods.callRemote('server:user:loginAccount', mp.storage.data.name);
...
ui.showPosition = function(className) {
    try {
        let sPos = JSON.parse(user.getCache('s_pos'));
        let idx = sPos.map(e => { return e[0]; }).indexOf(className);
        if (idx >= 0) {
            sPos.splice(idx, 1);
            user.set('s_pos', JSON.stringify(sPos));
            ui.updatePositionSettings();
        }
    }
    catch (e) {}
...
ui.hidePosition = function(className) {
    try {
        let sPos = JSON.parse(user.getCache('s_pos'));
        let idx = sPos.map(e => { return e[0]; }).indexOf(className);
        if (idx >= 0)
            sPos.splice(idx, 1);
        sPos.push([className, -999, -999]);
        user.set('s_pos', JSON.stringify(sPos));
        ui.updatePositionSettings();
    }
    catch (e) {}
...
ui.isShowPosition = function(className) {
    try {
        let sPos = JSON.parse(user.getCache('s_pos'));
        let idx = sPos.map(e => { return e[0]; }).indexOf(className);
        if (sPos[idx][1] === -999)
...
ui.isHudEdit = function() {
    return canEdit;
};

ui.disableHudEdit = function() {
    canEdit = false;
    ui.callCef('hud','{"type": "hideEdit"}');
    mp.game.ui.notifications.show('~y~Вы закончили редактирование интерфейса');
};

ui.enableHudEdit = function() {
    canEdit = true;
...
ui.updateValues = function() {
    ...
    if (user.isLogin()) {
        try {
            ...
            isGreenZone = false;
            user.setCache('isGreenZone', false)
            enums.zoneGreenList.forEach(item => {
                try {
                    if (methods.isInPoint(mp.players.local.position, item[2]) && mp.players.local.dimension === 0)
                    {
                        if (mp.players.local.position.z > item[0] && mp.players.local.position.z < item[1]){
                            isGreenZone = true;
                            user.setCache('isGreenZone', true)
                        }
            ...
            if (user.getCache('s_hud_bgс') === 0)
                isBlackHud = weather.getHour() >= 6 && weather.getHour() < 22;
            else if (user.getCache('s_hud_bgс') === 1)
                isBlackHud = false;
            else
                isBlackHud = true;
            ...
            let data = {
                ...
                isDarkBackground: isBlackHud,
                isTextStroke: user.getCache('s_hud_shadow') === 1,
            };
            ...
            if (mp.players.local.getVariable('enableAdmin'))
                dateLabel = 'ADMIN MOD · ' + dateLabel;
            if (mp.players.local.getVariable('isAfk'))
                dateLabel = 'AFK · ' + dateLabel;
            ...
            let ammoClip = methods.parseInt(mp.players.local.getAmmoInClip(user.getCurrentWeapon()));
            data = {
                type: 'updateValues',
                ...
                ammoCount: `${ammoClip} / ${user.getCurrentAmmo()}`,
                background: user.getCache('s_hud_bg'),
                isDarkBackground: isBlackHud,
                isAnimatedLogo: user.getCache('s_hud_logo') === 1,
                isTextStroke: user.getCache('s_hud_shadow') === 1,
            };
            ...
            if (user.getCache('quests') && user.getCache('s_hud_quest')) {
                ...
                if (user.getCache('startProlog') < 10)
                {
                    ...
                    data = {
                        type: 'updateQuest',
                        showQuest: !menuList.isShowPlayerMenu,
                        questTitle: 'Пролог',
                ...
                else if (user.getQuestCount('standart') < 10)
                {
                    ...
                    data = {
                        type: 'updateQuest',
                        showQuest: !menuList.isShowPlayerMenu,
                        questTitle: 'Ознакомительное задание',
                        questText: currentTask,
                        questDesc: questDesc,
                    };
        ...
        try {
            ...
            if (veh && mp.vehicles.exists(veh)) {
                ...
                if (vPrevInfo.fuel_type > 0) {
                    fuelLevel = methods.parseInt(vehicles.getFuel(veh));
...
new_events.add('client:ui:callCef', (event, value) => {
...
const handleRender = () => {
    if (!mp.game.ui.isPauseMenuActive())
        return;
    try {
        const firstLineText = user.getCache('name').toString();	// 'First line'
        const secondLineText = methods.moneyFormat(user.getCashMoney()); // 'Second line'
        const thirdLineText = methods.moneyFormat(user.getBankMoney()); // 'Third line'
...
new_events.add('render', handleRender);
