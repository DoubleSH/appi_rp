...
trucker.doneOffer = function() {
    ...
    setTimeout(function () {
        methods.callRemote('server:trucker:doneOffer', offerId);
    }, 4000);
...
trucker.stop = function() {
    ...
    setTimeout(function () {
        methods.callRemote('server:trucker:stop', offerId);
    }, 4000);
...
trucker.timer = function() {
    ...
    if (type == 2 && isProcess == false && methods.distanceToPos(new mp.Vector3(fromPosX, fromPosY, fromPosZ), mp.players.local.position) < 50) {
        methods.callRemote('server:trucker:trySpawnTrailer', offerId);
    }
};

new_events.add("client:jobs:trucker:isSpawn", (cbSpawn) => {
    if (!cbSpawn) {
