...
gr6.unload = function() {
    ...
    methods.callRemote('server:gr6:unload');
};
...
gr6.deleteVeh = function() {
    ...
    methods.callRemote('server:gr6:delete');
    jobPoint.delete();
};
...
gr6.grab = function() {
    ...
    gr6.grabMarkers.forEach(function (item) {
        if (methods.distanceToPos(new mp.Vector3(item[0], item[1], item[2]), mp.players.local.position) < 20) {
            methods.callRemote('server:gr6:grab');
            if(mp.players.local.getDrawableVariation(1) < 1){
...
gr6.findRandomPickup = function() {
    if (!isProcess) {
        ...
        methods.callRemote('server:gr6:findPickup', gr6.markers[pickupId][0], gr6.markers[pickupId][1], gr6.markers[pickupId][2]);
    } else {
...
gr6.workProcess = function() {
    jobPoint.delete();
    Container.Data.SetLocally(0, 'gr6Money', price + methods.getRandomInt(100, 200));
...
new_events.add("client:createGr6Checkpoint", (x, y, z) => {
    ...
    price = methods.parseFloat(methods.distanceToPos(pos, mp.players.local.position) / 2);
    if (price > 2200)
        price = 2200;
...
new_events.add("playerEnterVehicle", async function (vehicle, seat) {
    methods.debug('playerEnterVehicle3124231244')
    try {
        ...
        if (vehicle.getVariable('job') == 10) {
            if (Container.Data.HasLocally(0, 'gr6Money') && Container.Data.HasLocally(0, 'gr6MoneyBag')) {
                ...
                methods.callRemote('server:gr6:dropCar', money * 100, vehicle.remoteId);
                if (user.getCache('hand') > 0)
