        ...
        if (isStart) {
            ...
            if (posStart.x != 0 && methods.distanceToPos(posStart, mp.players.local.position) < 50) {
                ...
                setTimeout(function () {
                    ...
                    //mp.events.call('server:taxi:createGlobalPedInVehicle', taxi.peds[modelId], posStart.x, posStart.y, posStart.z);
...
new_events.add('client:taxi:createGlobalPedInVehicle', (id, model, vehicleId) => {
...
new_events.add('client:deleteGlobalPed', (id) => {
    pedList.forEach((item) => {
        if (item.pedId == id) {
            if (mp.peds.exists(item.ped))
                item.ped.destroy();
