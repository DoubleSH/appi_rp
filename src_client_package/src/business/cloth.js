...
cloth.buy = function(price, body, cloth, color, torso, torsoColor, parachute, parachuteColor, itemName = "Одежда", shopId = 0, isFree = false, payType = 0){
    ...
    methods.callRemote('server:business:cloth:buy', price, body, cloth, color, torso, torsoColor, parachute, parachuteColor, itemName, shopId, isFree, payType);
};

cloth.changeMask = function(cloth, color){
    checkInit();
    methods.callRemote('server:business:cloth:changeMask', cloth, color);
};

cloth.buyMask = function(price, maskId, shopId = 0, payType = 0) {
    checkInit();
    methods.callRemote('server:business:cloth:buyMask', price, maskId, shopId, payType);
};

cloth.change = function(body, cloth, color, torso, torsoColor, parachute, parachuteColor) {
    checkInit();
    methods.callRemote('server:business:cloth:change', body, cloth, color, torso, torsoColor, parachute, parachuteColor);
};

cloth.buyProp = function(price, body, cloth, color, itemName = "Аксессуар", shopId = 0, isFree = false, payType = 0){
    checkInit();
    methods.callRemote('server:business:cloth:buyProp', price, body, cloth, color, itemName, shopId, isFree, payType);
};

cloth.changeProp = function(body, cloth, color){
    checkInit();
    methods.callRemote('server:business:cloth:changeProp', body, cloth, color);
};
