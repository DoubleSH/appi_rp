...
new_events.add("client:raceUpdate", (json) => {
    try {
        racer.currentCpDestroy();
        racer.nextCpDestroy();
    }
    ...
    try {
        ...
        mp.game.cam.animateGameplayCamZoom(1, 1000);
        ...
        setTimeout(function () {
            ...
            try {
                mp.players.local.vehicle.setHandling('fTractionLossMult', 1);
            }
            catch (e) {}
        ...
        inRace = false;
        methods.callRemote('server:race:exit');
...
new_events.add("playerEnterCheckpoint", (checkpoint) => {
    ...
    try {
        ...
        if (currentCpId === currentRace.posList.length - 1) {
            ...
            racer.currentCpDestroy();
            methods.callRemote('server:race:finish');
            inRace = false;
...
new_events.add("playerDeath", function (player, reason, killer) {
    methods.debug('playerDeath47245615235')
    ...
    try {
        ...
        user.respawn(pos.x, pos.y, pos.z)
    }
    catch (e) {
