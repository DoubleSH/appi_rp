...
bind.allowKeyList = [
    ...
    },
    {
        ...
        keys: [
            ...
            ['Надеть/снять наручники', 's_bind_cuff'],
            ['Надеть/снять стяжки', 's_bind_tie'],
        ]
    },
    {
        name: 'Транспорт',
        keys: [
            ...
            ['Гидравлический тормоз', 's_bind_break'],
        ]
    },
    {
        name: 'Чат',
    ...
    mp.keys.bind(parseInt(code), true, async function() {
        ...
        if (user.getCache('s_bind_do') == parseInt(code)) {
            ...
            if (!methods.isBlockKeys()) {
                ...
                if (targetEntity &&  targetEntity.isNpc) {
                    if (targetEntity.keyQuest === 'standart') {
                        if (ui.isIslandZone())
                            menuList.showBotQuestRoleAll1Menu();
                        else
                            menuList.showBotQuestRoleAllMenu();
                    }
                    else if (targetEntity.keyQuest === 'role_0')
                        menuList.showBotQuestRoleAll1Menu();
                    else if (targetEntity.keyQuest === 'gang')
                        menuList.showBotQuestGangMenu();
                    else if (targetEntity.keyQuest === 'driftking')
                        menuList.showBotQuestDkMenu();
                    else if (targetEntity.keyQuest === 'detective')
                        menuList.showBotQuestDetectiveMenu();
                    else if (targetEntity.keyQuest === 'detectiveHusband')
                        menuList.showBotQuestDetectiveHusbandMenu();
                    else if (targetEntity.keyQuest === 'driftking_ded')
                        menuList.showBotQuestDkDedMenu();
                    else if (targetEntity.keyQuest === 'driftking_auto')
                        menuList.showBotQuestDkAutoMenu();
                }
        ...
        if (user.getCache('s_bind_inv') == parseInt(code)) {
            ...
            if(mp.players.local.getVariable('gangzone')){
                mp.game.ui.notifications.show("~r~Нельзя пользоваться инвентарем, на GangZone");
                return;
            }
            if (!methods.isBlockKeys() && phone.isHide() && mainMenu.isHide()) {
                ...
                Container.Data.SetLocally(mp.players.local.remoteId, "isInventoryTimeout1", true);
                ui.callCef('inventory', '{"type": "showOrHide"}');
                setTimeout(function () {
                    Container.Data.ResetLocally(mp.players.local.remoteId, "isInventoryTimeout1");
                }, 2000);
            }
        }
        if (user.getCache('s_bind_inv_world') == parseInt(code)) {
            ...
            if(!inventory.isHide()){
                mp.game.ui.notifications.show("~r~Чтобы открыть 'Предметы рядом',  нужно закрыть инвентарь.");
                return;
            }
            if (!methods.isBlockKeys() && phone.isHide() && mainMenu.isHide()) {
                ...
                if (ui.isGreenZone() || mp.players.local.dimension > 0 || mp.players.local.isInAnyVehicle(true)) {
                    ...
                    Container.Data.SetLocally(mp.players.local.remoteId, "isInventoryTimeout1", true);
                    ui.callCef('inventory', '{"type": "showOrHide"}');
                    mp.game.ui.notifications.show("~r~Предметы на земле не доступны в зеленых зонах, интерьерах и машине");
                    setTimeout(function () {
                        Container.Data.ResetLocally(mp.players.local.remoteId, "isInventoryTimeout1");
                    }, 2000);
        ...
        if (user.getCache('s_bind_stopanim') == parseInt(code)) {
            if(mp.players.local.isReloading() || mp.players.local.isShooting() || mp.game.player.isFreeAiming()){
                return;
            }
            if (!methods.isBlockKeys())
        ...
        if (user.getCache('s_bind_veh_menu') == parseInt(code)) {
            if (!methods.isBlockKeys())
                methods.callRemote('server:showVehMenu');
        }
        if (user.getCache('s_bind_cuff') == parseInt(code)) {
            let target = methods.getNearestPlayerWithCoords(mp.players.local.position, 4)
        ...
        if (user.getCache('s_bind_break') == parseInt(code)) {
            if (!methods.isBlockKeys() && mp.players.local.vehicle) {
                if (mp.players.local.vehicle.getVariable('sbreak') || mp.players.local.vehicle.getVariable('prolog')) {
                    mp.players.local.vehicle.setReduceGrip(true);
                    vehicles.reduceGrip = true;
                }
                //mp.game.invoke('0x5AC79C98C5C17F05', mp.players.local.vehicle.handle, true)
            }
        }
    ...
    mp.keys.bind(parseInt(code), false, function() {
        ...
        if (user.getCache('s_bind_break') == parseInt(code)) {
            if (!methods.isBlockKeys() && mp.players.local.vehicle) {
                ...
                //mp.game.invoke('0x5AC79C98C5C17F05', mp.players.local.vehicle.handle, false)
            }
        }
    });
}
