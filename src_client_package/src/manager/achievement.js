...
achievement.checkerAll = function() {
    try {
        ...
        if (moneyLvl >= 999999 && data[29] !== 1000000) {
            data[29] = 1000000;
            user.set('achiv', JSON.stringify(data));
            user.addCashMoney(5000, 'Выполнено достижение: ' + achievList[29].title);
...
achievement.doneAllById = function(id) {
    try {
        ...
        methods.error('achievement.doneAllById', e.toString())
    }
};
...
achievement.doneDailyById = async function(id) {
    try {
        ...
        methods.error('achievement.doneDailyById', e.toString())
    }
};
