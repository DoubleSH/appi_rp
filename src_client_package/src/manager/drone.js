...
drone.enter = function() {
    isInDrone = true;
    methods.callRemote('server:startSpecMission');
};

drone.exit = function() {
    isInDrone = false;
    vision_state = 0;
    methods.callRemote('server:stopSpecMission');
...
drone.enterLspd = function(vId) {
    ...
    methods.callRemote('server:startSpecMissionLspd', vId);
};
...
drone.exitLspd = function() {
    ...
    methods.callRemote('server:stopSpecMissionLspd');
    user.stopAllScreenEffect();
};

drone.enterSmall = function(vId) {
    achievement.doneAllById(6);
    methods.callRemote('server:startSpecMissionSmall', vId);
};
...
new_events.add('client:drone:status', (status) => {
    isInDrone = status;
    isLspd = status;
...
new_events.add('playerLeaveVehicle', () => {
    if (!isInDrone)
        return;
    isInDrone = false;
    isLspd = false;
