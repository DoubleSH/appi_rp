...
dispatcher.sendPos = function (title, desc, pos, withCoord = true, phone = 0) {
    methods.callRemote("server:dispatcher:sendPos", title, desc, pos.x, pos.y, pos.z, withCoord, phone.toString());
};
...
dispatcher.sendTaxiPos = function (title, desc, pos, wpPos, price, phone = 0) {
    methods.callRemote("server:dispatcher:sendTaxiPos", title, desc, pos.x, pos.y, pos.z, wpPos.x, wpPos.y, wpPos.z, price, phone.toString());
};
...
dispatcher.sendMechPos = function (title, desc, pos, phone = 0) {
    methods.callRemote("server:dispatcher:sendMechPos", title, desc, pos.x, pos.y, pos.z, phone.toString());
};
...
dispatcher.sendLocalPos = function (title, desc, pos, fractionId = 0, withCoord = true) {
    if (fractionId === 0)
        return;
    methods.callRemote("server:dispatcher:sendLocalPos", title, desc, pos.x, pos.y, pos.z, fractionId, withCoord);
};
...
dispatcher.getTaxiMenu = function () {
    methods.callRemote("server:dispatcher:getTaxiMenu");
};

dispatcher.getMechMenu = function () {
    methods.callRemote("server:dispatcher:getMechMenu");
};
