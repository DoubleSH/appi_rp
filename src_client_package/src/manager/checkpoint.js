    ...
    try {
        if (user.isLogin()) {
            ...
            itemList.forEach((item, idx) => {
                try {
                    /*if (methods.distanceToPos(playerPos, new mp.Vector3(item.item.x, item.item.y, item.item.z)) <= item.item.scale + 0.4) {
                        if (!entityList.has(idx)) {
                            entityList.set(idx, true);
                            methods.callRemote('client:enterStaticCheckpoint', item.item.id);
                        }
                    }
                    else {
                        if (entityList.has(idx)) {
                            ...
                            methods.callRemote('client:exitStaticCheckpoint', item.item.id);
                        }
                    }*/
                    if (methods.distanceToPos(playerPos, new mp.Vector3(item.x, item.y, item.z)) <= item.scale + 0.4) {
                        if (!entityList.has(idx)) {
                            entityList.set(idx, true);
                            methods.callRemote('client:enterStaticCheckpoint', item.id);
                        }
                    }
                    else {
                        if (entityList.has(idx)) {
                            entityList.delete(idx);
                            UIMenu.Menu.HideMenu();
                            methods.callRemote('client:exitStaticCheckpoint', item.id);
                        }
                    }
                } catch (e) {
...
checkpoint.fixCheckpointList = function() {
    try {
        ...
        itemList = [];
        methods.callRemote('server:fixCheckpointList');
        //methods.callRemote('server:updateGangZoneList');
    } catch (e) {
