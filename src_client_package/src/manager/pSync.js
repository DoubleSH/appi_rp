    ...
    try {
        if (entity.type === 'player' && mp.players.exists(entity)) {
            ...
            if (remotePlayer.handle) {
                ...
                try {
                    let val = JSON.parse(entity.getVariable('info'));
...
new_events.add('client:syncComponentVariation', (playerId, component, drawableId, textureId) => {
    try {
        let remotePlayer = mp.players.atRemoteId(playerId);
        if (remotePlayer && remotePlayer.handle) {
...
new_events.add('client:syncTaskEnter', (playerId, vehRemoteId, seat) => {
    setTimeout(function () {
        try {
            let remotePlayer = mp.players.atRemoteId(playerId);
            let vehicle = mp.vehicles.atRemoteId(vehRemoteId);
            if (remotePlayer && remotePlayer.handle && vehicle && mp.vehicles.exists(vehicle)) {
...
new_events.add('client:pSync:alpha', (playerId, alpha) => {
    try {
        let remotePlayer = mp.players.atRemoteId(playerId);
        if (remotePlayer && remotePlayer.handle) {
...
new_events.add('client:pSync:shoot', (playerId) => {
    try {
        let remotePlayer = mp.players.atRemoteId(playerId);
        if (remotePlayer && remotePlayer.handle) {
...
new_events.add("entityStreamIn", (entity) => {
    if (!user.isLogin() || !mp.players.exists(entity))
        return;
...
new_events.add('client:syncAnimation', async (playerId, dict, anim, flag) => {
...
new_events.add('client:syncStopAnimation', (playerId) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
...
new_events.add('client:syncStopAnimationNow', (playerId) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
...
new_events.add('client:syncRagdoll', (playerId, timeout) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
        methods.debug('client:syncStopAnimation', playerId);
        let remotePlayer = mp.players.atRemoteId(playerId);
...
new_events.add('client:syncScenario', (playerId, name) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
...
new_events.add('client:syncStopScenario', (playerId) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
        methods.debug('Execute: events:client:syncScenario');
        let remotePlayer = mp.players.atRemoteId(playerId);
...
new_events.add('client:syncHeadingToCoord', (playerId, x, y, z) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
        let remotePlayer = mp.players.atRemoteId(playerId);
        if (remotePlayer && remotePlayer.handle) {
...
new_events.add('client:syncHeading', (playerId, rot) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
        let remotePlayer = mp.players.atRemoteId(playerId);
        if (remotePlayer && remotePlayer.handle) {
...
new_events.add('client:syncHeadingToTarget', (playerId, targetId) => {
    //if (mp.players.local.remoteId == playerId || mp.players.local.id == playerId)
    try {
        let remotePlayer = mp.players.atRemoteId(playerId);
        let targetPlayer = mp.players.atRemoteId(targetId);
        if (remotePlayer && remotePlayer.handle&& targetPlayer && mp.players.exists(targetPlayer)) {
        ...
        if (this.active) {
            try {
                ...
                if ((Date.now() - this.lastSent) > 10) {
                    ...
                    methods.callRemote("server:pSync:fpUpdate", camPitch, camHeading);
                }
            }
            catch (e) {
                methods.debug(e);
...
new_events.add("client:pSync:fpUpdate", async (id, camPitch, camHeading) => {
    let netPlayer = getPlayerByRemoteId(parseInt(id));
    if (netPlayer != null) {
        if (netPlayer != mp.players.local && netPlayer.handle) {
            try {
...
methods.callRemote('server:playAnimation', "cellphone@female", "cellphone_call_listen_base", 49);
* */