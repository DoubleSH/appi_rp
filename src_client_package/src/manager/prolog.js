...
prolog.start = function() {
    ...
    let vehName = allowVehicles[methods.getRandomInt(0, allowVehicles.length)];
    ...
    user.set('prolog_vehicle', vehName);

    methods.callRemote('server:prolog:spawnVehicle', vehName);
...
prolog.stop = function() {
    ...
    setTimeout(function () {
        ...
        methods.callRemote('server:prolog:deleteVehicle');
        methods.setOpacityAllBlip(255);
        ...
        setTimeout(function () {
            ...
            user.sendMessageNumber('Фрэнк', user.getCache('phone'), `Это, кстати, твоя тачка будет в аропорту, в ангаре BILGECO, езжай туда и забери ее`, 's/MaxPayne3/n/MP3_24.png');
...
prolog.timer = function() {
    ...
    pickupsPool.forEach(p => {
        ...
        try {
            if (p.handle && !mp.game.object.doesPickupExist(p.handle)) //Проверить сущестоввание
            {
                if (p.fuel) {
...
new_events.add("playerEnterVehicle", function (vehicle, seat) {
    methods.debug('playerEnterVehicle31242312441243')
    ...
    if(isStart && currentTask === 8) {
        currentTask++;
        ui.showDialog('Двигайтесь в аэропорт по маршуту, соблюдая скоростной режим, потому что у полиции есть скоростные радары. И да, не забудьте пристегнуть ремень на клавишу X. На кнопку SHIFT включается гидравлический ручной тормоз.', 'Информация');
        _checkpointId = jobPoint.create(new mp.Vector3(road1[0][0], road1[0][1], road1[0][2]), true, 5);

        user.sendMessageNumber('Фрэнк', user.getCache('phone'), `Привет, так по поводу работы все готово, по прилёту дуй в здание правительства и устроишься на садовника или строителя, зарабатывать первые деньги и добиваться успеха. Я в тебя верю!`, 's/MaxPayne3/n/MP3_24.png');
    }
});

new_events.add("playerDeath", function () {
    methods.debug('playerDeath472456')
    if(isStart) {
        user.setVirtualWorld(0);
        user.reset('startProlog');
        isStart = false;
