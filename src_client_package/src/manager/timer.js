...
timer.setDeathTimer = function (sec) {
    ...
    if (sec > 0) {
        let dataSend = {
            type: 'updateValues',
            timer: `${sec}`,
            killer: `${user.deathscreen}`,
        };
        ui.callCef('deathscreen', JSON.stringify(dataSend));
    }
    else {
        user.deathscreen = 'Смэрть'
        mp.gui.cursor.show(false, false);
        ui.callCef('deathscreen', '{"type": "hide"}');
...
timer.updateTempLevel = function () {
    ...
    if (weather.getWeatherTemp() < 0 && user.getCache('torso') == 15) {
        mp.game.ui.notifications.show("~y~У Вас замерз торс");
        quest.weekend(false, -1, 8);
        return;
    }
...
timer.twoMinTimer = function () {
    ...
    if (veh && mp.vehicles.exists(veh) && veh.getClass() == 18 && !user.isGos()) {
        if (veh.getPedInSeat(-1) == mp.players.local.handle && mp.players.local.getDrawableVariation(1) < 1) {
...
timer.min15Timer = function () {
    ...
    try {
        if (user.getQuestCount('driftking') === 1 || user.getQuestCount('driftking') === 2)
            quest.driftking();
    }
    catch (e) { }
...
timer.fourSecTimer = function () {
    ...
    try {
        // if (mp.players.local.vehicle && user.isLogin() && mp.players.local.vehicle.isAttachedToTrailer()) {
        //     if (!mp.players.local.vehicle.getVariable('TRAILERED')) {
        //         let vHandle = mp.players.local.vehicle.getTrailer(mp.players.local.vehicle.handle)
        //         if (vHandle && mp.players.local.vehicle.handle != vHandle) {
        //             let trail = mp.vehicles.atHandle(vHandle)
        //             if (trail && mp.vehicles.exists(trail)) {
        //                 vehicles.setVariable('trailer', trail.remoteId)
        //                 vehicles.setVariable('TRAILERED', trail.model)
        //             }
        //         }
        //     }
        // } else if (mp.players.local.vehicle && user.isLogin() && mp.players.local.vehicle.getVariable('TRAILERED') && !mp.players.local.vehicle.isAttachedToTrailer()) {
        //     vehicles.setVariable('trailer', undefined)
        //     vehicles.setVariable('TRAILERED', null)
        // }


        //TODO
        // if (user.isLogin()) {
        //     let PedWeapon = user.getCurrentWeapon()
        //     user.getInvEquipWeapon().forEach((item) => {
        //         let kekus = items.getItemNameHashGTAById(item.item_id);
        //         if (kekus != PedWeapon && PedWeapon != -1569615261) {
        //             mp.game.ui.notifications.show("~r~Не нужно фантомить.");
        //             mp.game.ui.notifications.show(`~r~WPHASH ${kekus}.`);
        //             mp.game.ui.notifications.show(`~r~PedWeapon ${PedWeapon}.`);
        //             setTimeout(() => {
        ...
        user.showCustomNotify(JSON.stringify(e))
    }
    ...
    try {
        if(user.isLogin()){
            ...
            weapons.getMapList().forEach(item => {
                if (mp.game.invoke(methods.HAS_PED_GOT_WEAPON, mp.players.local.handle, (item[1] / 2), false) && !(user.getCurrentWeapon() == (item[1] / 2))) {
                    mp.attachmentMngr.addLocal(item[0].toUpperCase().replace('WEAPON_', ''));
                } else mp.attachmentMngr.removeLocal(item[0].toUpperCase().replace('WEAPON_', ''));
            });
        }
        ...
        if (user.isLogin()) {
            ...
            if (mp.players.local.isInAnyVehicle(true) && mp.players.local.dimension === 0 && giveTicketTimeout === 0) {
                ...
                */
                ...
                try {
                    if (veh.getVariable('user_id') === user.getCache('id') /*|| veh.getVariable('rentOwner') === user.getCache('id')*/) { //TODO RentOwner
                        if (
                            veh.getClass() === 0 ||
                            veh.getClass() === 1 ||
                            veh.getClass() === 2 ||
                            ...
                            mp.vehicles.forEach(async v => {
                                try {
                                    ...
                                    if (methods.distanceToPos(rPos, veh.position) < 30) {
                                        let ticketPrice = 0;
                                        ...
                                        if (ticketPrice > 0) {
                                            if (user.getBankMoney() > ticketPrice) {
                                                methods.callRemote('server:vehicle:radarCheck', ticketSpeed, ticketPrice, vehicles.getNumberPlate(veh), v.remoteId);
...
timer.tenSecTimer = function () {
    ...
    if (mp.players.local.dimension === 0 && !mp.players.local.isInAnyVehicle(true)) { //TODO Возможно вырезать это говно
        if (mp.players.local.weapon !== 2725352035 && !user.isGos() && mp.players.local.getDrawableVariation(1) < 1) {

            enums.cctvProps.forEach(async prop => {
...
timer.secTimer = function () {
    if (mp.players.local.getVariable('gangzone')) {
        if (!inventory.isHide()) {
            inventory.hide();
        }
    }
    try {
        if (user.isLogin()) {
            ...
            weapons.getMapList().forEach(item => {
                let hash = item[1] / 2;
                if (mp.game.invoke(methods.HAS_PED_GOT_WEAPON, mp.players.local.handle, hash, false)) {
                    if (!Container.Data.HasLocally(0, hash.toString()) && item[0] != 'weapon_unarmed' && item[0] != 'weapon_petrolcan') {
            ...
            if (ui.isGreenZone()) {
                mp.game.player.setVehicleDamageModifier(0);
                ...
                mp.game.player.setMeleeWeaponDamageModifier(1);
            }
            ...
            if (!mp.players.local.isInAnyVehicle(true) && dist < 100) {
                ...
                if (mp.players.local.vehicle.isInAir())
                    user.set('st_fly', user.getCache('st_fly') + methods.parseInt(dist));
                else
                    user.set('st_drive', user.getCache('st_drive') + methods.parseInt(dist));
            ...
            /*if (user.hasCache('rentTradeB') && !ui.isGreenZone())
                methods.callRemote('server:tradeMarket:unrentBlack', user.getCache('rentTradeB'));
            else if (user.hasCache('rentTrade') && !ui.isGreenZone())
                methods.callRemote('server:tradeMarket:unrentBeach', user.getCache('rentTrade'));*/
        }
    }
    catch (e) { }
    ...
    try {
        ...
        */
        ...
        if (user.getDrugLevel(drugId) > 0) {
            ...
            mp.game.invoke('0x9DCE1F0F78260875', true);
        ...
        if (mp.players.local.vehicle && wpPos.x != 0 && wpPos.y != 0) {
            if (prevWpPos.x != wpPos.x && prevWpPos.y != wpPos.y)
                methods.callRemote('server:changeWaypointPos', wpPos.x, wpPos.y);
        }
        prevWpPos = wpPos;
    }
    catch (e) {
...
timer.loadAll = function () {
    ...
    timer.createInterval('event.renderControl', function () {
        try {
            if (mp.game.invoke('0x5007A91D57C39FFC', mp.players.local.handle, 6)) {
