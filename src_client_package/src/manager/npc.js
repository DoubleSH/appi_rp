    ...
    try {
        ...
        if (npcList.length > 0) {
            for (const [key, item] of npcList) {
                npc.create(mp.game.joaat(item.skin), item.pos, item.skinRot, false, item.anim, '', '', 9, 5, 'GENERIC_HI', 'GENERIC_BYE', key);
            }
        }
    }
    catch (e) {
...
// Помошник шерифа на ресепшане Paleto Bay
    npc.create(mp.game.joaat("s_f_y_sheriff_01"), new mp.Vector3(1542.7093505859375, 816.1333618164062, 77.65579986572266), 77.01167297363281, false, "CODE_HUMAN_MEDIC_TIME_OF_DEATH");
...
//Автобазар
    npc.create(mp.game.joaat("u_m_y_ushi"), new mp.Vector3(-2133.630859375, 1097.26416015625, 25.66208839416504), -82.14453887939453, false, "WORLD_HUMAN_CLIPBOARD");
...
// Шериф на выдаче оружия Paleto Bay
    npc.create(mp.game.joaat("csb_cop"), new mp.Vector3(1549.9700927734375, 838.8641357421875, 77.65579986572266), 151.65089416503906, false, "WORLD_HUMAN_CLIPBOARD");
//Церковь
    ...
    * */
    ...
    npc.create(mp.game.joaat("a_m_m_salton_02"), new mp.Vector3(-810.9563598632812, -992.243896484375, 13.416413307189941), 175.0, 0, 'WORLD_HUMAN_SMOKING', '', '', 9, 5, 'GENERIC_HI', 'GENERIC_BYE', 'detectiveHusband'); //Муж детектив 1'detectiveHusband'
    npc.create(mp.game.joaat("s_m_y_sheriff_01"), new mp.Vector3(1850.401611328125, 3680.085205078125, 34.26767349243164), -145.10113525390625, 0, 'CODE_HUMAN_MEDIC_TIME_OF_DEATH', '', '', 9, 5, 'GENERIC_HI', 'GENERIC_BYE', 'detectiveSheriff'); //Шериф детектив квест1
    npc.create(mp.game.joaat("ig_janet"), new mp.Vector3(1392.245361328125, 3598.549072265625, 35.02996063232422), -157.36244201660156, 0, 'WORLD_HUMAN_SMOKING', '', '', 9, 5, 'GENERIC_HI', 'GENERIC_BYE', 'detectiveMom'); //Мамка детектив1
    npc.create(mp.game.joaat("a_f_y_tourist_02"), new mp.Vector3(-1597.3135986328125, 5205.8173828125, 4.310107231140137), 28.522253036499023, 0, 'WORLD_HUMAN_AA_COFFEE', '', '', 9, 5, 'GENERIC_HI', 'GENERIC_BYE', 'detectiveWife'); //Жена детектив 1
    npc.create(mp.game.joaat("a_m_m_salton_02"), new mp.Vector3(154.2190399169922, -1004.7070922851562, -98.41922760009766), 69.05327606201172, 0, '', 'missfbi1', 'cpr_pumpchest_idle', 9, 0, '', ''); //TODO детектив DEAD
    npc.create(mp.game.joaat("a_f_m_trampbeac_01"), new mp.Vector3(-1763.0504150390625, -262.7333068847656, 48.207889556884766), 150.0465850830078, 0, '', 'missfbi1', 'cpr_pumpchest_idle', 9, 0); //Мёртвая бабка


    npc.create(mp.game.joaat("a_m_m_indian_01"), new mp.Vector3(1717.36767578125, 3295.35107421875, 41.2018928527832), -169.2856903076172, 0, 'WORLD_HUMAN_SMOKING', '', '', 0, 5, 'GENERIC_HI', 'GENERIC_BYE', 'driftking_ded'); //Квестовый дед DK

    npc.create(mp.game.joaat("s_m_y_xmech_02"), new mp.Vector3(968.1239624023438, -2999.99755859375, -39.64699935913086), -84.04676055908203, 0, 'WORLD_HUMAN_CLIPBOARD', '', '', 0, 5, 'GENERIC_HI', 'GENERIC_BYE', 'driftking_auto'); //Автомеханик
        ...
        if (dist < _loadDist && !item.isCreate) {
            ...
            try {
                if (mp.game.streaming.hasModelLoaded(item.model)) {
                    ...
                    item.isCreate = true;
                    item.ped.isNpc = true;
                    item.ped.keyQuest = item.keyQuest;
...
npc.create = function(model, pos, heading, dim = 0, scenario = "", animation1 = "", animation2 = "", flag = 9, speechRadius = 5, speech1 = 'GENERIC_HI', speech2 = 'GENERIC_BYE', keyQuest = '') {

    if (typeof model == "string")
        model = mp.game.joaat(model);

    _npcList.push({model: model, pos: pos, heading: heading, keyQuest: keyQuest, ped: null, dim: dim, scenario: scenario, animation1: animation1, animation2: animation2, flag: flag, speechRadius: speechRadius, speech1: speech1, speech2: speech2, isSpeech: false, isCreate: false, handle: 0});
};
...
npc.showDialogDk1 = function(question, btn = []) {
    npc.getDialog(new mp.Vector3(-877.7675170898438, -2741.1162109375, 13.932750701904297 + 0.6), 26.7, 'Майкл', 'Рабочий', question, btn)
};

npc.showDialogDetective = function(question, btn = []) {
    npc.getDialog(new mp.Vector3(427.3970031738281, -986.3198852539062, 30.711612701416016 + 0.6), -1, 'Фрич', 'Детектив SAPD', question, btn)
};
npc.showDialogDetectiveHusband = function(question, btn = []) {
    npc.getDialog(new mp.Vector3(-810.9563598632812, -992.243896484375, 13.416413307189941 + 0.6), -1, 'Фрич', 'Муж пропавшей жены', question, btn)
};
npc.showDialogDkDed = function(question, btn = []) {
    npc.getDialog(new mp.Vector3(1717.36767578125, 3295.35107421875, 41.2018928527832 + 0.6), -169.2856903076172, 'Сальвадоре', 'Житель Sandy-Shores', question, btn)
};

npc.showDialogDkAuto = function(question, btn = []) {
    npc.getDialog(new mp.Vector3(968.1239624023438, -2999.99755859375, -39.64699935913086 + 0.6), -84.04676055908203, 'Диего', 'Автомеханик', question, btn)
};
...
npc.showDialogUsmc = function(question, btn = []) {
    npc.getDialog(new mp.Vector3(486.37030029296875, -3027.28515625, 6.014427661895752 + 0.6), 275.48919677734375, 'Офицер', 'Офицер USMC', question, btn)
};
