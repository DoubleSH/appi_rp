...
new_events.add(__CONFIG__.receivingTokenEventName, (token) => {
...
new_events.add('voice.requestMediaPeerResponse', (peerName, isSuccessful) => {
    if (!isSuccessful) {
        setTimeout(() => {
            try {
...
new_events.add('voice.changeStateConnection', (state) => {

    methods.callRemote('voice.changeStateConnection', state);
...
new_events.add('c.virtualStreamIn', (player) => {
    if (player.type === 'player') {
...
new_events.add('c.virtualStreamOut', (player, type) => {
    if (player.type !== 'player') {
        return false;
    }
...
new_events.add('client:restartVoice', () => {
...
new_events.add('client:restartVoice2', () => {
    if (__STORAGE__.browserReady) {
        mp.game.ui.notifications.show("~b~Перезагружаем голосовой чат...");
...
new_events.add('voice.phoneCall', (target_1, target_2, volume) => {
    if (!__PHONE__.target) {
...
new_events.add('voice.phoneStop', () => {
    if (__PHONE__.target) {
...
new_events.add('voice.radioConnect', (metaData, ...players) => {
...
new_events.add('voice.radioDisconnect', (metaData, ...players) => {
    if (!players.length) {
...
new_events.add('enable-voice-debug', () => {
        ...
        if (__STORAGE__.browserReady) {
            ...
            if (currentTime >= __STORAGE__.lastSendingProximityEvent + __CONFIG__.sendDataProximityInterval) {
                ...
                __STORAGE__.virtualStreamedPlayers.forEach((player) => {
                    ...
                    // methods.callRemote("server:debug:print", `VOICE HEAR ${player.remoteId}${__CONFIG__.prefixId}`)
                ...
                //     methods.callRemote("server:debug:print", `______VOICE RADIO HEAR______ ${player.remoteId}${__CONFIG__.prefixId}`)
...
new_events.add("voice:toogleRadioMic", async (player, isEnabled) => {
...
new_events.add('voice.toggleMicrophone', async (peerName, isEnabled) => {
...
new_events.add('voice.changeMicrophoneActivationKey', (newActivationKey) => {
...
const enableMicrophoneRadio = () => {
    if (
        ...
        if (user.getCache('walkietalkie_num') != "0" && user.getCache('jail_time') == 0) {
            ui.radioSoundOn();
            enableMicrophone();
            methods.callRemote("voice.server.enableMic");
...
const disableMicrophoneRadio = () => {
    if (
        ...
        if (user.getCache('walkietalkie_num') != "0" && user.getCache('jail_time') == 0) {
            ui.radioSoundOff();
            disableMicrophone();
            methods.callRemote("voice.server.disableMic");
