...
family.addMoney = function(id, money, itemName = 'Операция со счетом') {
    methods.callRemote('server:family:addMoney', id, money, itemName);
};

family.removeMoney = function(id, money, itemName = 'Операция со счетом') {
    methods.callRemote('server:family:removeMoney', id, money, itemName);
};

family.setMoney = function(id, money) {
    methods.callRemote('server:family:setMoney', id, money);
};

family.save = function(id) {
    methods.callRemote('server:family:save', id);
};
...
family.addHistory = function(name, doName, text, fractionId = 0) {
    methods.callRemote('server:addFamilyLog', name, doName, text, fractionId);
};
