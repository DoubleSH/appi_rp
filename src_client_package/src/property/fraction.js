...
fraction.addMoney = function(id, money, itemName = 'Операция со счетом') {
    methods.callRemote('server:fraction:addMoney', id, money, itemName);
};

fraction.removeMoney = function(id, money, itemName = 'Операция со счетом') {
    methods.callRemote('server:fraction:removeMoney', id, money, itemName);
};

fraction.setMoney = function(id, money) {
    methods.callRemote('server:fraction:setMoney', id, money);
};

fraction.save = function(id) {
    methods.callRemote('server:fraction:save', id);
};
...
fraction.unloadCargoVehTimer = async function(id, cargoId) {
    try {
        ...
        menuList.hide();
        methods.callRemote('server:vehicle:cargoUnload', id);
...
fraction.addHistory = function(name, doName, text, fractionId = 0) {
    methods.callRemote('server:addFractionLog2', name, doName, text, fractionId);
};
