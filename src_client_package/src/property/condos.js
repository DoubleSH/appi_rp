...
condos.enter = function (id) {
    methods.callRemote('server:condos:enter', id);
};
...
condos.buy = function (id) {
    ...
    methods.callRemote('server:condos:buy', id);
    return true;
};

condos.updatePin = function (id, pin) {
    methods.callRemote('server:condos:updatePin', id, pin);
};

condos.updateSafe = function (id, pin) {
    methods.callRemote('server:condos:updateSafe', id, pin);
};

condos.lockStatus = function (id, lockStatus) {
    methods.callRemote('server:condos:lockStatus', id, lockStatus);
};
