...
inventory.show = function () {
    ...
    try {
        ...
        mp.game.ui.notifications.show(`~b~Скрыть инвентарь на ~s~${bind.getKeyName(user.getCache('s_bind_inv'))}`);
...
inventory.setCoolDown = function (itemId, time = 3) {
    let dataSend = {
        type: 'updateCooldown',
        itemid: itemId,
        time: time,
...
inventory.updateEquipStatus = function (id, status) { //TODO, подумать как можно рессетнуть значения, чтобы не дюпали и не пропадли деньги
    methods.callRemote('server:inventory:updateEquipStatus', id, status);
};
...
inventory.updateItemCount = function (id, count) { //TODO, подумать как можно рессетнуть значения, чтобы не дюпали и не пропадли деньги
    methods.callRemote('server:inventory:updateItemCount', id, count);
};

inventory.updateItemsEquipByItemId = function (itemId, ownerId, ownerType, equip, count = -1) {
    methods.callRemote('server:inventory:updateItemsEquipByItemId', itemId, ownerId, ownerType, equip, count);
};

inventory.updateOwnerId = function (id, ownerId, ownerType) {
    ownerId = ownerId.toString();
    inventory.updateSubInvRadius(ownerId, ownerType);
    methods.callRemote('server:inventory:updateOwnerId', id, ownerId, ownerType);
};

inventory.updateOwnerIdWithPrice = function (id, ownerId, ownerType, price = 0) {
    ownerId = ownerId.toString();
    inventory.updateSubInvRadius(ownerId, ownerType);
    methods.callRemote('server:inventory:updateOwnerIdWithPrice', id, ownerId, ownerType, price);
};
...
inventory.updateOwnerAll = function (oldOwnerId, oldOwnerType, ownerId, ownerType) {
    ...
    inventory.updateSubInvRadius(ownerId, ownerType);
    methods.callRemote('server:inventory:updateOwnerAll', oldOwnerId, oldOwnerType, ownerId, ownerType);
};
...
inventory.updateItemParams = function (id, params) {
    try {
        ...
        methods.callRemote('server:inventory:updateItemParams', id, params);
    }
    catch (e) {
        methods.debug(e);
    }
    ...
    if (entity.getType() == 2) {
        ...
        try {
            ...
            if (entity.isDead()) {
                ...
                radialMenu.showVehMenu(entity, true);
        ...
        try {
            if (entity.getVariable('isDrop'))
                ...
                if (type === 0) {
                    ...
                    setTimeout(function () {
                        methods.callRemote('server:ems:removeObject', entity.remoteId);
                    ...
                    methods.blockKeys(true);
                    setTimeout(function () {
                        methods.callRemote('server:ems:attachObject', entity.remoteId);
                        methods.blockKeys(false);
                        user.addRep(10);
                        user.stopAllAnimation();
                    }, 8000);
                ...
                if (Container.Data.HasLocally(mp.players.local.remoteId, "isInventoryTimeout1")) {
        ...
        try {
            ...
            radialMenu.showUserMenu(entity);
        }
        catch (e) {
            methods.debug(e);
        }
...
inventory.useItem = function (id, itemId, ownerType, ownerId) {
    methods.callRemote('server:inventory:useItem', id, itemId, ownerType, ownerId);
};
...
inventory.dropItem = function (id, itemId, pos, rot) {
    methods.callRemote('server:inventory:dropItem', id, itemId, pos.x, pos.y, pos.z, rot.x, rot.y, rot.z);
};

inventory.deleteItemProp = function (id) {
    methods.callRemote('server:inventory:deleteDropItem', id);
};

inventory.deleteItem = function (id) {
    methods.callRemote('server:inventory:deleteItem', id);
};

inventory.deleteItemByItemId = function (itemId, isEquip = 0) {
    methods.callRemote('server:inventory:deleteItemByItemId', itemId, isEquip);
};

inventory.deleteItemsRange = function (itemIdFrom, itemIdTo) {
    methods.callRemote('server:inventory:deleteItemsRange', itemIdFrom, itemIdTo);
};
...
inventory.updateAmountMax = function (id, type) {
    if (type == inventory.types.Vehicle) {
        ...
        let invAmountMax = inventory.getAmountMaxByType(0, type);
...
inventory.getAmountMaxByType = function (id, type) {
    ...
    if (type == inventory.types.World)
        ...
        invAmountMax = 20000;
    else if (type == inventory.types.BagExclusive)
        invAmountMax = 100000;
...
inventory.updateAmount = function (id, type) {
    methods.callRemote('server:inventory:updateAmount', id, type); //TODO
};
...
inventory.updateSubInvRadius = function (ownerId, ownerType, withMe = false) {
    ...
    timeOut = setTimeout(function () {
        ...
        if (ownerType < 0 || ownerType === 1 /*|| ownerType === 7 || ownerType === 2*/)
            return;
        methods.callRemote('server:inventory:updateSubInvRadius', ownerId.toString(), ownerType, withMe);
        timeOut = null;
    }, 100)
};

inventory.addItem = function (itemId, count, ownerType, ownerId, countItems, isEquip = 0, params = "{}", timeout = 10) {
    methods.callRemote('server:inventory:addItem', itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout);

    // methods.callRemote('server:inventory:addItem', 474, 1, 1, 1, 1, 0, JSON.stringify({id:3}), 10);
};

inventory.addItemSql = function (itemId, count, ownerType, ownerId, countItems, isEquip = 0, params = "{}", timeout = 10) {
    methods.callRemote('server:inventory:addItemSql', itemId, count, ownerType, ownerId, countItems, isEquip, params, timeout);
};

inventory.addWorldItem = function (itemId, count, countItems, pos, rot, params = "{}", timeout = 10) {
    methods.callRemote('server:inventory:addWorldItem', itemId, count, countItems, pos.x, pos.y, pos.z + 1, rot.x, rot.y, rot.z, params, timeout);
};

inventory.dropWeaponItem = function (itemId, pos, rot) {
    methods.callRemote('server:inventory:dropWeaponItem', itemId, pos.x, pos.y, pos.z, rot.x, rot.y, rot.z);
};
...
inventory.addPlayerWeaponItem = function (itemId, count, ownerType, ownerId, countItems, isEquip = 0, params = "{}", text = 'Получено оружие', timeout = 10) {
    methods.callRemote('server:inventory:addPlayerWeaponItem', itemId, count, ownerType, ownerId, countItems, isEquip, params, text, timeout);
};
...
inventory.updateItemOwnerServer = function (id, ownerType, ownerId) {
    methods.callRemote('server:inventory:updateItemOwner', id, ownerType, ownerId);
};
...
inventory.getItemList = function (ownerType, ownerId) {
    setTimeout(() => {
       methods.callRemote('server:inventory:getItemList', ownerType, ownerId.toString()); 
    }, 200);
    
};

inventory.getItemListSell = function () {
    methods.callRemote('server:inventory:getItemListSell');
};

inventory.getItemListSellFish = function (shopId = 0) {
    methods.callRemote('server:inventory:getItemListSellFish', shopId);
};

inventory.getVehicleHash = function (vehicle) {
    let numberPlate = vehicles.getNumberPlate(vehicle);
    return mp.game.joaat(numberPlate.trim());
};
let fishing = false;
