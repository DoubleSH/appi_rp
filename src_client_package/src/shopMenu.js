...
shopMenu.updateShop2 = function(list, banner = '', bgColor = '', type = 0, title = 'Добро пожаловать', backEvent = '', subTitle = '', selected = -1) {
    let sendData = {
        type: 'updateItems',
        items: list,
        title: title,
...
new_events.add('client:shopMenu:hide', function() {
    hidden = false;
    shopMenu.resetControl();
});

new_events.add('client:carRent:hide', function() {
    hidden = false;
    shopMenu.resetControl();
});

new_events.add('client:shopMenu:hideLeft', function() {
    hidden = false;
    shopMenu.resetControl2();
    if (user.hasCache('seeMask')) {
        user.reset('seeMask');
...
new_events.add('client:shopMenu:hideLeft', function(eventName) {
    if (eventName === '') {
...
new_events.add('client:shopMenu:changeSelect2', async function(json) {
    try {
        ...
        if (params.type === 'lsc:list:remove') {
            if (params.modType === 78) {
                ...
                methods.callRemote('server:lsc:showTun', params.modType, -1);
            }
        }
        if (params.type === 'lsc:list:buy') {
            if (params.modType === 999)
                return;
            if (params.modType === 80) {
                try {
                    methods.callRemote('server:lsc:showTun', params.modType, params.id);
                }
                catch (e) {
                }
            }
        ...
        if(params.type == 'lsc:color:buy') {
            methods.callRemote('server:lsc:showColor' + params.ev, params.modType);
        }
        if(params.type == 'lsc:s:setSmoke')
            menuList.showLscTyreColorChoiseMenu(params.shop)
        if(params.type == 'lsc:s:numberPlate')
            methods.callRemote('server:lsc:showNumberType', params.id);
        if(params.type == 'lsc:list:show')
...
new_events.add('client:shopMenu:buyCash2', async function(json) {
    try {
        let params = JSON.parse(json);
        methods.debug(json);
        if(params.doName == 'tt:show') {
            quest.weekend(false, -1, 0);
            methods.callRemote('server:tattoo:buy', params.tatto1, params.tatto2, '', params.price, params.tatto0, params.shop, 0);
        }
        if(params.doName == 'tt:destroy')
            methods.callRemote('server:tattoo:destroy', params.tatto1, params.tatto2, '', params.price,'Лазерная коррекция', params.shop, 0);
        if(params.type == 'lsc:number:buy') {
            shopMenu.hideShop2();
            let number = await menuList.getUserInput("Номер", "", 8);
            methods.callRemote('server:lsc:buyNumber', params.shop, number.toUpperCase(), 0);
        }
        if(params.type == 'lsc:repair:buy')
            methods.callRemote('server:lsc:repair', params.shop, params.price, 0);
        if(params.type == 'lsc:color:buy')
            methods.callRemote('server:lsc:buyColor' + params.ev, params.modType, params.price, params.shop, `Цвет: ${params.itemName}`, 0);
        if(params.type == 'lsc:s:setNeon')
            methods.callRemote('server:lsc:buyNeon', params.shop, params.price, 0);
        if(params.type == 'lsc:s:setLight')
            methods.callRemote('server:lsc:buyLight', params.shop, params.price, 0);
        if(params.type == 'lsc:s:setSpecial')
            methods.callRemote('server:lsc:buySpecial', params.shop, params.price, 0);
        if(params.type == 'lsc:s:setSmoke')
            menuList.showLscTyreColorChoiseMenu(params.shop)
        if(params.type == 'lsc:s:buySmoke')
            methods.callRemote('server:lsc:buyTyreColor', params.id, params.price, params.shop, 0);
        if(params.type == 'lsc:s:numberPlate')
            methods.callRemote('server:lsc:buyNumberType', params.id, params.price, params.shop, 0);
        if(params.type == 'lsc:s:mod:reset')
            methods.callRemote('server:lsc:resetSTun', params.idx);
        if (params.type === 'lsc:list:remove')
            methods.callRemote('server:lsc:buyTun',  params.modType, -1, params.price, params.shop, 'Стандартная деталь', 0);
        if (params.type === 'lsc:list:buy')
            methods.callRemote('server:lsc:buyTun',  params.modType, params.id, params.price, params.shop, params.itemName, 0);
        if(params.type == 'lsc:s:mod:buy') {
            if (params.mod === 0)
                methods.callRemote('server:lsc:buySTun', params.mod, params.idx, params.price, params.shop, enums.lscSNames[params.mod][0], 0);
            else
                methods.callRemote('server:lsc:buySTun', params.mod, params.idx, params.price, params.shop, enums.lscSNames[params.mod][0], 0);
        }
        if(params.type == 'lsc:s:fix') {
            methods.callRemote('server:lsc:buySFix', params.idx, params.price, params.shop, params.fixName, 0);
        }
        if (params.type === 'mask:buy') {
        ...
        if(params.type == 'b:show') {
            ...
            if(params.zone === 'SKIN_OVERLAY_8')
                quest.weekend(false, -1, 7);
        ...
        if (params.type === 'print:buy') {
            methods.callRemote('server:print:buy', params.tatto1, params.tatto2, params.price, params.shop, 0);
        }
    }
    catch (e) {
...
new_events.add('client:shopMenu:buyCard2', async function(json) {
    ...
    try {
        ...
        if(params.doName == 'tt:show') {
            quest.weekend(false, -1, 0);
            methods.callRemote('server:tattoo:buy', params.tatto1, params.tatto2, '', params.price, params.tatto0, params.shop, 1);
        }
        if(params.doName == 'tt:destroy')
            methods.callRemote('server:tattoo:destroy', params.tatto1, params.tatto2, '', params.price,'Лазерная коррекция', params.shop, 1);
        if(params.type == 'lsc:number:buy') {
            shopMenu.hideShop2();
            let number = await menuList.getUserInput("Номер", "", 8);
            methods.callRemote('server:lsc:buyNumber', params.shop, number.toUpperCase(), 1);
        }
        if(params.type == 'lsc:repair:buy')
            methods.callRemote('server:lsc:repair', params.shop, params.price, 1);
        if(params.type == 'lsc:color:buy')
            methods.callRemote('server:lsc:buyColor' + params.ev, params.modType, params.price, params.shop, `Цвет: ${params.itemName}`, 1);
        if(params.type == 'lsc:s:setNeon')
            methods.callRemote('server:lsc:buyNeon', params.shop, params.price, 1);
        if(params.type == 'lsc:s:setLight')
            methods.callRemote('server:lsc:buyLight', params.shop, params.price, 1);
        if(params.type == 'lsc:s:setSpecial')
            methods.callRemote('server:lsc:buySpecial', params.shop, params.price, 1);
        if(params.type == 'lsc:s:setSmoke')
            menuList.showLscTyreColorChoiseMenu(params.shop);
        if(params.type == 'lsc:s:buySmoke')
            methods.callRemote('server:lsc:buyTyreColor', params.id, params.price, params.shop, 1);
        if(params.type == 'lsc:s:numberPlate')
            methods.callRemote('server:lsc:buyNumberType', params.id, params.price, params.shop, 1);
        if(params.type == 'lsc:s:mod:reset')
            methods.callRemote('server:lsc:resetSTun', params.idx);
        if (params.type === 'lsc:list:remove')
            methods.callRemote('server:lsc:buyTun',  params.modType, -1, params.price, params.shop, 'Стандартная деталь', 1);
        if (params.type === 'lsc:list:buy')
            methods.callRemote('server:lsc:buyTun',  params.modType, params.id, params.price, params.shop, params.itemName, 1);
        if(params.type == 'lsc:s:mod:buy') {
            if (params.mod === 0)
                methods.callRemote('server:lsc:buySTun', params.mod, params.idx, params.price, params.shop, enums.lscSNames[params.mod][0], 1);
            else
                methods.callRemote('server:lsc:buySTun', params.mod, params.idx, params.price, params.shop, enums.lscSNames[params.mod][0], 1);
        }
        if(params.type == 'lsc:s:fix') {
            methods.callRemote('server:lsc:buySFix', params.idx, params.price, params.shop, params.fixName, 1);
        }
        if (params.type === 'mask:buy') {
            quest.gang(false, -1, 4);
        ...
        if (params.type === 'print:buy') {
            methods.callRemote('server:print:buy', params.tatto1, params.tatto2, params.price, params.shop, 1);
        }
    }
    catch (e) {
...
new_events.add('client:carRent:buyCash', async function(name, json) {
    try {
        let params = JSON.parse(json);
        quest.standart(false, -1, 2);
        methods.callRemote('server:rent:buy', params.hash, params.price, params.shop, 0);
...
new_events.add('client:carRent:buyCard', async function(name, json) {
    if (user.getCache('bank_card') < 1) {
    ...
    try {
        let params = JSON.parse(json);
        quest.standart(false, -1, 2);
        methods.callRemote('server:rent:buy', params.hash, params.price, params.shop, 1);
        shopMenu.hideCarRent();
    }
    catch (e) {
        mp.game.ui.notifications.show(`~r~Произошла неизвестная ошибка`);
...
new_events.add('client:shopMenu:buyCash', async function(json) {
    try {
        let params = JSON.parse(json);
        methods.debug(json);
        if (params.doName == 'radio') {
            ...
            methods.callRemote('server:tradeMarket:buy', params.id, params.price, params.name, params.ownerId, 1);
        }
        else {
            if (items.isWeapon(params.id)) {
                ...
                methods.callRemote('server:gun:buy', params.id, params.price, 1, methods.parseInt(params.superTint), methods.parseInt(params.tint), params.shop, 0);
            }
            else {
                if (params.id === 251)
                ...
                methods.callRemote('server:shop:buy', params.id, params.price, params.shop);
            }
        }
    }
    catch (e) {
...
new_events.add('client:shopMenu:buyCard', async function(json) {
    if (user.getCache('bank_card') < 1) {
        mp.game.ui.notifications.show(`~r~У Вас нет банковской карты`);
        return;
    }
    try {
        ...
        if (params.doName === 'radio') {
            ...
            if (items.isWeapon(params.id)) {
                ...
                methods.callRemote('server:gun:buy', params.id, params.price, 1,  methods.parseInt(params.superTint), methods.parseInt(params.tint), params.shop, 1);
            }
            else {
                if (params.id === 251)
                    quest.fish(false, -1, 1);
                    ...
                    quest.standart(false, -1, 5);
                methods.callRemote('server:shop:buyCard', params.id, params.price, params.shop);
            }
        }
    }
    catch (e) {
...
new_events.add('client:shopMenu:doName', function(json) {
    try {
        let params = JSON.parse(json);
...
new_events.add('client:dialog:btn', async function(json) {
    try {
        let params = JSON.parse(json);
        ...
        if (params.doName === 'quest:lamarDk') {
            shopMenu.hideDialog();
            quest.driftking(true);
        }
        if (params.doName === 'quest:ded:dialog0') {
            menuList.showBotQuestDkDedMenu(1)
        }
        if (params.doName === 'quest:ded:dialog1') {
            menuList.showBotQuestDkDedMenu(2)
        }
        ...
        if (params.doName === 'lspd:takeWeap') {
            shopMenu.hideDialog();
            methods.callRemote('server:user:toLspdSafe');
        }
        if (params.doName === 'lspd:setNumber') {
            menuList.showBotLspdSetVehNumberMenu(params.idx);
        }
        if (params.doName === 'lspd:setNumberId') {
            shopMenu.hideDialog();
            methods.callRemote('server:vehicles:setNumberLspd', params.vId);
        }
        ...
        if (params.doName === 'inv:wantWork') {
            ...
            methods.callRemote('server:discord:sendWorkNews', discord, text);
        }
        if (params.doName === 'lspd:wantWork') {
            ...
            let text = await menuList.getUserInput('Почему вы хотите тут работать?', '', 100);
            mp.game.ui.notifications.show(`~g~Заявление было отправлено, скоро с вами свяжутся в дискорде`);
            methods.callRemote('server:discord:sendWorkLspd', discord, text);
        }
        if (params.doName === 'lspd:quest') {
            shopMenu.hideDialog();
            quest.driftking();
            mp.game.ui.notifications.show(`~g~Ваше заявление на угон было принято сотрудником`);
        }
        if (params.doName === 'bcsd:wantWork') {
            ...
            let text = await menuList.getUserInput('Почему вы хотите тут работать?', '', 100);
            mp.game.ui.notifications.show(`~g~Заявление было отправлено, скоро с вами свяжутся в дискорде`);
            methods.callRemote('server:discord:sendWorkBcsd', discord, text);
        }
        if (params.doName === 'usmc:wantWork') {
            ...
            mp.game.ui.notifications.show(`~g~Заявление было отправлено, скоро с вами свяжутся в дискорде`);
            methods.callRemote('server:discord:sendWorkUsmc', discord, text);
        }
        if (params.doName === 'ems:wantWork') {
            ...
            let text = await menuList.getUserInput('Почему вы хотите тут работать?', '', 100);
            mp.game.ui.notifications.show(`~g~Заявление было отправлено, скоро с вами свяжутся в дискорде`);
            methods.callRemote('server:discord:sendWorkEms', discord, text);
        }
        ...
        if (params.doName === 'lspd:toJail') {
            shopMenu.hideDialog();
            methods.callRemote('server:user:arrest');
        }
        if (params.doName === 'lamar:car') {
            shopMenu.hideDialog();
            lamar.start();
        }
        if (params.doName === 'quit') {
            let btn = [];
            npc.showDialogDetective('Ну и теряйся от сюда!', btn);
        }
        if (params.doName === 'questDetective2') {
            quest.detective()
        }
        if (params.doName === 'questDetective1') {
            let btn = [];
            btn.push(
                {
                    text: 'Хочу',
                    bgcolor: '',
                    params: { doName: 'questDetective2' }
                }
            );
            btn.push(
                {
            ...
            npc.showDialogDetective('Хм... Окей. Не хочешь подработать? А то я гемморой заработал, мне сейчас не до побегушек.', btn);
        }
        ...
        if (params.doName === 'lamar:buy') {
            if (user.getCryptoMoney() < 0.1) {
                mp.game.ui.notifications.show(`~r~На счету BitCoin нет столько денег`);
                return;
            }

            user.removeCryptoMoney(0.1, 'Покупка спец. отмычек');
        ...
        if (params.doName === 'user:lspd:takeVehicle') {
            shopMenu.hideDialog();
            methods.callRemote('server:lspd:takeVehicle', params.x, params.y, params.z, params.rot, params.vid)
        }
        if (params.doName === 'ave:brak') {
            shopMenu.hideDialog();
            let id = methods.parseInt(await menuList.getUserInput('Введите ID игрока'));
            methods.callRemote('server:user:askAve', id);
        }
        if (params.doName === 'ave:nobrak') {
            shopMenu.hideDialog();
            let id = methods.parseInt(await menuList.getUserInput('Введите ID игрока'));
            methods.callRemote('server:user:askNoAve', id);
        }
