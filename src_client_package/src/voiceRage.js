...
voiceRage.enableMic = function() {
    ...
    methods.callRemote('voice.toggleMicrophone', true);
    //user.setVariable('voiceMic', mp.voiceChat.muted);
};

voiceRage.disableMic = function() {
    mp.voiceChat.muted = true;
    methods.callRemote('voice.toggleMicrophone', false);
    //user.setVariable('voiceMic', mp.voiceChat.muted);
};
...
voiceRage.enableRadioMic = function() {
    if (user.isDead() || methods.isBlockKeys())
        return;
    voiceRage.enableMic();
    methods.callRemote('voice.toggleMicrophoneRadio', true);
...
voiceRage.disableRadioMic = function() {
    ...
    methods.callRemote('voice.toggleMicrophoneRadio', false);
    if(mp.players.local.isReloading() || mp.players.local.isShooting() || mp.game.player.isFreeAiming()){
        return;
    }
...
voiceRage.add = (player) => {
    ...
    methods.callRemote("addVoiceListener", player);
...
voiceRage.remove = (player, notify) => {
    ...
    if(notify)
    {
        methods.callRemote("removeVoiceListener", player);
    }
};
...
voiceRage.generateVolumeNew = (localPlayerPosition, targetPlayer, dist) => {
    ...
    const calcDublDist = distanceToPlayer * distanceToPlayer;
    const maxVolume = methods.parseFloat(user.getCache('s_voice_vol'));
    let volume = voiceRage.clamp(0, maxVolume, 1 - (dist / MaxRangeVol));
...
voiceRage.timer = () => {
    try {
        ...
        {
            try {
                if(player.handle && player != localPlayer)
                {
                    if(!player.isListening)
                    {
                        /*let walk = player.getVariable('walkie');
                        if (localPlayer.getVariable('walkie') !== '000.000' && walk === localPlayer.getVariable('walkie') && user.getCache('walkie_buy') && player.getVariable('walkieBuy')) {
                            voiceRage.add(player);
                            return;
                        }*/

                        /*if (typeof player.getVariable('callId') !== 'undefined' && player.getVariable('callId') === localPlayer.remoteId) {
                            voiceRage.add(player);
                            return;
                        }*/
        ...
        voiceRage.listeners.forEach((player) =>
        {
            try {
                if(player.handle !== 0)
                {
                    ...
                    {
                        const voiceDistance = voiceRage.clamp(3, MaxRange, player.getVariable('voice.distance') || __CONFIG__.defaultDistance);
                        ...
                        if (dist >= MaxRange)
                            ...
                            player.voiceVolume = voiceRage.generateVolumeNew(localPos, player, dist);
...
new_events.add("playerQuit", (player) =>
{
    if(player.isListening)
    {
...
new_events.add('voice.toggleMicrophone', async (playerId, isEnabled) => {
...
new_events.add('voice.toggleMicrophoneRadio', async (playerId, isEnabled) => {
    /*const player = mp.players.atRemoteId(playerId);

    if (player && mp.players.exists(player) && user.getCache('walkie_buy') && player.getVariable('walkieBuy')) {

        if (mp.players.local.getVariable('walkie') === '000.000')
            return;
