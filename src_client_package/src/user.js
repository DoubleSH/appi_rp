...
new_events.add('render', () => {
    ...
    try {
        if (user.isLogin() && entity && entity.getAlpha() > 0) {
            if (user.getCache('s_hud_raycast') === 0) {
                ui.drawText(`•`, 0.5, 0.5, 0.3, 255, 255, 255, 200, 0, 1, false, true);
            }
            else if (user.getCache('s_hud_raycast') === 1) {
                ...
                if(entity.getVariable('boxedId')){
                    ui.drawText3D(`${currentKey} \n #${entity.getVariable('boxedId')}`, pos.x, pos.y, pos.z, 0.3 + currentScale);
                }
                else {
            ...
            else if (user.getCache('s_hud_raycast') === 2) {
                let pos = entity.position;
                if(entity.getVariable('boxedId')){
                    ui.drawText3D(`• \n #${entity.getVariable('boxedId')}`, pos.x, pos.y, pos.z, 0.3 + currentScale);
                }
                else {
                    ui.drawText3D(`•`, pos.x, pos.y, pos.z, 0.3 + currentScale);
                }
            }
        }
    }
...
user.timerRayCast = async function () {
    ...
    try {
        if (!mp.players.local.isSittingInAnyVehicle()) {
            ...
            targetEntityPrev = user.getTargetEntityValidate();
        }
        else
...
user.getTargetEntityValidate = function () {
    try {
        if (user.targetEntity.entity && user.targetEntity.entity.isNpc && user.targetEntity.entity.keyQuest === '')
            return null;
        if (
            user.targetEntity &&
...
user.giveWeaponComponentByHash = function (model, component, wpName) {
    //mp.game.invoke(methods.GIVE_WEAPON_COMPONENT_TO_PED, mp.players.local.handle, model, component);
    mp.events.callRemote('server:user:giveWeaponComponent', model.toString(), component.toString(), wpName);
};

user.giveWeaponComponent = function (model, component) {
    user.giveWeaponComponentByHash(weapons.getHashByName(model), component, model);
};

user.removeWeaponComponentByHash = function (model, component, wpNamed) {
    //mp.game.invoke(methods.GIVE_WEAPON_COMPONENT_TO_PED, mp.players.local.handle, model, component);
    mp.events.callRemote('server:user:removeWeaponComponent', model.toString(), component.toString(), wpNamed);
};

user.removeWeaponComponent = function (model, component) {
    user.removeWeaponComponentByHash(weapons.getHashByName(model), component, model);
};
...
user.setWeaponTintByHash = function (model, tint, wpNamed) {
    mp.events.callRemote('server:user:setWeaponTint', model.toString(), methods.parseInt(tint), wpNamed);
};

user.setWeaponTint = function (model, tint) {
    user.setWeaponTintByHash(weapons.getHashByName(model), tint, model);
};
    ...
    setTimeout(function () {
        try {
            ...
            if (mp.players.local.vehicle) {
                ...
                mp.events.callRemote('server:vehicle:position', pos.x, pos.y, pos.z);
                if (rot != undefined)
...
user.tpToWaypoint = function () { //TODO машина
    try {
        ...
        if (mp.players.local.vehicle)
            mp.events.callRemote('server:vehicle:position', pos.x, pos.y, pos.z + 20);
        else
        ...
        let interval = setInterval(function () {
            try {
                ...
                if (mp.players.local.vehicle)
                    mp.events.callRemote('server:vehicle:position', pos.x, pos.y, entity.position.z + 20);
                else
                ...
                if (entity.position.z > 1000 || zPos != 0) {
                    if (mp.players.local.vehicle)
                        mp.events.callRemote('server:vehicle:position', pos.x, pos.y, zPos + 2);
                    else
...
user.sendMessageNumber = function (numberFrom, phoneNumber, message, avatar) {
    mp.events.callRemote('server:phone:sendMessageNumber', numberFrom.toString(), phoneNumber.toString(), message, avatar);
};
...
user.login = function (name, spawnName) {
    ...
    setTimeout(function () {
        try {
            ...
            mp.events.callRemote('server:user:loginUser', name, spawnName, user.hash);
        }
        catch (e) {
            methods.debug(e);
        }
...
user.setWeaponComponents = function (key, value) {
    try {
        if (!user.weaponComponents[key].hasOwnProperty(value)) user.weaponComponents[key] = new Set();
        user.weaponComponents[key].add(value);
    } catch (e) {
        methods.error(e)
    }

};
user.delWeaponComponents = function (key, value) {
    try {
        if(user.weaponComponents[key].hasOwnProperty(value)){
            user.weaponComponents[key].delete(value);
        }
    } catch (e) {
        methods.error(e)
    }
...
user.getById = async function (key) {
    return await Container.Data.Get(user.getCache('id') - 88888888, key);
};

user.hasById = async function (key) {
    return await Container.Data.Has(user.getCache('id') - 88888888, key);
};

user.setById = function (key, value) {
    Container.Data.Set(user.getCache('id') - 88888888, key, value);
};
...
user.stopAllScreenEffect = function () {
    ...
    mp.game.invoke('0x9DCE1F0F78260875', false);
    mp.game.invoke('0xD821490579791273', false);
};
...
user.getCasinoMoney = function () {
    return methods.parseInt(user.getCache('money_casino') || 0);
};

user.addCasinoMoney = function (money, text = 'Финансовая операция') {
    user.setCache('money_casino', user.getCasinoMoney() + methods.parseInt(money));
    mp.events.callRemote('server:user:addCasinoMoney', money, text);
};

user.removeCasinoMoney = function (money, text = 'Финансовая операция') {
    user.setCache('money_casino', user.getCasinoMoney() - methods.parseInt(money));
    mp.events.callRemote('server:user:removeCasinoMoney', money, text);
};

user.setCasinoMoney = function (money) {
    user.setCache('money_casino', methods.parseInt(money));
    mp.events.callRemote('server:user:setCasinoMoney', money);
};
...
user.isInOcean = function () {
    ...
    return (mp.game.zone.getNameOfZone(pos.x, pos.y, pos.z) === "OCEANA" || mp.game.zone.getNameOfZone(pos.x, pos.y, pos.z) === "ALAMO");
};
...
user.giveJobSkill = function (name = 'no') {
...
user.giveJobMoney = async function (money, jobId = 0) {
    ...
    if (user.getCache('bank_card') == 0) {
        ...
        mp.game.ui.notifications.show('~y~Деньги зачислены на руки, рекомендуем оформить банковскую карту');
    }
    else {
...
user.isVip = function () {
    if (!user.isLogin())
        return false;
    return user.getCache('vip_type') > 0;
};
...
user.playAnimation = function (dict, anim, flag = 49, sendEventToServer = true) {
    ...
    if (dict === 'anim@mp_player_intupperfinger' || dict === 'anim@mp_player_intcelebrationmale@finger') {
        quest.weekend(false, -1, 2);
    }

    if (dict === 'mp_safehouse') {
        if (mp.game.interior.getInteriorAtCoords(mp.players.local.position.x, mp.players.local.position.y, mp.players.local.position.z) === 197121) {
            quest.weekend(false, -1, 3);
        }
    }
...
user.isGang = function () {
    return user.isLogin() && (fraction.isGang(user.getCache('fraction_id2')));
};
