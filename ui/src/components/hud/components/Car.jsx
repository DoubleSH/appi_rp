...
class Car extends React.Component {
    ...
    speed() { // Чуть позже добавлю перерисованный спидометр, ибо текущий с погрешностью
        ...
        let deg = 177 / 400 * this.state.speed;
        this.setState({deg});
    }
                ...
                <Draggable id="car-speedbox" isShowSmall={this.state.isShowSmall}>
                    <div className={this.state.isShowSmall ? 'hide' : 'speedbox-main-small'}>
                        <div className="speedbox">
                            <div className="speedbox-scale" style={speed_score}></div>
                        </div>
                        <div className="backg-speed">
                            <div className="speed-txt">{this.state.speed}</div>
                            <div className="speed-label">{this.state.speedLabel}</div>
                        </div>
                    </div>
                </Draggable>
                <Draggable id="car-speedbox-small" isShowSmall={this.state.isShowSmall}>
                    <div className={this.state.isShowSmall ? 'backg-speed small' : 'hide'}>
                        <div className="speed-txt">{this.state.speed}</div>
                        <div className="speed-label">{this.state.speedLabel}</div>
                    </div>
                </Draggable>
                <div className="car-hud">
                    <Draggable id="car-elements">
                        <div className="elements-auto"
                             style={{backgroundColor: `rgba(${this.state.isDarkBackground ? '0,0,0' : '255,255,255'}, ${this.state.background})`}}>
                            <div className={this.state.light ? 'light-auto active' : 'light-auto'}></div>
                            <div className={this.state.door ? 'on-door-auto' : 'off-door-auto'}></div>
                            <div className={this.state.engine ? 'key-auto active' : 'key-auto'}></div>
                        </div>
                    </Draggable>
                    <Draggable id="car-fuel">
                        <div className="bak-oil" style={{backgroundColor: `rgba(${this.state.isDarkBackground ? '0,0,0' : '255,255,255'}, ${this.state.background})`}}>
                            <div className="oil-text">
                                <div className="oil-tt">Топливо</div>
                                <div className="oil-num">{this.state.fuel}</div>
                            </div>
                            <div className="oil-liner line-scale">
                                <div className="full-liner line-scale-active"
