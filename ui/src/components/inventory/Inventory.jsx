...
class Inventory extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            secondary_items_name: 'Эксклюзивная сумка',
            selected_item: {},
            ...
            equipment_weapon: [ // Экипированное оружие
                { id: 33, item_id: 119, name: "Похоже на AWP", volume: 15, desc: "AR10-1234567891222", counti: 0, params: { serial: '456', slot1: true, slot2: true, slot3: true, slot4: true } }
            ],
    ...
    componentDidCatch(error, errorInfo) {
        mp.trigger('client:ui:debug', 'Inventory.jsx', error.toString(), errorInfo.toString()); // eslint-disable-line
    }
    ...
    componentDidMount() {
        ...
        EventManager.addHandler('inventory', value => {
            ...
            if (value.type === 'updateSubItems') {
                ...
                this.setState({ secondary_items_name: value.name });
            }
    ...
    updateStacks(inventory) {
        ...
        updatedItemsCounted.sort((a, b) => a.name > b.name ? 1 : -1);
        inventory === 'primary' ?
    ...
    getActions(item, source) {
        if ((this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || source === 'inventory' || source === 'weapon' || source === 'outfit') {
            ...
            else actions = ["drop", "close"]; // Стандартные действия для всех предметов (передать, выбросить, закрыть)
            ...
            if ((this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || source === 'inventory') { // По айди предмета (item_id) определяет какие действия можно совершить с предметом
                actions.push('give') // Передать
            }
            ...
            if (this.state.itemsById.usable.includes(item.item_id) && (this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || this.state.itemsById.usable.includes(item.item_id) && source === 'inventory') { // По айди предмета (item_id) определяет какие действия можно совершить с предметом
                actions.push('use') // Использовать
            }
            if (this.state.itemsById.usablePlayer.includes(item.item_id) && (this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || this.state.itemsById.usablePlayer.includes(item.item_id) && source === 'inventory') { // По айди предмета (item_id) определяет какие действия можно совершить с предметом
                actions.push('usePlayer') // Использовать
            }
            if (this.state.itemsById.equipable.includes(item.item_id) && source !== 'weapon' && (this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || this.state.itemsById.equipable.includes(item.item_id) && source !== 'weapon' && source === 'inventory') { // По айди предмета (item_id) определяет какие действия можно совершить с предметом
                actions.push('equip') // Экипировать
            }
            if (this.state.itemsById.putOnGun.includes(item.item_id) && source !== 'weapon' && (this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || this.state.itemsById.putOnGun.includes(item.item_id) && source !== 'weapon' && source === 'inventory') { // По айди предмета (item_id) определяет какие действия можно совершить с предметом
                actions.push('put_on_gun') // Надеть на оружие
            }
            if (this.state.itemsById.ammo.includes(item.item_id) && (this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || this.state.itemsById.ammo.includes(item.item_id) && source === 'inventory') { // По айди предмета (item_id) определяет какие действия можно совершить с предметом
            ...
            for (let i = 0; i < Object.keys(this.state.outfitById).length; i++) {
                if (Object.values(this.state.outfitById)[i].includes(item.item_id) && (this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || Object.values(this.state.outfitById)[i].includes(item.item_id) && source === 'inventory') {
                    if (source !== 'outfit') {
                        actions.push('put_on') // Надеть
                        break;
                    } else {
    ...
    handlePos(e, item, source) { // Функция которая генерирует меню взаимодействия
        if ((this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) && source === 'secondary_inv' || source === 'inventory' || source === 'weapon' || source === 'outfit') {
            ...
            let actions = [];
            if (source === 'weapon' || source === 'outfit') actions = ["drop", "close"];
            else actions = ["drop", "close"]; // Стандартные действия для всех предметов (передать, выбросить, закрыть)"give"
            if (source === 'weapon') { // По айди предмета (item_id) определяет какие действия можно совершить с предметом
                actions.push('select') // Выбрать оружие
                actions.push('unloadW') // Выбрать оружие
            }
        ...
        switch (source) {
            case 'inventory':
                if (this.checkItem(item, 'inventory') !== null) {
                    ...
                    item = this.checkItem(item, 'inventory')
                    this.setState({ items: this.arrayRemove(this.state.items, item) })
                    mp.trigger('client:inventory:use', item.id, item.item_id, 1); // eslint-disable-line
                }
                break;
            case 'secondary_inv':
                if (this.checkItem(item, 'secondary_inv') !== null) {
                    ...
                    item = this.checkItem(item, 'secondary_inv')
                    this.setState({ secondary_items: this.arrayRemove(this.state.secondary_items, item) })
                    mp.trigger('client:inventory:use', item.id, item.item_id, this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()); // eslint-disable-line
                }
                break;
            default:
                break;
        ...
        switch (source) {
            case 'inventory':
                if (this.checkItem(item, 'inventory') !== null) {
                    ...
                    this.setCooldown(item.item_id);
                    item = this.checkItem(item, 'inventory');
                    mp.trigger('client:inventory:usePlayer', item.id, item.item_id, 1); // eslint-disable-line
                }
                break;
            case 'secondary_inv':
                if (this.checkItem(item, 'secondary_inv') !== null) {
                    ...
                    this.setCooldown(item.item_id);
                    item = this.checkItem(item, 'secondary_inv');
                    mp.trigger('client:inventory:usePlayer', item.id, item.item_id, this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()); // eslint-disable-line
                }
                break;
            default:
                break;
    ...
    itemOpenBag(item, source) {
        switch (source) {
            case 'inventory':
                if (this.checkItem(item, 'inventory') !== null) {
                    item = this.checkItem(item, 'inventory')
                    mp.trigger('client:inventory:openBag', item.id, item.item_id, JSON.stringify(item.params)); // eslint-disable-line
                }
                break;
            case 'outfit':
                item = this.getOutfitByType(item.type)[0]
        ...
        switch (source) {
            case 'inventory':
                if (this.checkItem(item, 'inventory') !== null) {
                    ...
                    mp.trigger('client:inventory:equip', item.id, item.item_id, item.counti, JSON.stringify(item.params), 1); // eslint-disable-line
                }
                break;
            case 'secondary_inv':
                if (this.checkItem(item, 'secondary_inv') !== null && this.state.secondary_items_owner_type == 7) {
                    this.closeInterMenu(null, { action: null });
                    item = this.checkItem(item, 'secondary_inv')
                    this.setState({ secondary_items: this.arrayRemove(this.state.secondary_items, item) })
                    this.setState({ equipment_weapon: this.state.equipment_weapon.concat(item) })
                    ...
                    mp.trigger('client:inventory:equip', item.id, item.item_id, item.counti, JSON.stringify(item.params), this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()); // eslint-disable-line
                }
                break;
            default:
                break;
    ...
    itemGive(item, source, trade_player_id) {
        ...
        switch (source) {
            ...
            case 'secondary_inv':
                if (this.checkItem(item, 'secondary_inv') !== null) {
                    if (this.state.secondary_items_owner_type === 2 || this.state.secondary_items_owner_type === 4 || this.state.secondary_items_owner_type === 7) {
                        item = this.checkItem(item, 'secondary_inv')
                        this.setState({ secondary_items: this.arrayRemove(this.state.secondary_items, item) })
                        console.log(`Передаю ${item.name}, ID ${item.id}, игроку ID ${trade_player_id}`)
                        ...
                        this.notifyToClient('~r~Вы не можете передавать предметы с земли или склада');
                    }
    ...
    loadW(item, source, loadw_id) {
        ...
        switch (source) {
            case 'inventory':
                if (this.checkItem(item, 'inventory') !== null) {
                    item = this.checkItem(item, 'inventory')
                    //this.setState({ items: this.arrayRemove(this.state.items, item) }), this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()
                    // // // // mp.call ... зарядить оружие ID loadw_id и удалить из инвентаря патроны ID item.id
                    mp.trigger('client:inventory:loadWeapon', item.id, item.item_id, loadw_id, item.counti, 1); // eslint-disable-line
                }
                break;
            case 'secondary_inv':
                if (this.checkItem(item, 'secondary_inv') !== null) {
                    ...
                    //this.setState({ items: this.arrayRemove(this.state.items, item) }), this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()
                    // // // // mp.call ... зарядить оружие ID loadw_id и удалить из инвентаря патроны ID item.id
                    mp.trigger('client:inventory:loadWeapon', item.id, item.item_id, loadw_id, item.counti, this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()); // eslint-disable-line
                }
                break;
            default:
                break;
        ...
        switch (source) {
            case 'inventory':
                if (this.checkItem(item, 'inventory') !== null) {
                    ...
                    mp.trigger('client:inventory:upgradeWeapon', item.id, item.item_id, upgradew, 1); // eslint-disable-line
                }
                break;
            case 'secondary_inv':
                if (this.checkItem(item, 'secondary_inv') !== null) {
                    ...
                    // // // // mp.call ... зарядить оружие ID loadw_id и удалить из инвентаря патроны ID item.id
                    console.log(upgradew);
                    mp.trigger('client:inventory:upgradeWeapon', item.id, item.item_id, upgradew, this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()); // eslint-disable-line
                }
                break;
            default:
                break;
        ...
        switch (source) {
            case 'inventory':
                if (this.checkItem(item, 'inventory') !== null) {
                    ...
                    this.setCooldown(item.item_id);
                    mp.trigger('client:inventory:drop', item.id, item.item_id, 1); // eslint-disable-line
                }
                break;
            case 'secondary_inv':
                if (this.checkItem(item, 'secondary_inv') !== null) {
                    ...
                    mp.trigger('client:inventory:drop', item.id, item.item_id, this.state.secondary_items_owner_type, this.state.secondary_items_owner_id.toString()); // eslint-disable-line
                }
                break;
            case 'outfit':
                item = this.getOutfitByType(item.type)[0]
                            ...
                            <div className="inv-row-main">
                                <div className="player-inv">
                                    <div className="title-inv">
                                        <span className="title-inv__name" id="title_inventory">Инвентарь</span>
                                        <div className={this.state.weight_max >= 999999999 ? "bar-inv-inf" : "bar-inv"}>
                                            <ProgressBar
                                                completed={this.state.weight_now}
                                                maxCompleted={this.state.weight_max}
                                                className={this.state.weight_max >= 999999999 ? "wrapper_bar_close" : "wrapper_bar"}
                                                customLabel=" "
                                                width="100%"
                                                height="5px"
                                                baseBgColor="rgba(255, 255, 255, 0.15)"
                                                bgColor={((this.state.weight_now / this.state.weight_max) * 100) <= 50 ? "rgba(255, 255, 255, 0.85)" : ((this.state.weight_now / this.state.weight_max) * 100) <= 85 ? "rgba(242, 201, 76, 1)" : "rgba(235, 87, 87, 1)"}
                                            />
                                            <div className={this.state.weight_max >= 999999999 ? "inf-open" : "inf-close"} />
                                            <span className={this.state.weight_max >= 999999999 ? "weight-title-inv-id-close" : "weight-title-inv-id"}>
                                                <span className="weight_now" style={{ color: ((this.state.weight_now / this.state.weight_max) * 100) <= 50 ? "rgba(255, 255, 255, 0.85)" : ((this.state.weight_now / this.state.weight_max) * 100) <= 85 ? "rgba(242, 201, 76, 1)" : "rgba(235, 87, 87, 1)" }}>{this.numberToK(this.state.weight_now)}</span>&nbsp;/&nbsp;<span className="weight_max">{this.numberToK(this.state.weight_max)}</span>
                                            </span>
                                        </div>
                                    </div>
                                                    ...
                                                    <Droppable className="droppable" id="take_off" that={this}>
                                                        <div className="object-box" style={this.isCooldownActive(item.item_id) ? { opacity: 0.2 } : { opacity: 1 }}
                                                            ...
                                                            <div className="obj-inf-box">
                                                                ...
                                                                <span className="obj-inf-desc">{item.desc}</span>
                                                            </div>
                                                            <div className="obj-inf-box-second">
                                                                <span className="obj-inf-volume">{item.volume} см³</span>
                                                                {item.count > 1 ?
                                                                    <span className="obj-inf-count">{item.count} шт.</span> : null}
                                                            </div>
                                        ...
                                        {this.state.outfit[0].map((item, i) => {
                                                    ...
                                                    <Draggable that={this} id={index} type="take_off drop" item={item} key={index}>
                                                        <Droppable className="droppable" id="put_on" that={this}>
                                                            <div onContextMenu={(e) => this.handlePos(e, item, 'outfit')} className={`item__background outfit-pos-${item.iconName}`} key={index}>
                                                                <img src={`https://appi-rp.ru/client/images/icons/components/inventory/img/outfit/${item.iconName}.png`} />
                                                            </div>
                                                        </Droppable>
                                                    </Draggable>
                                                )
                                            ...
                                            return (
                                                <Droppable className="droppable" key={index} id="put_on" that={this}>
                                                    <div onContextMenu={(e) => this.handlePos(e, item, 'outfit')} className={`item__background outfit-pos-${item.iconName}`} key={index}>
                                                        <img src={`https://appi-rp.ru/client/images/icons/components/inventory/img/outfit/${item.iconName}.png`} style={{ filter: 'grayscale(100%)', opacity: '0.7' }} />
                                                    </div>
                                                </Droppable>
                                            )
                                        })}
                                ...
                                <div className="player-craft">
                                    <div className="title-inv-craft">
                                        <div
                                            className="close-window-craft"
                                            onClick={() => this.closeInventory()}
                                        >
                                            <img src={`https://appi-rp.ru/client/images/icons/components/inventory/img/close.svg`} width="40%" className="close-window-craft__img" />
                                        </div>
                                        <span className="title-inv__name">
                                            Мастерская
                                        </span>
                                    ...
                                    <div className="menu-craft-change">
                                        <React.Fragment>
                                            ...
                                            <label htmlFor="btn-craft-class1" className="btn-craft-weapon">
                                    ...
                                    {this.state.craft ?
                                            ...
                                            {this.state.recipes_craft && Object.entries(this.state.selected_recipe).length !== 0 && this.state.selected_recipe.constructor === Object ?
                                                <React.Fragment>
                                                    ...
                                                    <div className="crafting-object-main">
                                                        <div className="persantage-craft-time">
                                                        ...
                                                        <div className="main-box-craft-subject">
                                                            <div className="box-craft-object box-cr-top box-cr-right">
                                                            ...
                                                            <div className="box-craft-object box-cr-top">
                                                            ...
                                                            <div className="box-craft-object box-cr-bottom box-cr-right">
                                                            ...
                                                            <div className="box-craft-object box-cr-bottom">
                                                    ...
                                                    <div className="craft-btn-main">
                                                        <div className="craft-btn-obj" id="craft-btn-obj-close" onClick={() => this.closeRecipesCraft()}>
                                                            <span>Закрыть</span>
                                                        </div>
                                                        <div className="craft-btn-obj" id="craft-btn-obj-craft" onClick={() => this.craftItem()}>
                                                            <span>Крафт</span>
                                                        </div>
                                                    </div>
                                                </React.Fragment>
                                        ...
                                        <React.Fragment>
                                                    ...
                                                    return (
                                                        <Draggable that={this} id={index} type="unequip drop" item={item} key={index}>
                                                            <Droppable className="droppable" id="equip" that={this}>
                                                                <div className={`${item.id === this.state.selected_weapon_id ? "weapon-player-equip__container_selected" : 'weapon-player-equip__container'}`}>
                                                                    <div
                                                                        key={index}
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div className="style-weapon-txt-craft__wname_block">
                                                                            <span className="style-weapon-helper"></span>
                                                                            <span className="style-weapon-txt-craft__wname">{item.name}</span>
                                                                        </div>
                                                                        <div className="style-desc-weapon">
                                                                            <span className="style-serial-weapon" id='style-serial-weapon-id'>{item.desc.replace(/·.*/, "")}</span>
                                                                            <span className="style-serial-weapon" id='style-serial-weapon-countI'>{item.counti}%</span>
                                                                            <span className="style-serial-weapon" id='style-serial-weapon-volume'>{this.numberToK(item.volume)}</span>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </Droppable>
                                            ...
                                            <div className="weapon-craft-box">
                                                <div className="line_space_weapon_craft">
                                                </div>
                                ...
                                <Droppable className="trunk-info-menu droppable" id="move" that={this}>
                                    <React.Fragment>
                                        <div className="title-secondary-inv">
                                            ...
                                            <span className="secondary-title-inv__name">
                                                {this.state.secondary_items_name}
                                            </span>
                                            <div className={this.state.secondary_weight_max >= 999999999 ? "bar-inv-inf" : "bar-inv"}>
                                                <ProgressBar
                                                    completed={this.state.secondary_weight_now}
                                                    maxCompleted={this.state.secondary_weight_max}
                                                    className={this.state.secondary_weight_max >= 999999999 ? "wrapper_bar_close" : "wrapper_bar"}
                                                    customLabel=" "
                                                    width="100%"
                                                    height="5px"
                                                    baseBgColor="rgba(255, 255, 255, 0.15)"
                                                    bgColor={((this.state.secondary_weight_now / this.state.secondary_weight_max) * 100) <= 50 ? "rgba(255, 255, 255, 0.85)" : ((this.state.secondary_weight_now / this.state.secondary_weight_max) * 100) <= 85 ? "rgba(242, 201, 76, 1)" : "rgba(235, 87, 87, 1)"}
                                                />
                                                <div className={this.state.secondary_weight_max >= 999999999 ? "inf-open" : "inf-close"} />
                                                <span className={this.state.secondary_weight_max >= 999999999 ? "weight-title-inv-trunk-close" : "weight-title-inv-trunk"}>
                                                    <span className="weight_now" style={{ color: ((this.state.secondary_weight_now / this.state.secondary_weight_max) * 100) <= 50 ? "rgba(255, 255, 255, 0.85)" : ((this.state.secondary_weight_now / this.state.secondary_weight_max) * 100) <= 85 ? "rgba(242, 201, 76, 1)" : "rgba(235, 87, 87, 1)" }}>{this.numberToK(this.state.secondary_weight_now)}</span>&nbsp;/&nbsp;<span className="weight_max">{this.numberToK(this.state.secondary_weight_max)}</span>
                                                </span>
                                            </div>
