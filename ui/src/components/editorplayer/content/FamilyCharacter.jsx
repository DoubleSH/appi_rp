...
class FamilyCharacter extends React.Component {
    ...
    componentDidCatch(error, errorInfo) {
        mp.trigger('client:ui:debug', 'FamilyCharacter.jsx', error.toString(), errorInfo.toString()); // eslint-disable-line
    }

    render() {
        return (
            <React.Fragment>
                <div className="menu-editor-default family-character">
                    ...
                    >
                        <div className="select-slider">
                            <span className="prev"
                                onClick={this.props.chekedSex.bind(this)}></span>
                            <div className="select-slider-label">
                                Пол:&nbsp;
                                <span>
                                    {!this.props.gender ? "Мужской" : "Женский"}
                                </span>
                            </div>
                            <span className="next" onClick={this.props.chekedSex.bind(this)}></span>
                        </div>
                    </div>
                    <SliderArrow
                        index={0}
                    ...
                    <SliderArrow
                        ...
                        clickRightArrow={this.props.clickRightArrow.bind(this)}
                    />
                    <div className="block-editor-range__element">
                        <div className="range-description"><span>{this.props.sliderHeadAndBody.minLabel}</span><span className="bold">{this.props.title_three}</span><span>{this.props.sliderHeadAndBody.maxLabel}</span></div>
                        <div className="range-slider-editor-big">
                            <InputRange
                                maxValue={this.props.sliderHeadAndBody.maxValue}
                                minValue={this.props.sliderHeadAndBody.minValue}
                                value={this.props.sliderHeadAndBody.index_help}
                                onChange={value => this.props.onInputRangeChange(value, 2)}
                                onChangeComplete={value => console.log(value)}
                            />
                            {this.props.sliderHeadAndBody.index_help !== 0 ? <div className="range-label">{this.props.sliderHeadAndBody.index_help}%</div> : null}
                        </div>
                    </div>
                    <div className="block-editor-range__element">
                        <div className="range-description"><span>{this.props.sliderSkin.minLabel}</span><span className="bold">{this.props.title_four}</span><span>{this.props.sliderSkin.maxLabel}</span></div>
                        <div className="range-slider-editor-big">
                            <InputRange
                                maxValue={this.props.sliderSkin.maxValue}
                                minValue={this.props.sliderSkin.minValue}
                                value={this.props.sliderSkin.index_help}
                                onChange={value => this.props.onInputRangeChange(value, 3)}
                                onChangeComplete={value => console.log(value)}
                            />
                            {this.props.sliderSkin.index_help !== 0 ? <div className="range-label">{this.props.sliderSkin.index_help}%</div> : null}
                        </div>
                    </div>
                    ...
                    <div className="buttons-wrap">
                        <div className="button" onClick={this.props.reset.bind(this)}>Назад</div>
                        <div className="button" onClick={this.props.reset.bind(this)}>Сброс</div>
                        <Link className="button blue" to="/editor/editor-character/face-first">
                            Далее
                        </Link>
                    </div>
                    <div className="button purple" onClick={this.props.randomize.bind(this)}>
                        Рандомные параметры
