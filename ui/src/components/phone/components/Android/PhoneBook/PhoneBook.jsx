...
class PhoneBook extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            search: '',
            pagesList: [
                {icon: `https://appi-rp.ru/client/images/icons/components/phone/img/Favorites.svg`, name: 'Избранное', page: 'favorit'},
                {icon: `https://appi-rp.ru/client/images/icons/components/phone/img/Recents.svg`, name: 'Журнал', page: 'history'},
                {icon: `https://appi-rp.ru/client/images/icons/components/phone/img/Contacts.svg`, name: 'Контакты', page: 'contact'},
                {icon: `https://appi-rp.ru/client/images/icons/components/phone/img/keyboard.svg`, name: 'Клавиши', page: 'calls'}
            ]
        }
    }
    ...
    componentDidCatch(error, errorInfo) {
        mp.trigger('client:ui:debug', 'PhoneBook.jsx', error.toString(), errorInfo.toString()); // eslint-disable-line
    }
                                ...
                                <div className="calls__navbar">
                                    {this.state.pagesList.map((item, index) => (
                                        <div onClick={() => this.setActivePage(item.page)} className="calls__navbar__item" key={`calls__navbar__item-${index}`}>
                                            <img src={item.icon} style={{width: '37px', height: '35px'}} className={this.state.page === item.page ? `calls__navbar__item__icon-active` : `calls__navbar__item__icon`} />
